...
try
    fnout = fullfile(rc.eyeOutputDir, ['21-Sep-2015_' mouse_str]);
    load([fnout 'EyeSummary.mat'])
catch
    fnout = fullfile(rc.eyeOutputDir, [date '_' mouse_str]);
    load([rc.eyeOutputDir '\20-Sep-2015_i613_i614_EyeSummary'])
end
...
for imouse = 1:size({av.mouse},2)
    col = av(imouse).col_str;
    %transient
    a = mouse(imouse).align(ialign).av(1).outcome(out_mat(1)).avg_hor_trans(:,1)-mouse(imouse).align(1).av(1).outcome(out_mat(1)).avg_hor_pre(:,1);
    b = mouse(imouse).align(ialign).av(2).outcome(out_mat(2)).avg_hor_trans(:,1)-mouse(imouse).align(1).av(2).outcome(out_mat(2)).avg_hor_pre(:,1);
    scatter(a, b,200, ['*' col]);
    hold on;
    
    %sustained
    a = mouse(imouse).align(ialign).av(1).outcome(out_mat(1)).avg_hor_sust(:,1)-mouse(imouse).align(1).av(1).outcome(out_mat(1)).avg_hor_pre(:,1);
    b = mouse(imouse).align(ialign).av(2).outcome(out_mat(2)).avg_hor_sust(:,1)-mouse(imouse).align(1).av(2).outcome(out_mat(2)).avg_hor_pre(:,1);
    scatter(a, b, 200, ['o' col]);
    hold on;
...
for imouse = 1:size({av.mouse},2)
    col = av(imouse).col_str;
    %transient
    a = mouse(imouse).align(ialign).av(1).outcome(out_mat(1)).avg_ver_trans(:,1)-mouse(imouse).align(1).av(1).outcome(out_mat(1)).avg_ver_pre(:,1);
    b = mouse(imouse).align(ialign).av(2).outcome(out_mat(2)).avg_ver_trans(:,1)-mouse(imouse).align(1).av(2).outcome(out_mat(2)).avg_ver_pre(:,1);
    scatter(a, b, 200,['*' col]);
    hold on;
    
    %sustained
    a = mouse(imouse).align(ialign).av(1).outcome(out_mat(1)).avg_ver_sust(:,1)-mouse(imouse).align(1).av(1).outcome(out_mat(1)).avg_ver_pre(:,1);
    b = mouse(imouse).align(ialign).av(2).outcome(out_mat(2)).avg_ver_sust(:,1)-mouse(imouse).align(1).av(2).outcome(out_mat(2)).avg_ver_pre(:,1);
    scatter(a, b, 200, ['o' col]);
    hold on;
