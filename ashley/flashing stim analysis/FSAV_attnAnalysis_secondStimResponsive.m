...
if cellsOrDendrites == 1
    fnout = fullfile(rc.caOutputDir, dataGroup,[titleStr '_startAlign_secondStimRespCells']); 
elseif cellsOrDendrites == 2
...
%             mouse(imouse).expt(iexp).align(pressAlign).av(auditoryTrials).outcome(hitTrials).cmlvCycResp{1});
        ...
        dAV_cycle = cat(3,...
            ...
            visualTrials).outcome(hitTrials).cmlvCycResp{2},...
            mouse(imouse).expt(iexp).align(pressAlign).av(...
            auditoryTrials).outcome(hitTrials).cmlvCycResp{2});

        respwin_shift = respwin+cycLengthFr;
        basewin_shift = basewin_0+cycLengthFr;

        secondStimRespCells = ttest(...
            squeeze(mean(dAV_cycle(respwin_shift,:,:),1)),...
            squeeze(mean(dAV_cycle(basewin_shift,:,:),1)),...
            'dim',2,'tail','right','alpha',cellGroupsAlpha)' ... 
            & mean(mean(dAV_cycle(respwin,:,:),3),1) > minRespThreshold;
        
        responsiveCellsAV{exptN} = secondStimRespCells;
