function mouse = createFSHVADataStruct2(datasetStr,cellsOrDendrites)
...
%         fnout = fullfile(rc.caOutputDir, expt(iexp).mouse, expt(iexp).folder, expt(iexp).date, [expt(iexp).date '_' expt(iexp).mouse '_' runstr '_']);
        ...
        elseif expt(iexp).folder(1:5) == 'linds'
            runstr = [expt(iexp).runs(1,:) '_' expt(iexp).dirtuning];
             fnTC = fullfile(rc.lindseyAnalysis,...
                [expt(iexp).date '_i' expt(iexp).mouse],[expt(iexp).date '_i' expt(iexp).mouse '_runs-' runstr]);
            load(fullfile(fnTC,[expt(iexp).date '_i' expt(iexp).mouse '_runs-' runstr '_TCs.mat']))
            dataTC = npSub_tc;
            fprintf([num2str(size(npSub_tc,2)) '/n'])
...
%             catchOutcome = catchOutcome(tr);
        ...
        for irun = 1:nrun
            ...
            elseif expt(iexp).folder(1:5) == 'linds'
                if ~isnan(expt(iexp).frames(irun))
                    nfr_run = double(expt(iexp).frames(irun));
                else
                    time = expt(iexp).time_mat(irun,:);
                    fn_mworks = [rc.pathStr...
                        '\data-i' expt(iexp).SubNum '-' expt(iexp).date '-' time '.mat'];
                    mworks = mwLoadData(fn_mworks, [], []);
                    nfr_run = double(mworks.counterValues{end}(end));
                end
            end
        ...
        elseif expt(iexp).folder(1:5) == 'linds'
            fnTun = fullfile(rc.lindseyAnalysis,...
                [expt(iexp).date '_i' expt(iexp).mouse],[expt(iexp).date '_i' expt(iexp).mouse '_runs-' expt(iexp).dirtuning]);
            load(fullfile(fnTun,[expt(iexp).date '_i' expt(iexp).mouse '_runs-' expt(iexp).dirtuning '_oriTuningAndFits.mat']));
            vonMisesFitAllCells = squeeze(vonMisesFitAllCellsAllBoots(:,1,:));
        end
        
        mouse(imouse).expt(exptN(:,imouse)).area = expt(iexp).img_loc{1};
    ...
    %%
    if cellsOrDendrites == 1
        save(fullfile(rc.caOutputDir, dataGroup, [mouse_str '_trOutcomeStruct_cells_' datasetStr '.mat']), 'mouse','-v7.3');
%         print(fullfile(rc.caOutputDir, dataGroup, [datasetStr(5:end) '_motionHist.pdf']), '-dpdf')
    elseif cellsOrDendrites == 2
        save(fullfile(rc.caOutputDir, dataGroup, [mouse_str '_trOutcomeStruct_dendrites_' datasetStr '.mat']), 'mouse','-v7.3');
%         print(fullfile(rc.caOutputDir, dataGroup, [datasetStr(5:end) '_motionHist.pdf']), '-dpdf')
    end
end
