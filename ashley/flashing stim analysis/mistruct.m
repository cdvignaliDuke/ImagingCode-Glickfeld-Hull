        ...
        for iav = 1:2 %for both visual and auditory target enhancement 
            for iout = 1:4 %for hits, misses, invalid hits, invalid misses
                ...
                if sum(cell2mat(cellfun(@(x) size(x,3),tc_cyc(shortCycs),'unif',false))) > 1 %combine response to short and long trials, only if there is more than one total trial
                    if any (cell2mat(cellfun(@(x) length(size(x)),tc_cyc(shortCycs),'unif',false)) < 3) % need to do this to correct for reshape that happens when there is only one trial for that cycle type
                ...
                if sum(cell2mat(cellfun(@(x) size(x,3),tc_cyc(longCycs),'unif',false))) > 1  
                    if any (cell2mat(cellfun(@(x) length(size(x)),tc_cyc(longCycs),'unif',false)) < 3)
                        ind = find(cell2mat(cellfun(@(x) length(size(x)),tc_cyc(longCycs),'unif',false)) == 2);
                ...
                for imi = 1:2 
                    ...
                    % long trials
                        ...
                        auc_short = nan(1,ncells);
                        for icell = 1:ncells
                            auc_short(icell) = roc(base_cyc_short(icell,:),tar_cyc_short(icell,:));
                        end
                        ...
                        for icell = 1:ncells
                            auc_long(icell) = roc(base_cyc_long(icell,:),tar_cyc_long(icell,:));
                        end
                        ...
                        for icell = 1:ncells
                            auc_late(icell) = roc(tc_late_a_resp(icell,:),tc_late_v_resp(icell,:));
                        end
                        ...
                        for icell = 1:ncells
                            auc_early(icell) = roc(tc_early_a_resp(icell,:),tc_early_v_resp(icell,:));
                        end
