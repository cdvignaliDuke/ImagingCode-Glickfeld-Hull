...
if doLoadPreviousAnalysis
    ...
    for imouse = 1:size(mouse,2)
        for iexp = 1:size(mouse(imouse).expt,2)
            ...
            nTrialsPerExpt(exptN) = length(d.align(1).outcome) + ...
                length(d.align(1).outcome);

            cycLengthFr = d.info.cycTimeFrames;
            nCycLongTC = ceil(longTrialLengthFr./cycLengthFr);

            maxCycles = max(cat(2,d.align(alignStart).nCycles,...
                d.align(alignStart).nCycles));
                ...
                decodeDataExpt(exptN).outcome = trOut;
                decodeDataExpt(exptN).stim = trStim;
                decodeDataExpt(exptN).resp = trResp;
                decodeDataExpt(exptN).tc = trTC(1:(nBaselineFr*2),:,:);

                trStimID = discretize(trStim,unique(trStim));
    ...
    %                     fprintf(num2str(iout+4))
                ...
                targetDataExpt(exptN).trOutTC = trOutTC;



                de = d.align(alignTarget);
                
                    binEdges = oriBins;
                    targets = de.ori;
                tc = de.respTC(1:(nBaselineFr*2),:,:);
                targetInd = discretize(targets,binEdges);
                targetDataExpt(exptN).tc = cell(1,2);
                for itar = 1:nTargets
                    ind = targetInd == itar+1;
                    targetDataExpt(exptN).tc{itar} = tc(:,:,ind);
                    targetDataExpt(exptN).targets{itar} = targets(ind);
                end
            end
            ...
            if size(d.align,2) > 4
                r = squeeze(mean(...
                    d.align(5).respTC(respwin_target,:,:),1) - ...
                    mean(d.align(5).respTC(basewin_0_target,:,:),1));
                trOut =  d.align(5).outcome;
                ind = cellfun(@(x) ~isempty(x),trOut);
                trOut = trOut(ind);
                r = r(:,ind);
                trOut(strcmp(trOut,'FA')) = {'h'};
                trOut(strcmp(trOut,'CR')) = {'m'};
                decodeDataExpt(exptN).catchResp = r;
                decodeDataExpt(exptN).catchOutcome = trOut;
                decodeDataExpt(exptN).catchStim = ...
                    d.align(5).ori;
            end
            ...
            respCellsExpt(iexp).exptName = [mouse(imouse).mouse_name '-' d.date];
            respCellsExpt(iexp).decodeAnalysisCells = oriTuningExpt(exptN).isTuned;
        end
    end
...
%         dcInfo = struct;
        ...
        for iexp = 1:nexp
            ...
            cellInd = find(respCellsExpt(iexp).decodeAnalysisCells);
...
%             dcInfo(iexp).cellInd = cellInd;
            ...
            if isfield(decodeDataExpt(iexp),'catchOutcome')
                respAllCells_av_withInv = zscore(cat(2,...
                    decodeDataExpt(iexp).resp,...
                    decodeDataExpt(iexp).resp,...
                    decodeDataExpt(iexp).catchResp)');
                nvis = size(decodeDataExpt(iexp).resp,2);
                ninv = size(decodeDataExpt(iexp).catchResp,2);
                ...
                trOut = decodeDataExpt(iexp).outcome;
%                 respAllCells = zscore(decodeDataExpt(iexp).resp');
                

                    respAllCells = respAllCells_vis;
                    trStimID = discretize(decodeDataExpt(iexp).stim,oriBins);
                ...
                decodeAnalysis(iexp).respAllCells = respAllCells(matchTrialsInd,:);
                decodeAnalysis(iexp).trOut = trOut(matchTrialsInd);
                ...
                nwins = size(decodeDataExpt(iexp).movWinResp,3);
                ...
                for iwin = 1:nwins
                    rAllCells = zscore(decodeDataExpt(iexp).movWinResp(:,:,iwin)');
                ...
                decodeAnalysis(iexp).correlationDetect = detectCorr;
                decodeAnalysis(iexp).correlationTarget = targetCorr;
                decodeAnalysis(iexp).weightDetect = detectWeight;
                decodeAnalysis(iexp).weightTarget = targetWeight;
                ...
                decodeAnalysis(iexp).pctCorrectDetectMovRespWin_train = pctCorrectDetect_train_respwin;
                decodeAnalysis(iexp).pctCorrectDetectMovRespWin_holdout = pctCorrectDetect_ho_respwin;
                decodeAnalysis(iexp).pctCorrectTargetMovRespWin_train = pctCorrectTarget_train_respwin;
                decodeAnalysis(iexp).pctCorrectTargetMovRespWin_holdout = pctCorrectTarget_ho_respwin;
        ...
        for iexp = 1:nexp
            ...
            rng(0)
                ...
                for iboot = 1:nBoot
                    ...
                    dcBootstrap(iexp).cellInd{iboot} = cellInd;
                    dcBootstrap(iexp).targetCorrelation{iboot} = targetCorr;
                    dcBootstrap(iexp).detectCorrelation{iboot} = detectCorr;
                    dcBootstrap(iexp).targetWeight{iboot} = targetWeight;
                    dcBootstrap(iexp).detectWeight{iboot} = detectWeight;
                end
        ...
        targetWeightBoot = [];
        detectWeightBoot = [];
        nWeights_target = cell(nexp,1);
        nWeights_detect = cell(nexp,1);
        for iexp = 1:nexp
            n = dcBootstrap(iexp).nTotalCells;
                ...
                targetWeightBoot = cat(2,targetWeightBoot,nanmean(bootMatrix_w_target));
                detectWeightBoot = cat(2,detectWeightBoot,nanmean(bootMatrix_w_detect));
                
                nWeights_target{iexp,1} = sum(~isnan(bootMatrix_w_target));
                nWeights_detect{iexp,1} = sum(~isnan(bootMatrix_w_detect));
        end
    ...
    %         detectGLMOther = cell(1,2);
            ...
            respAllCells_vis = respAllCells_av_withInv(1:nvis,:);
                if sum(cellInd) < minCellN
                    decodeAnalysis(iexp).dvDetect = nan;
                    decodeAnalysis(iexp).dvTarget = nan;
                    decodeAnalysis(iexp).correlationDetect = [];
                    decodeAnalysis(iexp).correlationTarget = [];
                    decodeAnalysis(iexp).weightDetect = [];
                    decodeAnalysis(iexp).weightTarget = [];
                    decodeAnalysis(iexp).pctCorrectAllDetect_train = [];
                    decodeAnalysis(iexp).pctCorrectAllDetect_holdout = [];
                    decodeAnalysis(iexp).pctCorrectAllTarget_train = nan;
                    decodeAnalysis(iexp).pctCorrectAllTarget_holdout = nan;
                    decodeAnalysis(iexp).pctCorrectXStimDetect_train = [];
                    decodeAnalysis(iexp).pctCorrectXStimDetect_holdout = [];
                    decodeAnalysis(iexp).pctCorrectXStimTarget_train = nan(1,3);
                    decodeAnalysis(iexp).pctCorrectXStimTarget_holdout = nan(1,3);
                    decodeAnalysis(iexp).pctCorrectDetectMovRespWin_train = [];
                    decodeAnalysis(iexp).pctCorrectDetectMovRespWin_holdout = [];
                    decodeAnalysis(iexp).pctCorrectTargetMovRespWin_train = [];
                    decodeAnalysis(iexp).pctCorrectTargetMovRespWin_holdout = [];
...
%                     respAllCells = zscore(decodeDataExpt(iexp).resp');
            ...
            %         resp = zscore(respAllCells(cellInd,matchTrialsInd)');
                    resp = respAllCells(matchTrialsInd,cellInd);
            %         [c,score,latent] = pca(decodeDataExpt(iexp).resp(:,matchTrialsInd)');
...
%             for iav = 1:nav
%                 if sum(cellInd) < minCellN
%                     decodeAnalysis(iexp).pctCorrectTarget_otherAV = nan;
%                 else
%                     if iav == 1
%                         otherAV = 2;
%                     elseif iav == 2
...
% 
%         %             decodeAnalysis(iexp).pctCorrectDetect_otherAV = pctCorrectDetect;
%                     decodeAnalysis(iexp).pctCorrectTarget_otherAV = pctCorrectTarget;
%                 end
%             end
        end
        ...
        for iexp = 1:nexp
            ...
            nTotalCells = length(respCellsExpt(iexp).decodeAnalysisCells);
                if decodeAnalysis(iexp).nCellsSelected < minCellN
                    dcBootstrap(iexp).targetWeight = cell(1,nBoot);
                else
                    trOut = decodeDataExpt(iexp).outcome;
                    respAllCells = zscore(decodeDataExpt(iexp).resp');
                    ...
                    for iboot = 1:nBoot
                        ...
                        dcBootstrap(iexp).cellInd{iboot} = cellInd;
                        dcBootstrap(iexp).targetCorrelation{iboot} = targetCorr;
                        dcBootstrap(iexp).detectCorrelation{iboot} = [];
                        dcBootstrap(iexp).targetWeight{iboot} = targetWeight;
                        dcBootstrap(iexp).detectWeight{iboot} = [];
                    end
                end
            dcBootstrap(iexp).nTotalCells = nTotalCells;
        end
        ...
        for iexp = 1:nexp
            n = dcBootstrap(iexp).nTotalCells;
                if decodeAnalysis(iexp).nCellsSelected < minCellN
                    targetWeightBoot = cat(2,targetWeightBoot,nan(1,dcBootstrap(iexp).nTotalCells));
                else
...
for iav = 1:nav
    imgStats.lateWinSupp = mean(mean(antiAnalysis.longTC{iav}(lateWinFr,...
        (ind)),1));
    imgStats.lateWinSuppErr = ste(mean(antiAnalysis.longTC{iav}(lateWinFr,...
        (ind)),1),2);
    fprintf('%s Suppr. Late Win mean/err: %s/%s\n',avName{iav},...
        num2str(round(imgStats.lateWinSupp,2,'significant')),...
        num2str(round(imgStats.lateWinSuppErr,2,'significant')))
    
    imgStats.lateCycSupp = mean(mean(antiAnalysis.lateCycTC{iav}(respwin,...
        (ind)),1));
    imgStats.lateCycSuppErr = ste(mean(antiAnalysis.lateCycTC{iav}(respwin,...
        (ind)),1),2);
    fprintf('%s Suppr. Late Cyc Resp mean/err: %s/%s\n',avName{iav},...
        num2str(round(imgStats.lateCycSupp,2,'significant')),...
        num2str(round(imgStats.lateCycSuppErr,2,'significant')))
...
if strcmp(ds,'FSAV_attentionV1')
    ...
    for imod = 1:2
        ...
        for iav = 1:nav
            dcModel(2).pctYes = nan(3,nexp);
            dcModel(imod).dv = nan(1,nexp);
            dcModel(imod).correlation = [];
            dcModel(imod).weight = [];
            dcModel(imod).pctCorrect = nan(4,nexp);
            dcModel(imod).testPerformance = nan(4,nexp);
            dcModel(imod).pctCorrect_movRespWin = nan(nMovWin,nexp);
            dcModel(imod).pctCorrect_otherAV = nan(1,nexp);
        end
    end
    for iav = 1:nav
        for iexp = 1:nexp
            d = decodeAnalysis(iexp).av(iav);
            dcModel(1).dv(iexp) = d.dvTarget;
            dcModel(2).dv(iexp) = d.dvDetect;
%             dcModel(2).pctYes(:,iexp) = d.pctYes;

            dcModel(1).correlation = cat(2, ...
                dcModel(1).correlation,d.correlationTarget);
            dcModel(1).weight = cat(1, ...
                dcModel(1).weight,d.weightTarget);
            dcModel(1).pctCorrect(:,iexp) = cat(2,...
                d.pctCorrectXStimTarget_holdout,d.pctCorrectAllTarget_holdout);
            dcModel(1).testPerformance(:,iexp) = cat(2,...
                d.pctCorrectXStimTarget_train,d.pctCorrectAllTarget_train) - ...
                cat(2,...
                d.pctCorrectXStimTarget_holdout,d.pctCorrectAllTarget_holdout);
            dcModel(1).pctCorrect_movRespWin(:,iexp) = d.pctCorrectTargetMovRespWin_holdout;
            dcModel(1).pctCorrect_otherAV(iexp) = d.pctCorrectTarget_otherAV;

            dcModel(2).correlation = cat(2, ...
                dcModel(2).correlation,d.correlationDetect);
            dcModel(2).weight = cat(1, ...
                dcModel(2).weight,d.weightDetect);
            dcModel(2).pctCorrect(:,iexp) = cat(2,...
                d.pctCorrectXStimDetect_holdout,d.pctCorrectAllDetect_holdout);
            dcModel(2).testPerformance(:,iexp) = cat(2,...
                d.pctCorrectXStimDetect_train,d.pctCorrectAllDetect_train) - ...
                cat(2,...
                d.pctCorrectXStimDetect_holdout,d.pctCorrectAllDetect_holdout);
            dcModel(2).pctCorrect_movRespWin(:,iexp) = d.pctCorrectDetectMovRespWin_holdout;
            dcModel(2).pctCorrect_otherAV(iexp) = d.pctCorrectDetect_otherAV;
        end
    end
...
%     r = dc.tc(:,ind,trialInd);
    ...
    wDetect = decodeAnalysis(iexp).weightDetect;
    wTarget = decodeAnalysis(iexp).weightTarget;
...
% within modality decoding in V1  (Figure 4)
    ...
    for imod = 1:2
        ...
        for iav = 1:nav
            y = dcModel(imod).pctCorrect;
            yerr = ste(dcModel(imod).pctCorrect,2);
    ...
    %             ind = ~isnan(y(:,iexp));
            ...
            title(sprintf('%s Model, Train %s, Test %s',dcModel(imod).name,...
                dcModel(imod).name,dcModel(imod).name))
        end
    end
    ...
    for imod = 1:2
        ...
        for iav = 1:nav
            y = dcModel(imod).testPerformance;
            yerr = ste(dcModel(imod).testPerformance,2);
            stimInd = sum(~isnan(y),2) > minTrN;
            y = y(stimInd,:);
            yerr = yerr(stimInd,:);
            x = 1:sum(stimInd);
...
% timing of decoding (Suppl. Fig. 8)
    ...
    for imod = 1:2
        ...
        for iav = 1:nav
            y = dcModel(imod).pctCorrect_movRespWin(winInd,:);
            yerr = ste(dcModel(imod).pctCorrect_movRespWin(winInd,:),2);
            targetTimes = x(x>0);
...
for iav = 1:nav
        ...
        weightInd = dcModel(imod).weight > 0;
        ...
        binnedTargetWeight(1) = mean(dcModel(1).weight(~weightInd));
        binnedTargetWeight(2) = mean(dcModel(1).weight(weightInd));
        binnedTargetWeightErr(1) = ste(dcModel(1).weight(~weightInd),1);
        binnedTargetWeightErr(2) = ste(dcModel(1).weight(weightInd),1);
        binnedDetectWeight(1) = mean(dcModel(2).weight(~weightInd));
        binnedDetectWeight(2) = mean(dcModel(2).weight(weightInd));
        binnedDetectWeightErr(1) = ste(dcModel(2).weight(~weightInd),1);
        binnedDetectWeightErr(2) = ste(dcModel(2).weight(weightInd),1);
        [~,binnedDetectWeightTest(1)] = ttest(dcModel(2).weight(~weightInd));
        [~,binnedDetectWeightTest(2)] = ttest(dcModel(2).weight(weightInd));

        for imod = 1:2
            x = dcModel(imod).correlation;
            y = dcModel(imod).weight;
        ...
        dcModel(1).binnedWeight = binnedTargetWeight;
        dcModel(1).binnedWeightErr = binnedTargetWeightErr;
        dcModel(2).binnedWeight = binnedDetectWeight;
        dcModel(2).binnedWeightErr = binnedDetectWeightErr;
        dcModel(2).binnedWeightTest = binnedDetectWeightTest;

        print([fnout 'decodeModelWeights_' dcModel(1).name],'-dpdf','-fillpage')
end
...
% orthogonal coding (Figure 5)
    ...
    %     ind = cellInfo.lateCycRespCells;
        ...
        vline(0,'k:')
        hline(0,'k:')
        title(sprintf('Train %s, R=%s, p=%s',dcModel(1).name,...
        ...
        vline(0,'k:')
        hline(0,'k:')
        title(sprintf('Train %s',dcModel(1).name))
        ...
        figAxForm
        hline(0,'k:')
        title(sprintf('Train %s, p=%s',dcModel(1).name,...
            num2str(round(p,2,'significant'))))
        L = legend(n,'location','southwest');
...
%     multcompare(stats)
    ...
    %     ind = cellInfo.lateCycRespCells;
        ...
        vline(0,'k:')
        hline(0,'k:')
        title(sprintf('Train %s, R=%s, p=%s',dcModel(2).name,...
            num2str(round(r,2,'significant')),num2str(round(p,2,'significant'))))
    ...
    for imod = 1:2
        for iav = 1:nav
            ...
            x = 1:2;
            y = [dcModel(imod).pctCorrect(end,:)', dcModel(imod).pctCorrect_otherAV'];
            hold on
            plot(x,y,'k')
            ...
            figXAxis([],'Test Trials',[0 3],x,{dcModel(imod).name,otherAV})
            figYAxis([],'Fraction Correct',[0 1])
            figAxForm
            hline(0.5,'k:')
            title(sprintf('%s Model, Train %s',dcModel(imod).name,dcModel(imod).name))
            for i = 1:2
                if p(i) < 0.05/7
...
for iav = 1:nav
    ...
    for iexp = 1:nexp
%             if iav == 1
            y = cat(1,y,[decodeAnalysis(iexp).pctCorrectAllTarget_holdout,...
                decodeAnalysis(iexp).pctCorrectTarget_comboTrain]);
%             else
%                 y = cat(1,y,[decodeAnalysis(iexp).pctCorrectTarget_audTrainComboMatch,...
%                     decodeAnalysis(iexp).pctCorrectTarget_comboTrain]);
%             end
    end
    plot(1:2,y,'k-')
    ...
    for iexp = 1:nexp
%             if iav == 1
            y = cat(1,y,[decodeAnalysis(iexp).pctCorrectAllDetect_holdout,...
                decodeAnalysis(iexp).pctCorrectTarget_comboTrain]);
%             else
%                 y = cat(1,y,[decodeAnalysis(iexp).pctCorrectDetect_audTrainComboMatch,...
%                     decodeAnalysis(iexp).pctCorrectTarget_comboTrain]);
%             end
    end
    plot(1:2,y,'k-')
...
for iav = 1:nav
    ...
    if iav == 1
        [p,~,stats] = anova1(dcModel(1).pctCorrect(stimID,:)',[],'off');
        tbl = multcompare(stats,[],'off');
        tbl = tbl(:,[1:2,end]);
    else
        y = dcModel(1).pctCorrect(stimID,:)';
        [~,p] = ttest(y(:,1),y(:,2));
        tbl = [];
    end
    imgStats.targetPerformanceXStimAnova = p;
    imgStats.targetPerformanceXStimPostHocTests = tbl;
    fprintf('%s Target Performance x Stim ANOVA, p=%s\n',nav,num2str(round(...
        imgStats.targetPerformanceXStimAnova,2,'significant')))
    disp(imgStats.targetPerformanceXStimPostHocTests)
    
    if iav == 1
        [p,~,stats] = anova1(dcModel(2).pctCorrect(stimID,:)',[],'off');
        tbl = multcompare(stats,[],'off');
        tbl = tbl(:,[1:2,end]);
    else
        y = dcModel(2).pctCorrect(stimID,:)';
        [~,p] = ttest(y(:,1),y(:,2));
        tbl = [];
    end
    imgStats.detectPerformanceXStimAnova = p;
    imgStats.detectPerformanceXStimPostHocTests = tbl;
    fprintf('%s Detect Performance x Stim ANOVA, p=%s\n',nav,num2str(round(...
        imgStats.detectPerformanceXStimAnova,2,'significant')))
    disp(imgStats.detectPerformanceXStimPostHocTests)
end
...
%     dcModel(1).cellTypeID = cellTypeIDAllCells(cellInfo.dcModelCells);
    ...
    for iav = 1:nav
        dcModel.dv = nan(1,nexp);
        dcModel.correlation = [];
        dcModel.weight = [];
        dcModel.pctCorrect = nan(4,nexp);
        dcModel.testPerformance = nan(4,nexp);
        dcModel.pctCorrect_otherAV = nan(1,nexp);
    end
    for iav = 1:nav
        for iexp = 1:nexp
            d = decodeAnalysis(iexp).av(iav);
    ...
    figure
    for iav = 1:nav
        y = dcModel.pctCorrect;
        yerr = ste(dcModel.pctCorrect,2);
        stimInd = sum(~isnan(y),2) > minTrN;
        y = y(stimInd,:);
%         yerr = yerr(stimInd,:);
        x = 1:sum(stimInd);
