...
for iexp=1:nexp
   ...
   X_targets_vis = zscore(X_targets_vis);
...
%    pcNumber = cat(2,pcNumber,1:npc);
   ...
   detectWeightsAll_vis = cat(1,detectWeightsAll_vis,B_yeses);
    ...
    yhat_target = glmval(targetGLM_vis.beta,X_targets_vis,'logit') > dv_target_vis;
    pctCorr_target_vis = mean((Y_targets_vis-yhat_target) == 0);
   
    yhat_detect = glmval(detectGLM_vis.beta,X_targets_vis,'logit') > dv_detect_vis;
    ...
    for i = 1:nTrials_all
        ...
        ysub_targetHoldouts(i) = Y_holdout - (yhat > dv_target_vis);
        ...
        ysub_detectHoldouts(i) = Y_holdout - (yhat > dv_detect_vis);       
    end
...
%     nOri = length(orientations);
   ...
   for iori = 1:nOri
        ...
        X = X_targets_vis(trInd,:);
        
        yhat = glmval(detectGLM_vis.beta,X,'logit') > dv_detect_vis;
        pctCorrOri_detect(iori) = mean((Y_yeses_vis(trInd) - yhat) == 0);
        
        yhat = glmval(targetGLM_vis.beta,X,'logit') > dv_target_vis;
        ...
        for i = 1:nTrials
            ...
            yhat_detectHoldout = glmval(...
                detectOthersGLM.beta,X_holdout,'logit') > dv_detect_vis;
            ...
            yhat_targetHoldouts = glmval(...
                targetOthersGLM.beta,X_holdout,'logit') > dv_target_vis;
...
%****AUDITORY TRIALS   
   ...
   tAmp = dcExpt(iexp).audTrialAmp;
    ...
    nAmp_minCutoff = sum(nAmpBin > minTrN);
    minBinN = min(nAmpBin);
    if minBinN < minTrN
   ...
   X_targets_aud = zscore(X_targets_aud);
...
%    pcNumber = cat(2,pcNumber,1:npc);
   ...
   detectWeightsAll_aud = cat(1,detectWeightsAll_aud,B_yeses);
    ...
    yhat_target = glmval(targetGLM_aud.beta,X_targets_aud,'logit') > dv_target_aud;
    pctCorr_target_aud = mean((Y_targets_aud-yhat_target) == 0);
   
    yhat_detect = glmval(detectGLM_aud.beta,X_targets_aud,'logit') > dv_detect_aud;
    ...
    nTrials_all = size(X_targets_aud,1);
   
    ysub_targetHoldouts = nan(1,nTrials_all);
    ysub_detectHoldouts = nan(1,nTrials_all);
    for i = 1:nTrials_all
        X_holdout = X_targets_aud(i,:);
        othersInd = [1:(i-1),(i+1):nTrials_all];
        X_others = X_targets_aud(othersInd,:);
        
        [~,~,targetOthersGLM] = glmfit(X_others,Y_targets_aud(othersInd),'binomial');        
        Y_holdout = Y_targets_aud(i);
        yhat = glmval(targetOthersGLM.beta,X_holdout,'logit');
        ysub_targetHoldouts(i) = Y_holdout - (yhat > dv_target_aud);
        
        [~,~,detectOthersGLM] = glmfit(X_others,Y_yeses_aud(othersInd),'binomial');
        Y_holdout = Y_yeses_aud(i);
        yhat = glmval(detectOthersGLM.beta,X_holdout,'logit');
        ysub_detectHoldouts(i) = Y_holdout - (yhat > dv_detect_aud);       
    end
...
%     nAmp = length(amplitudes);
   ...
   for iamp = 1:nAmp
        ...
        X = X_targets_aud(trInd,:);
        
        yhat = glmval(detectGLM_aud.beta,X,'logit') > dv_detect_aud;
        pctCorrAmp_detect(iamp) = mean((Y_yeses_aud(trInd) - yhat) == 0);
        
        yhat = glmval(targetGLM_aud.beta,X,'logit') > dv_target_aud;
        ...
        for i = 1:nTrials
            ...
            yhat_detectHoldout = glmval(...
                detectOthersGLM.beta,X_holdout,'logit') > dv_detect_aud;
            ...
            yhat_targetHoldouts = glmval(...
                targetOthersGLM.beta,X_holdout,'logit') > dv_target_aud;
   ...
   pctCorrTarget_audXamp_XVal(iexp,ind) = pctCorrXValAmp_target;
    ...
    yhat_target = glmval(targetGLM_vis.beta,X_targets_aud,'logit') > dv_target_aud;
    pctCorr_target_testAudXamp(iexp,end) = mean((Y_targets_aud-yhat_target) == 0);
    
    yhat_detect = glmval(detectGLM_vis.beta,X_targets_aud,'logit') > dv_detect_aud;
    ...
    for iamp = 1:nAmp
        ...
        yhat = glmval(detectGLM_vis.beta,X,'logit') > dv_detect_aud;
        pctCorr_detect(iamp) = mean((Y_yeses_aud(trInd) - yhat) == 0);
        
        yhat = glmval(targetGLM_vis.beta,X,'logit') > dv_target_aud;
   ...
   ind = ismember(allAmps,amplitudes);
    ...
    yhat_target = glmval(targetGLM_aud.beta,X_targets_vis,'logit') > dv_target_aud;
    pctCorr_target_testVisXori(iexp,end) = mean((Y_targets_vis-yhat_target) == 0);
    
    yhat_detect = glmval(detectGLM_aud.beta,X_targets_vis,'logit') > dv_detect_aud;
...
pctCorrOri_detect_sub = cellfun(@(x,y) x-y,pctCorrDetect_visXori,...
    pctCorrDetect_visXori_XVal,'unif',0);
pctCorrOri_target_sub = cellfun(@(x,y) x-y,pctCorrTarget_visXori,...
