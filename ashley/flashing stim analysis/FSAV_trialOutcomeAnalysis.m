...
figure
suptitle(sprintf('First Stim Responsive Cells (%s)',num2str(sum(firstRespCellsInd_FACR))))
for iav = 1:nav
    leg = [];
    subplot(1,2,iav)
    x = mean(respTC{faInd,iav}(ttWinInd,firstRespCellsInd_FACR),2);
    xerr = ste(respTC{faInd,iav}(ttWinInd,firstRespCellsInd_FACR),2);
    ...
    leg(1) = h.mainLine;
    hold on
    x = mean(respTC{crInd,iav}(ttWinInd,firstRespCellsInd_FACR),2);
    xerr = ste(respTC{crInd,iav}(ttWinInd,firstRespCellsInd_FACR),2);
...
for iav = 1:nav
    ...
    leg(1) = h.mainLine;
    hold on
    x = nanmean(respTCmatched{mInd,iav}(ttWinInd,targetRespCellsInd),2);
    xerr = ste(respTCmatched{mInd,iav}(ttWinInd,targetRespCellsInd),2);
