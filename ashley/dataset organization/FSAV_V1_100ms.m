%% AW13 150504
expt(1).SubNum = '613';
expt(1).mouse = 'AW13';
expt(1).date = '150504';
expt(1).img_loc  = {'V1';'L2/3'};
expt(1).img_strct  = {'cells'};
expt(1).time_mat = ['1425'; '1441';'1457'];
expt(1).runs = ['002'; '003'; '004'];
expt(1).nrun = size(expt(1).runs,1);
expt(1).frame_rate = 30;
expt(1).folder = 'two-photon imaging';
expt(1).catch = 0;
expt(1).catchRew = 0;
expt(1).dirtuning = '006';
expt(1).dirtuning_time = '1518';
expt(1).rettuning = {'005';'1514'};
expt(1).z = 299;
expt(1).regImgStartFrame = 49839;
expt(1).trial_range = nan;

%% AW13 150508
expt(2).SubNum = '613';
expt(2).mouse = 'AW13';
expt(2).date = '150508';
expt(2).img_loc  = {'V1';'L2/3'};
expt(2).img_strct  = {'cells'};
expt(2).time_mat = ['1413'; '1430'; '1446'];
expt(2).runs = ['001'; '002'; '003'];
expt(2).nrun = size(expt(2).runs,1);
expt(2).frame_rate = 30;
expt(2).folder = 'two-photon imaging';
expt(2).catch = 1;
expt(2).catchRew = 0;
expt(2).dirtuning = '006';
expt(2).dirtuning_time = '1510';
expt(2).rettuning = {'005';'1506'};
expt(2).z = 294;
expt(2).regImgStartFrame = 41998;
expt(2).trial_range = nan;

%% AW13 150511 - FA rate high
expt(3).SubNum = '613';
expt(3).mouse = 'AW13';
expt(3).date = '150511';
expt(3).img_loc  = {'V1';'L2/3'};
expt(3).img_strct  = {'cells'};
expt(3).time_mat = ['1420'; '1437'; '1453'];
expt(3).runs = ['001'; '002'; '003'];
expt(3).nrun = size(expt(3).runs,1);
expt(3).frame_rate = 30;
expt(3).folder = 'two-photon imaging';
expt(3).catch = 0;
expt(3).catchRew = 0;
expt(3).dirtuning = '005';
expt(3).dirtuning_time = '1513';
expt(3).rettuning = {'004';'1509'};
expt(3).z = 301;
expt(3).regImgStartFrame = nan;
expt(3).trial_range = nan;


%% AW14 150623
expt(4).SubNum = '614';
expt(4).mouse = 'AW14';
expt(4).date = '150623';
expt(4).img_loc  = {'V1';'L2/3'};
expt(4).img_strct  = {'cells'};
expt(4).time_mat = ['1142'; '1158'; '1217'];
expt(4).runs = ['002'; '003'; '004'];
expt(4).nrun = size(expt(4).runs,1);
expt(4).frame_rate = 30;
expt(4).folder = 'two-photon imaging';
expt(4).catch = 0;
expt(4).catchRew = 0;
expt(4).dirtuning = '006';
expt(4).dirtuning_time = '1239';
expt(4).rettuning = {'005';'1235'};
expt(4).z = 303;
expt(4).regImgStartFrame = nan;
expt(4).trial_range = nan;

%% AW14 150626
expt(5).SubNum = '614';
expt(5).mouse = 'AW14';
expt(5).date = '150626';
expt(5).img_loc  = {'V1';'L2/3'};
expt(5).img_strct  = {'cells'};
expt(5).time_mat = ['1458'; '1514'; '1532'];
expt(5).runs = ['001'; '002'; '003'];
expt(5).nrun = size(expt(5).runs,1);
expt(5).frame_rate = 30;
expt(5).folder = 'two-photon imaging';
expt(5).catch = 0;
expt(5).catchRew = 0;
expt(5).dirtuning = '005';
expt(5).dirtuning_time = '1558';
expt(5).rettuning = {'004';'1553'};
expt(5).z = 300;
expt(5).regImgStartFrame = 28393;
expt(5).trial_range = nan;

%% AW14 150701 - might need to cut off trials for behavior
expt(6).SubNum = '614';
expt(6).mouse = 'AW14';
expt(6).date = '150701';
expt(6).img_loc  = {'V1';'L2/3'};
expt(6).img_strct  = {'cells'};
expt(6).time_mat = ['1556'; '1615'; '1633'];
expt(6).runs = ['002'; '003'; '004'];
expt(6).nrun = size(expt(6).runs,1);
expt(6).frame_rate = 30;
expt(6).folder = 'two-photon imaging';
expt(6).catch = 1;
expt(6).catchRew = 0;
expt(6).dirtuning = '006';
expt(6).dirtuning_time = '1659';
expt(6).rettuning = {'005';'1650'};
expt(6).z = 292;
expt(6).regImgStartFrame = 76434;
expt(6).trial_range = nan;

% %% AW16 151019 - miss rate high
% expt(7).SubNum = '616';
% expt(7).mouse = 'AW16';
% expt(7).date = '151019';
% expt(7).img_loc  = {'V1';'L2/3'};
% expt(7).img_strct  = {'cells'};
% expt(7).time_mat = ['1114'; '1131'; '1147'];
% expt(7).runs = ['003'; '004'; '005'];
% expt(7).nrun = size(expt(7).runs,1);
% expt(7).frame_rate = 30;
% expt(7).folder = 'two-photon imaging';
% expt(7).catch = 1;
% expt(7).dirtuning = '008';
% expt(7).rettuning = {'007';'1208'};
% expt(7).z = 0;

% %% AW16 151028 - FA rate high
% expt(8).SubNum = '616';
% expt(8).mouse = 'AW16';
% expt(8).date = '151028';
% expt(8).img_loc  = {'V1';'L2/3'};
% expt(8).img_strct  = {'cells'};
% expt(8).time_mat = ['1041'; '1115'; '1130'];
% expt(8).runs = ['002'; '004'; '005'];
% expt(8).nrun = size(expt(8).runs,1);
% expt(8).frame_rate = 30;
% expt(8).folder = 'two-photon imaging';
% expt(8).catch = 1;
% expt(8).dirtuning = '007';
% expt(8).rettuning = {'006';'1146'};
% expt(8).z = 0;
% 
% %% AW16 151030 - FA rate high
% expt(9).SubNum = '616';
% expt(9).mouse = 'AW16';
% expt(9).date = '151030';
% expt(9).img_loc  = {'V1';'L2/3'};
% expt(9).img_strct  = {'cells'};
% expt(9).time_mat = ['1050'; '1107'; '1123'];
% expt(9).runs = ['002'; '003'; '004'];
% expt(9).nrun = size(expt(9).runs,1);
% expt(9).frame_rate = 30;
% expt(9).folder = 'two-photon imaging';
% expt(9).catch = 1;
% expt(9).dirtuning = '006';
% expt(9).rettuning = {'005';'1140'};
% expt(9).z = 1;

% %% AW16 151105 - maybe suppressed, good behavior, not enough long trials
% expt(7).SubNum = '616';
% expt(7).mouse = 'AW16';
% expt(7).date = '151105';
% expt(7).img_loc  = {'V1';'L2/3'};
% expt(7).img_strct  = {'cells'};
% expt(7).time_mat = ['1255'; '1311'; '1330'];
% expt(7).runs = ['004'; '005'; '007'];
% expt(7).nrun = size(expt(7).runs,1);
% expt(7).frame_rate = 30;
% expt(7).folder = 'two-photon imaging';
% expt(7).catch = 1;
% expt(7).catchRew = 1;
% expt(7).dirtuning = '008';
% expt(7).dirtuning_time = '1351';
% expt(7).rettuning = {'008';'1347'};
% expt(7).z = 0;
% 
%% AW70 170222 - 100ms 
expt(7).SubNum = '670';
expt(7).mouse = 'AW70';
expt(7).date = '170222';
expt(7).img_loc  = {'V1';'L2/3'};
expt(7).img_strct  = {'cells'};
expt(7).time_mat = ['1116';'1132'];
expt(7).runs = ['001';'002'];
expt(7).nrun = size(expt(7).runs,1);
expt(7).frame_rate = 30;
expt(7).folder = 'two-photon imaging';
expt(7).catch = 1;
expt(7).catchRew = 1;
expt(7).dirtuning = '004';
expt(7).dirtuning_time = '1158';
expt(7).rettuning = {'005';'1208'};
expt(7).z = 0;
expt(7).z = 256;
expt(7).regImgStartFrame = nan;
expt(7).trial_range = nan;

%% AW13 150430
expt(8).SubNum = '613';
expt(8).mouse = 'AW13';
expt(8).date = '150430';
expt(8).img_loc  = {'V1';'L2/3'};
expt(8).img_strct  = {'cells'};
expt(8).time_mat = ['1505'; '1521'; '1538'];
expt(8).runs = [ '001'; '002'; '004'];
expt(8).nrun = size(expt(8).runs,1);
expt(8).frame_rate = 30;
expt(8).folder = 'two-photon imaging';
expt(8).catch = 0;
expt(8).catchRew = 1;
expt(8).dirtuning = '006';
expt(8).dirtuning_time = '1559';
expt(8).rettuning = {'005';'1555'};
expt(8).z = 299;
expt(8).regImgStartFrame = 79176;
expt(8).trial_range = nan;

% %% AW25 160316 - 100ms, not enough long trials
% expt(9).SubNum = '625';
% expt(9).mouse = 'AW25';
% expt(9).date = '160316';
% expt(9).img_loc  = {'V1';'L2/3'};
% expt(9).img_strct  = {'cells'};
% expt(9).time_mat = ['1224'; '1240'; '1256';'1312'];
% expt(9).runs = ['001'; '002'; '003';'004'];
% expt(9).nrun = size(expt(9).runs,1);
% expt(9).frame_rate = 30;
% expt(9).folder = 'two-photon imaging';
% expt(9).catch = 1;
% expt(9).catchRew = 1;
% expt(9).dirtuning = '006';
% expt(9).dirtuning_time = '1333';
% expt(9).rettuning = {'005';'1328'};
% expt(9).z = 0;
% expt(9).z = 267;
% 
% %% AW13 150428
% expt(10).SubNum = '613';
% expt(10).mouse = 'AW13';
% expt(10).date = '150428';
% expt(10).img_loc  = {'V1';'L2/3'};
% expt(10).img_strct  = {'cells'};
% expt(10).time_mat = ['0955'; '1011'; '1029'];
% expt(10).runs = ['004'; '005'; '006'];
% expt(10).nrun = size(expt(10).runs,1);
% expt(10).frame_rate = 30;
% expt(10).folder = 'two-photon imaging';
% expt(10).catch = 0;
% expt(10).dirtuning = '008';
% expt(10).dirtuning_time = '1050';
% expt(10).rettuning = {'007';'1046'};
% expt(10).z = 0;
% expt(10).z = 288;

% %% AW68 161019 - 100ms - high FA rate
% expt(10).SubNum = '668';
% expt(10).mouse = 'AW68';
% expt(10).date = '161019';
% expt(10).img_loc  = {'V1';'L2/3'};
% expt(10).img_strct  = {'cells'};
% expt(10).time_mat = ['1123'; '1139';'1155'];
% expt(10).runs = ['002'; '003';'004'];
% expt(10).nrun = size(expt(10).runs,1);
% expt(10).frame_rate = 30;
% expt(10).folder = 'two-photon imaging';
% expt(10).catch = 1;
% expt(10).dirtuning = '005';
% expt(10).dirtuning_time = '1212';
% expt(10).rettuning = {'006';'1221'};
% expt(10).z = 0;
% expt(10).z = 230;

% %% AW68 161021 - 100ms - imaging very dim, some files corrupted
% expt(10).SubNum = '668';
% expt(10).mouse = 'AW68';
% expt(10).date = '161021';
% expt(10).img_loc  = {'V1';'L2/3'};
% expt(10).img_strct  = {'cells'};
% expt(10).time_mat = ['1104';'1120'];
% expt(10).runs = ['002';'003'];
% expt(10).nrun = size(expt(10).runs,1);
% expt(10).frame_rate = 30;
% expt(10).folder = 'two-photon imaging';
% expt(10).catch = 1;
% expt(10).dirtuning = '004';
% expt(10).dirtuning_time = '1142';
% expt(10).rettuning = {'005';'1151'};
% expt(10).z = 0;
% expt(10).z = 203;

%% AW68 161024 - 100ms
expt(9).SubNum = '668';
expt(9).mouse = 'AW68';
expt(9).date = '161024';
expt(9).img_loc  = {'V1';'L2/3'};
expt(9).img_strct  = {'cells'};
expt(9).time_mat = ['1042';'1058';'1115';'1131'];
expt(9).runs = ['001';'002';'003';'005'];
expt(9).nrun = size(expt(9).runs,1);
expt(9).frame_rate = 30;
expt(9).folder = 'two-photon imaging';
expt(9).catch = 1;
expt(9).catchRew = 1;
expt(9).dirtuning = '006';
expt(9).dirtuning_time = '1147';
expt(9).rettuning = {'007';'1156'};
expt(9).z = 203;
expt(9).regImgStartFrame = nan;
expt(9).trial_range = nan;

%% AW68 161031 - 100ms
expt(10).SubNum = '668';
expt(10).mouse = 'AW68';
expt(10).date = '161031';
expt(10).img_loc  = {'V1';'L2/3'};
expt(10).img_strct  = {'cells'};
expt(10).time_mat = ['1130';'1146';'1202'];
expt(10).runs = ['001';'002';'003'];
expt(10).nrun = size(expt(10).runs,1);
expt(10).frame_rate = 30;
expt(10).folder = 'two-photon imaging';
expt(10).catch = 1;
expt(10).catchRew = 1;
expt(10).dirtuning = '006';
expt(10).dirtuning_time = '1231';
expt(10).rettuning = {'007';'1246'};
expt(10).z = 249;
expt(10).regImgStartFrame = 55561;
expt(10).trial_range = 1:208;

% %% AW60 161101 - 100ms % not  enout cells or trials
% expt(11).SubNum = '660';
% expt(11).mouse = 'AW60';
% expt(11).date = '161101';
% expt(11).img_loc  = {'V1';'L2/3'};
% expt(11).img_strct  = {'cells'};
% expt(11).time_mat = ['1230';'1246';'1302';'1318'];
% expt(11).runs = ['001';'002';'003';'004'];
% expt(11).nrun = size(expt(11).runs,1);
% expt(11).frame_rate = 30;
% expt(11).folder = 'two-photon imaging';
% expt(11).catch = 1;
% expt(11).catchRew = 1;
% expt(11).dirtuning = '005';
% expt(11).dirtuning_time = '1335';
% expt(11).rettuning = {'006';'1344'};
% expt(11).z = 0;
% expt(11).z = 220;
% expt(11).regImgStartFrame = 37844;

% %% AW68 161102 - 100ms - **same data as 161031? check it out, FA too high
% expt(13).SubNum = '668';
% expt(13).mouse = 'AW68';
% expt(13).date = '161102';
% expt(13).img_loc  = {'V1';'L2/3'};
% expt(13).img_strct  = {'cells'};
% expt(13).time_mat = ['1144';'1200';'1215'];
% expt(13).runs = ['003';'004';'005'];
% expt(13).nrun = size(expt(13).runs,1);
% expt(13).frame_rate = 30;
% expt(13).folder = 'two-photon imaging';
% expt(13).catch = 1;
% expt(13).dirtuning = '006';
% expt(13).dirtuning_time = '1233';
% expt(13).rettuning = {'007';'1242'};
% expt(13).z = 0;
% expt(13).z = 266;

% %% AW68 161121 - 100ms
% expt(13).SubNum = '668';
% expt(13).mouse = 'AW68';
% expt(13).date = '161121';
% expt(13).img_loc  = {'V1';'L2/3'};
% expt(13).img_strct  = {'cells'};
% expt(13).time_mat = ['1130';'1145';'1201';'1217'];
% expt(13).runs = ['001';'002';'003';'004'];
% expt(13).nrun = size(expt(13).runs,1);
% expt(13).frame_rate = 30;
% expt(13).folder = 'two-photon imaging';
% expt(13).catch = 1;
% expt(13).dirtuning = '007';
% expt(13).dirtuning_time = '1235';
% expt(13).rettuning = {'008';'1244'};
% expt(13).z = 0;
% expt(13).z = 228;

%% AW68 170103 - 100ms
expt(11).SubNum = '668';
expt(11).mouse = 'AW68';
expt(11).date = '170103';
expt(11).img_loc  = {'V1';'L2/3'};
expt(11).img_strct  = {'cells'};
expt(11).time_mat = ['1448';'1504';'1525'];
expt(11).runs = ['003';'004';'005'];
expt(11).nrun = size(expt(11).runs,1);
expt(11).frame_rate = 30;
expt(11).folder = 'two-photon imaging';
expt(11).catch = 1;
expt(11).catchRew = 1;
expt(11).dirtuning = '006';
expt(11).dirtuning_time = '1559';
expt(11).rettuning = {'007';'1613'};
expt(11).z = 244;
expt(11).regImgStartFrame = 25169;
expt(11).trial_range = nan;

%% AW68 161121 - 100ms %needs to be registered
expt(12).SubNum = '668';
expt(12).mouse = 'AW68';
expt(12).date = '161121';
expt(12).img_loc  = {'V1';'L2/3'};
expt(12).img_strct  = {'cells'};
expt(12).time_mat = ['1130';'1145';'1201';'1217'];
expt(12).runs = ['001';'002';'003';'004'];
expt(12).nrun = size(expt(12).runs,1);
expt(12).frame_rate = 30;
expt(12).folder = 'two-photon imaging';
expt(12).catch = 1;
expt(12).catchRew = 1;
expt(12).dirtuning = '007';
expt(12).dirtuning_time = '1235';
expt(12).rettuning = {'008';'1244'};
expt(12).z = 228;
expt(12).regImgStartFrame = 54798;
expt(12).trial_range = nan;

% %% AW72 170224 - 100ms **high lapse rate on auditory trials *mouse has no attention effect
% expt(12).SubNum = '672';
% expt(12).mouse = 'AW72';
% expt(12).date = '170224';
% expt(12).img_loc  = {'V1';'L2/3'};
% expt(12).img_strct  = {'cells'};
% expt(12).time_mat = ['1010';'1025';'1041'];
% expt(12).runs = ['002';'003';'004'];
% expt(12).nrun = size(expt(12).runs,1);
% expt(12).frame_rate = 30;
% expt(12).folder = 'two-photon imaging';
% expt(12).catch = 1;
% expt(12).catchRew = 1;
% expt(12).dirtuning = '006';
% expt(12).dirtuning_time = '1102';
% expt(12).rettuning = {'007';'1111'};
% expt(12).z = 288;
% expt(12).regImgStartFrame = nan;
% expt(12).trial_range = nan;

% %% AW72 170303 - 100ms *mouse has no attention effect
% expt(13).SubNum = '672';
% expt(13).mouse = 'AW72';
% expt(13).date = '170303';
% expt(13).img_loc  = {'V1';'L2/3'};
% expt(13).img_strct  = {'cells'};
% expt(13).time_mat = ['0916'];
% expt(13).runs = ['001'];
% expt(13).nrun = size(expt(13).runs,1);
% expt(13).frame_rate = 30;
% expt(13).folder = 'two-photon imaging';
% expt(13).catch = 1;
% expt(13).catchRew = 1;
% expt(13).dirtuning = '003';
% expt(13).dirtuning_time = '1018';
% expt(13).rettuning = {'004';'1027'};
% expt(13).z = 252;
% expt(13).regImgStartFrame = nan;
% expt(13).trial_range = [1:155];

%% AW70 170320 - 100ms %miss rate may be too high, *mouse has no attn
expt(13).SubNum = '670';
expt(13).mouse = 'AW70';
expt(13).date = '170320';
expt(13).img_loc  = {'V1';'L2/3'};
expt(13).img_strct  = {'cells'};
expt(13).time_mat = ['1111';'1129';'1146'];
expt(13).runs = ['001';'002';'003'];
expt(13).nrun = size(expt(13).runs,1);
expt(13).frame_rate = 30;
expt(13).folder = 'two-photon imaging';
expt(13).catch = 1;
expt(13).catchRew = 1;
expt(13).dirtuning = '005';
expt(13).dirtuning_time = '1224';
expt(13).rettuning = {'006';'1236'};
expt(13).z = 219;
expt(13).regImgStartFrame = 73279;
expt(13).trial_range = nan;
% 
% %% AW82 170925 - 100ms
% expt(14).SubNum = '682';
% expt(14).mouse = 'AW82';
% expt(14).date = '170925';
% expt(14).img_loc  = {'V1';'L2/3'};
% expt(14).img_strct  = {'cells'};
% expt(14).time_mat = ['1100';'1132'];
% expt(14).runs = ['001';'002'];
% expt(14).nrun = size(expt(14).runs,1);
% expt(14).frame_rate = 30;
% expt(14).folder = 'two-photon imaging';
% expt(14).catch = 1;
% expt(14).catchRew = 1;
% expt(14).dirtuning = '003';
% expt(14).dirtuning_time = '1207';
% expt(14).rettuning = {'004';'1217'};
% expt(14).z = 214;
% expt(14).regImgStartFrame = 45509;
% expt(14).trial_range = nan;
% 
% %% AW25 160302
% expt(15).SubNum = '625';
% expt(15).mouse = 'AW25';
% expt(15).date = '160302';
% expt(15).img_loc  = {'V1';'L2/3'};
% expt(15).img_strct  = {'cells'};
% expt(15).time_mat = ['1040'; '1057';'1113';'1130'];
% expt(15).runs = ['001';'002'; '003'; '004'];
% expt(15).nrun = size(expt(15).runs,1);
% expt(15).frame_rate = 30;
% expt(15).folder = 'two-photon imaging';
% expt(15).catch = 0;
% expt(15).catchRew = 0;
% expt(15).dirtuning = '006';
% expt(15).dirtuning_time = '1150';
% expt(15).rettuning = {'005';'1146'};
% expt(15).z = 281;
% expt(15).regImgStartFrame = 48048;
% expt(15).trial_range = 120:269;
% 
% %% AW25 160307
% expt(16).SubNum = '625';
% expt(16).mouse = 'AW25';
% expt(16).date = '160307';
% expt(16).img_loc  = {'V1';'L2/3'};
% expt(16).img_strct  = {'cells'};
% expt(16).time_mat = ['1041'; '1057';'1113';'1130'];
% expt(16).runs = ['001';'002'; '003'; '005'];
% expt(16).nrun = size(expt(16).runs,1);
% expt(16).frame_rate = 30;
% expt(16).folder = 'two-photon imaging';
% expt(16).catch = 0;
% expt(16).catchRew = 0;
% expt(16).dirtuning = '007';
% expt(16).dirtuning_time = '1150';
% expt(16).rettuning = {'006';'1146'};
% expt(16).z = 296;
% expt(16).regImgStartFrame = 74952;
% expt(16).trial_range = 125:257;
% 
% %% AW25 160309
% expt(17).SubNum = '625';
% expt(17).mouse = 'AW25';
% expt(17).date = '160309';
% expt(17).img_loc  = {'V1';'L2/3'};
% expt(17).img_strct  = {'cells'};
% expt(17).time_mat = ['1346'; '1402';'1421';'1437'];
% expt(17).runs = ['006';'007'; '008'; '009'];
% expt(17).nrun = size(expt(17).runs,1);
% expt(17).frame_rate = 30;
% expt(17).folder = 'two-photon imaging';
% expt(17).catch = 0;
% expt(17).catchRew = 0;
% expt(17).dirtuning = '011';
% expt(17).dirtuning_time = '1457';
% expt(17).rettuning = {'010';'153'};
% expt(17).z = 244;
% expt(17).regImgStartFrame = 34573;
% expt(17).trial_range = nan;
% 
% %% AW25 160314
% expt(18).SubNum = '625';
% expt(18).mouse = 'AW25';
% expt(18).date = '160314';
% expt(18).img_loc  = {'V1';'L2/3'};
% expt(18).img_strct  = {'cells'};
% expt(18).time_mat = ['1005'; '1021';'1037';'1053'];
% expt(18).runs = ['001';'002'; '003'; '004'];
% expt(18).nrun = size(expt(18).runs,1);
% expt(18).frame_rate = 30;
% expt(18).folder = 'two-photon imaging';
% expt(18).catch = 0;
% expt(18).catchRew = 0;
% expt(18).dirtuning = '007';
% expt(18).dirtuning_time = '1119';
% expt(18).rettuning = {'006';'1115'};
% expt(18).z = 245;
% expt(18).regImgStartFrame = 20627;
% expt(18).trial_range = nan;
% 
% %% AW25 160316
% expt(19).SubNum = '625';
% expt(19).mouse = 'AW25';
% expt(19).date = '160316';
% expt(19).img_loc  = {'V1';'L2/3'};
% expt(19).img_strct  = {'cells'};
% expt(19).time_mat = ['1224'; '1240';'1256';'1312'];
% expt(19).runs = ['001';'002'; '003'; '004'];
% expt(19).nrun = size(expt(19).runs,1);
% expt(19).frame_rate = 30;
% expt(19).folder = 'two-photon imaging';
% expt(19).catch = 0;
% expt(19).catchRew = 0;
% expt(19).dirtuning = '006';
% expt(19).dirtuning_time = '1333';
% expt(19).rettuning = {'005';'1328'};
% expt(19).z = 267;
% expt(19).regImgStartFrame = 3253;
% expt(19).trial_range = 1:250;
%% 750 180616
expt(14).SubNum = '750';
expt(14).mouse = '750';
expt(14).date = '180616';
expt(14).img_loc  = {'V1';'L2/3'};
expt(14).img_strct  = {'cells'};
expt(14).time_mat = ['1354'];
expt(14).runs = ['001'];
expt(14).nrun = size(expt(14).runs,1);
expt(14).frame_rate = 30;
expt(14).folder = 'two-photon imaging';
expt(14).catch = 1;
expt(14).catchRew = 1;
expt(14).dirtuning = '002';
expt(14).dirtuning_time = '1456';
expt(14).rettuning = {'003';'1513'};
expt(14).z = 232;
expt(14).regImgStartFrame = 59009;
expt(14).trial_range = nan;
%% 750 180618
expt(15).SubNum = '750';
expt(15).mouse = '750';
expt(15).date = '180618';
expt(15).img_loc  = {'V1';'L2/3'};
expt(15).img_strct  = {'cells'};
expt(15).time_mat = ['1303'];
expt(15).runs = ['001'];
expt(15).nrun = size(expt(15).runs,1);
expt(15).frame_rate = 30;
expt(15).folder = 'two-photon imaging';
expt(15).catch = 1;
expt(15).catchRew = 1;
expt(15).dirtuning = '002';
expt(15).dirtuning_time = '1408';
expt(15).rettuning = {'003';'1426'};
expt(15).z = 258;
expt(15).regImgStartFrame = 98808;
expt(15).trial_range = nan;

%% 750 180619
expt(16).SubNum = '750';
expt(16).mouse = '750';
expt(16).date = '180619';
expt(16).img_loc  = {'V1';'L2/3'};
expt(16).img_strct  = {'cells'};
expt(16).time_mat = ['0937'];
expt(16).runs = ['003'];
expt(16).nrun = size(expt(16).runs,1);
expt(16).frame_rate = 30;
expt(16).folder = 'two-photon imaging';
expt(16).catch = 1;
expt(16).catchRew = 1;
expt(16).dirtuning = '004';
expt(16).dirtuning_time = '1041';
expt(16).rettuning = {'005';'1058'};
expt(16).z = 294;
expt(16).regImgStartFrame = 89320;
expt(16).trial_range = 1:250;