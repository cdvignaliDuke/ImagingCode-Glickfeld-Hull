 ...
 *
 *  2008/01/21: Modified to do Gaussian fitting by Stefan Wrz (s.woerz at dkfz.de).
 *
 */
public class My_Plugin{    
    ...
    public My_Plugin (double[] xData, double[] yData) {
        this.xData = xData;
        this.yData = yData;
    ...
    void initialize() {
        ...
        switch (fit) {
            ...
            case GAMMA_VARIATE:
                ...
                //  therefore an estimate for a and B is sqrt(tm-t0)                //  K [a] can now be calculated from these estimates
                simp[0][0] = firstx;
    ...
    public static double f(int fit, double[] p, double x) {
        ...
        switch (fit) {
            ...
            case EXP_WITH_OFFSET2:
                return p[0]*(1-Math.exp(p[1]*x*-1))+p[2];
            case GAUSSIAN:
