...
mike_dir = 'H:\home\mike\Analysis\CC 2P';
CRP_expt_list_mike;
...
nexp = size(expt(id).date,1);
    ...
    for iexp = exp_subset
        ...
        if exist(fullfile(mike_dir,img_fn, [img_fn '_input.mat']))
            load(fullfile(mike_dir,img_fn, [img_fn '_input.mat']))
        end

        if exist(fullfile(mike_dir,img_fn, [img_fn '_targetAlign.mat']))
            load(fullfile(mike_dir,img_fn, [img_fn '_targetAlign.mat']))
        end
        ...
        ylabel('Lick rate (Hz)')
        title('Reward')
        if length(ind_block2>5)
        subplot(3,1,2)
        shadedErrorBar(tt, nanmean(lickCueAlign(:,ind_block2),2).*(1000./frameRateHz), (nanstd(lickCueAlign(:,ind_block2),[],2)./sqrt(length(ind_block2))).*(1000./frameRateHz),'r');
        hold on
        scatter((lickBurstStart(:,ind_block2)-prewin_frames).*(1000./frameRateHz), 10.*ones(size(lickBurstStart(:,ind_block2))), 'x');
        xlabel('Time from cue')
        ylabel('Lick rate (Hz)')
        title('Omit')
        end
        ...
        suptitle([date ' ' mouse])
        savefig(fullfile(mike_dir,img_fn, [img_fn '_cueAlign_lickHz.fig']))
        ...
        if sum(~isnan(lickBurstStart(:,ind_block2)))>6
            [sortlick sortlick_ind] = sort(lickBurstStart(:,ind_block2),'ascend');
            nburst = sum(~isnan(lickBurstStart(:,ind_block2)));
            nnan = sum(isnan(lickBurstStart(:,ind_block2)));
            ind_early_omit = sortlick_ind(1:floor(nburst/4));
            ind_late_omit = sortlick_ind(nburst-floor(nburst/4)+1:end-nnan);
            early_omit_time = mean((lickBurstStart(:,ind_block2(ind_early_omit))-prewin_frames).*(1000./frameRateHz),2);
            late_omit_time = mean((lickBurstStart(:,ind_block2(ind_late_omit))-prewin_frames).*(1000./frameRateHz),2);
        else
            ind_early_omit = [];
            ind_late_omit = [];
            early_omit_time = NaN;
        ...
        savefig(fullfile(mike_dir,img_fn, [img_fn '_cueAlignSpiking_byLickTime.fig']))
        %savefig(fullfile(mike_out,img_fn, [img_fn '_cueAlignSpiking_byLickTime.fig']))
        end
        ...
        hold on
        if sum(~isnan(postRew_lickBurstStart(ind_block2)))>4
            shadedErrorBar(tl, nanmean(nanmean(postRew_lickAlignEvents(:,:,ind_block2),3),2).*(1000./frameRateHz), (nanstd(nanmean(postRew_lickAlignEvents(:,:,ind_block2),3),[],2).*(1000./frameRateHz))./sqrt(nIC),'r');
        end
        xlabel('Time from lick')
        ylabel('Spike rate (Hz)')
        ylim([0 inf])
        title(['Reward (black- ' num2str(sum(~isnan(squeeze(postRew_lickAlignEvents(1,1,ind_rew))))) '); omit (red- ' num2str(sum(~isnan(squeeze(postRew_lickAlignEvents(1,1,ind_block2))))) ')'])
        subplot(2,3,2)
        ...
        hold on
        if sum(~isnan(postRew_lickBurstStart(ind_block2)))>4
            shadedErrorBar(tl, nanmean(postRew_lickAlign(:,ind_block2),2).*(1000./frameRateHz), (nanstd(postRew_lickAlign(:,ind_block2),[],2).*(1000./frameRateHz))./sqrt(length(ind_block2)),'r');
        end
        xlabel('Time from lick')
        ylabel('Lick rate (Hz)')
        ylim([0 inf])
        ...
        savefig(fullfile(mike_dir,img_fn, [img_fn '_postRew_lickAlignSpiking.fig']))
        ...
        savefig(fullfile(mike_dir,img_fn, [img_fn '_cumulativeLicking.fig']))   
        ...
        hold on
        if length(ind_block2)>5
            shadedErrorBar(tl_rew, nanmean(nanmean(lastPreRew_lickAlignEvents(:,:,ind_block2),3),2).*(1000./frameRateHz), (nanstd(nanmean(lastPreRew_lickAlignEvents(:,:,ind_block2),3),[],2).*(1000./frameRateHz))./sqrt(nIC),'r');
        end
        if length(ind_unexp)>5
        ...
        hold on
        if length(ind_block2)>5
            shadedErrorBar(tl_rew, nanmean(lastPreRew_lickAlign(:,ind_block2),2).*(1000./frameRateHz), (nanstd(lastPreRew_lickAlign(:,ind_block2),[],2).*(1000./frameRateHz))./sqrt(length(ind_block2)),'r');
            omit_str = [' Omit- ' num2str(length(intersect(ind_block2,preRewTrials))) ' trials; '];
        else
            omit_str = [];
        end
        if length(ind_unexp)>5
        ...
        hold on
        if length(ind_block2)>5
            shadedErrorBar(tl_rew, nanmean(nanmean(firstPostRew_lickAlignEvents(:,:,ind_block2),3),2).*(1000./frameRateHz), (nanstd(nanmean(firstPostRew_lickAlignEvents(:,:,ind_block2),3),[],2).*(1000./frameRateHz))./sqrt(nIC),'r');
        end
        if length(ind_unexp)>5
        ...
        hold on
        if length(ind_block2)>5
            shadedErrorBar(tl_rew, nanmean(firstPostRew_lickAlign(:,ind_block2),2).*(1000./frameRateHz), (nanstd(firstPostRew_lickAlign(:,ind_block2),[],2).*(1000./frameRateHz))./sqrt(length(ind_block2)),'r');
            omit_str = [' Omit- ' num2str(length(intersect(ind_block2,postRewTrials))) ' trials; '];
        else
            omit_str = [];
        end
        if length(ind_unexp)>5
        ...
        savefig(fullfile(mike_dir,img_fn, [img_fn '_lastVsFirstLick.fig'])) 
        ...
        if length(ind_block2)>5
            [sortlickHz sortlickHz_ind] = sort(lickBurstHz_all(:,ind_block2),'ascend');
            nnan = sum(isnan(lickBurstHz_all(:,ind_block2)));
            ind_low_omit = sortlickHz_ind(1:floor(length(ind_block2)/4));
            ind_high_omit = sortlickHz_ind(length(ind_block2)-floor(length(ind_block2)/4)+1:end-nnan);
            HL_lickrate.low_omit = mean(lickBurstHz_all(:,ind_block2(ind_low_omit)),2);
            HL_lickrate.high_omit = mean(lickBurstHz_all(:,ind_block2(ind_high_omit)),2);
            
            [sortlickHz sortlickHz_ind] = sort(preRew_lickBurstHz(:,ind_block2),'ascend');
            nnan = sum(isnan(preRew_lickBurstHz(:,ind_block2)));
            ind_low_preomit = sortlickHz_ind(1:floor(length(ind_block2)/4));
            ind_high_preomit = sortlickHz_ind(length(ind_block2)-floor(length(ind_block2)/4)+1:end-nnan);
            HL_lickrate.low_preomit = mean(preRew_lickBurstHz(:,ind_block2(ind_low_preomit)),2);
            HL_lickrate.high_preomit = mean(preRew_lickBurstHz(:,ind_block2(ind_high_preomit)),2);

            [sortlickHz sortlickHz_ind] = sort(postRew_lickBurstHz(:,ind_block2),'ascend');
            nnan = sum(isnan(postRew_lickBurstHz(:,ind_block2)));
            ind_low_postomit = sortlickHz_ind(1:floor(length(ind_block2)/4));
            ind_high_postomit = sortlickHz_ind(length(ind_block2)-floor(length(ind_block2)/4)+1:end-nnan);
            HL_lickrate.low_postomit = mean(postRew_lickBurstHz(:,ind_block2(ind_low_postomit)),2);
            HL_lickrate.high_postomit = mean(postRew_lickBurstHz(:,ind_block2(ind_high_postomit)),2);
        else
            ind_low_omit = [];
            ind_high_omit = [];
            ind_low_preomit = [];
        ...
        if length(ind_block2)>5
            subplot(2,3,4)
            shadedErrorBar(tt,nanmean(nanmean(targetAlign_events(:,:,ind_block2(ind_low_omit)),3),2).*(1000./frameRateHz), (nanstd(nanmean(targetAlign_events(:,:,ind_block2(ind_low_omit)),3),[],2).*(1000./frameRateHz))./sqrt(nIC), 'b');
            hold on
            shadedErrorBar(tt,nanmean(nanmean(targetAlign_events(:,:,ind_block2(ind_high_omit)),3),2).*(1000./frameRateHz), (nanstd(nanmean(targetAlign_events(:,:,ind_block2(ind_high_omit)),3),[],2).*(1000./frameRateHz))./sqrt(nIC),'k');
            xlabel('Time from cue')
            ylabel('Spike rate (Hz)')
            ylim([-1 inf])
            title(['1s- Omit: ' num2str(chop(HL_lickrate.low_omit,2)) ' vs ' num2str(chop(HL_lickrate.high_omit,2)) ' Hz'])
            subplot(2,3,5)
            shadedErrorBar(tt,nanmean(nanmean(targetAlign_events(:,:,ind_block2(ind_low_preomit)),3),2).*(1000./frameRateHz), (nanstd(nanmean(targetAlign_events(:,:,ind_block2(ind_low_preomit)),3),[],2).*(1000./frameRateHz))./sqrt(nIC), 'b');
            hold on
            shadedErrorBar(tt,nanmean(nanmean(targetAlign_events(:,:,ind_block2(ind_high_preomit)),3),2).*(1000./frameRateHz), (nanstd(nanmean(targetAlign_events(:,:,ind_block2(ind_high_preomit)),3),[],2).*(1000./frameRateHz))./sqrt(nIC),'k');
            xlabel('Time from cue')
            ylabel('Spike rate (Hz)')
            ylim([-1 inf])
            title(['Pre- Omit: ' num2str(chop(HL_lickrate.low_preomit,2)) ' vs ' num2str(chop(HL_lickrate.high_preomit,2)) ' Hz'])
            subplot(2,3,6)
            shadedErrorBar(tt,nanmean(nanmean(targetAlign_events(:,:,ind_block2(ind_low_postomit)),3),2).*(1000./frameRateHz), (nanstd(nanmean(targetAlign_events(:,:,ind_block2(ind_low_postomit)),3),[],2).*(1000./frameRateHz))./sqrt(nIC), 'b');
            hold on
            shadedErrorBar(tt,nanmean(nanmean(targetAlign_events(:,:,ind_block2(ind_high_postomit)),3),2).*(1000./frameRateHz), (nanstd(nanmean(targetAlign_events(:,:,ind_block2(ind_high_postomit)),3),[],2).*(1000./frameRateHz))./sqrt(nIC),'k');
            xlabel('Time from cue')
            ylabel('Spike rate (Hz)')
            ylim([-1 inf])
