%function RegInfo = do_work_scatterplot_SFori_NEW_SEPT2011(RegInfo);

%NOTE: this does the same thing, but only using upwards dirn
...
if isfield(RegInfo,'Stats2') == 0
    RegInfo.Stats2 = [];
end
if isfield(RegInfo.Stats2,'Fit_struct') == 0
    RegInfo.Stats2.Fit_struct = [];
end

if isfield(RegInfo.Stats2.Fit_struct,'Shuf') == 0
    RegInfo.Stats2.Fit_struct.Shuf = 1;
    RegInfo.Stats2.Fit_struct.True = 1;
end
...
%remo ve chunks with smaller xsections
...
RegInfo.Stats2.Stats_cellarea = Stats_cellarea;
RegInfo.Stats2.thresh_crosssection = thresh_crosssection;
RegInfo.Stats2.XC_cells = XC_cells;
RegInfo.Stats2.lowcut1 = lowcut1;
RegInfo.Stats2.labelimg = RegInfo.Cell.labelimg;
RegInfo.Stats2.avg_norm = RegInfo.Cell.avg_norm;
...
%note: now subtract the blank response from each one: this should help
%avoid meaningful -ve numbers.. 
...
Wheel_mat_USE3 = sum(Wheel_mat_USE2(:,[ind_post]),2)>0; %for now, any running during response window counts..
...
tmp = sort(oris(1:(Nstimtypes-1)));
ORIvec000 = tmp([1 find(diff(tmp))+1]);


ORIvecuse = [0 45 90 135 180 225 270 315]*pi/180;
TFuse = [.5 1 2 4 8 15 24];
%SFuse = [.02 .04 .08 .16 .32 .64];
SFuse = [0 .02 .04 .08 .16 .32 .64];

ind_ORIuse = [];
for count = 1:length(ORIvecuse)
    ind2 = find(ORIvecuse(count) == ORIvec000);
    if length(ind2)>0
        ind_ORIuse = [ind_ORIuse; [ind2(1) ORIvecuse(count)]];
    end
end
NORI = length(ind_ORIuse);
...
ind_SFuse2 = [];
Mask_SF = zeros(size(SFvec));
for count = 1:size(ind_SFuse,1)
    ind = find(SFvec == ind_SFuse(count,2) & TFvec>0);
    Mask_SF(ind) = 1;
end
ind_SFfinal = find(Mask_SF);


%%%end stim randomization work

PLOTRUN_NORUN_RUN_ALLRUN_vec = [3]
RUN_EO_vec = [3];
...
for RUN_EO = RUN_EO_vec;
    ...
    Mask_stimtypeAK = max(Mask_stimtype,[],2);
    %tc_resp_pre
    %tc_resp_post
    save('Preanova_data_forAK.mat','tc_resp_pre','tc_resp_post','Mask_stimtypeAK');
    
        %first, root out crappy cells as ones without at least one
    %significant, at significance level alpha2

%%% Now modify the TFSF stuff for TF by ori

   
    
    %now generate the mean for fitting:   
    ORI_vec0 = ind_ORIuse(:,2);
    SF_vec0 = ind_SFuse(:,2);
    [sfsf,oriori]=meshgrid(SF_vec0,ORI_vec0);
    grid2.sfsf = sfsf;
    grid2.oriori = oriori;
    ...
    Nshuf = 2; %100; %100; %=0 means only run for true data
    ...
    for count_shuf = 0:Nshuf
        for count_run = 1:length(PLOTRUN_NORUN_RUN_ALLRUN_vec)
            ...
            if count_shuf == 0
                %first, root out crappy cells as ones without at least one
                %significant, at significance level alpha2
               % eval(['Info_ttest_mat_',str_run,' = zeros(Ncells,NstimtypesMA,3);']);
%%            
...
%                            [h_ttestB1,p_ttestB1] = ttest2(Rpre(:,cellnum),Rpost(:,cellnum),alphaB,'left','equal');
                ...
                H_ttest_OLDWAY = min(p_ttest(:,1:(NstimtypesMA-1)),[],2) < alpha2;
                %%%%%%%%%%%
%%
%CHANGE THIS SEPT 2011 to only evaluate upwards direction
                %data
                H_ttest = zeros(size(p_ttest,1),1);
                for count_cell = 1:size(p_ttest,1);
                    a = p_ttest(count_cell,1:(end-1));
                    if length(a) == NORI*NSF
                        p_ttestSEPT = reshape(a(1:NSF*NORI),NORI,NSF)';
                    else
                        %include fullfield, and put it in all ori columns..
                        b0 =  reshape(p_ttest(count_cell,2:(2+((NSF-1)*NORI)-1)),NORI,(NSF-1))';
                        p_ttestSEPT = [ones(1,8); b0];
                    end
                    p_ttestSEPTB = p_ttestSEPT(:,3);
                    H_ttest0 = min(p_ttestSEPTB) < alpha2;
%min(p_ttestSEPTB) 
                    H_ttest(count_cell) = H_ttest0;
                    
                    %for fun: 
                    a = Im_mat_USE(count_cell,ind_SFfinal);
                    if length(a) == NORI*NSF
                        b = reshape(a(1:NSF*NORI),NORI,NSF)';
                    else
                        %include fullfield, and put it in all ori columns..
                        b0 =  reshape(a(2:(2+((NSF-1)*NORI)-1)),NORI,(NSF-1))';
                        b = [a(1)*ones(1,8); b0];
                    end
                    b2 = b( :,ind_ORIuse(:,1));
                    data = b2';   
...
%%                
                ...
                Mask_cells_USE = H_ttest .* Mask_nosmallbits .* Mask_crossection; % .* (Npixpercell > 10);
%                Mask_cells_USE = H_ttest; % .* (Npixpercell > 10);
                %force last value = 1 (neuropil)
                Mask_cells_USE(end) = 1;

                Mask_cells_USE = H_ttest; %NOTE: THIS LINE WAS HERE BEFORE SEPT 2011; COULD MEAN I INCLUDED DENDRITES IN ORIG SUBMISSION? CHECK.. 
                
                if ~isempty(find(isnan(Im_mat_USE(:,1:NstimtypesMA))))
                    Mask_cells_USE(:) = 0;
                end

%                eval(['RegInfo.Stats2.tstat_',str_run,' = tstat;'])
                eval(['RegInfo.Stats2.p_ttest_',str_run,' = p_ttest;'])
                eval(['RegInfo.Stats2.H_ttest_',str_run,' = H_ttest;'])
                eval(['RegInfo.Stats2.Mask_cells_USE_',str_run,' = Mask_cells_USE;'])
            ...
            for count_cell = 1:Ncells;
                if Mask_cells_USE(count_cell) == 1
                    ...
                    b2 = b( :,ind_ORIuse(:,1));
                    data = b2';                    
%CHANGE ON SEPT2011 %make all 8 oris have the same val as upwards
                    data(:) = ones(8,1)*data(3,:);
...
RegInfo.Stats2.Fit_struct = Fit_struct;
RegInfo.Stats2.Ind_struct = Ind_struct;
...
for count_run = 1:length(PLOTRUN_NORUN_RUN_ALLRUN_vec)
    ...
    %    eval(['tmp = RegInfo.Stats2.Fit_struct(1).Shuf(1).s_',str_run,'.x;']);
    sz_fit = size(tmp);
    fit_shuf_vec = zeros(Ncells,sz_fit(2),Nshuf);
    fit_true_vec = zeros(Ncells,sz_fit(2));
    for count = 1:NstimtypesMA
        ind_run_1 = eval(['Ind_struct(count).',str_run,'_trials;']);
        %if count_shuf > 0 %resample with replacement, don't resample by trial for now because running-rejection may be uneven for various trials..
        %    ind_run_1 = ind_run_1(randsample(length(ind_run_1),length(ind_run_1),1));
        ...
        if length(ind_run_1)>0
            ...
            eval(['Im_mat_std_',str_run,'(:,count) = std(tc_resp(ind_run_1,:),[],1);']);
               ...
               %add a new one for avg timecourse and std of avg timecourse: 
           %NEWADD MA 110531           %add a new one for avg timecourse and std of avg timecourse: 
    ...
    eval(['RegInfo.Stats2.Im_mat_',str_run,' = Im_mat_',str_run,';'])
    eval(['RegInfo.Stats2.Im_mat_std_',str_run,' = Im_mat_std_',str_run,';'])
    eval(['RegInfo.Stats2.Im_N_std_',str_run,' = Im_N_',str_run,';'])
    RegInfo.Stats2.ind_SFfinal = ind_SFfinal;

           %add a new one for avg timecourse and std of avg timecourse: 
           %NEWADD MA 110531
    eval(['RegInfo.Stats2.Im_mat_tc_',str_run,' = Im_mat_tc_',str_run,';'])
    eval(['RegInfo.Stats2.Im_mat_tc_std_',str_run,' = Im_mat_tc_std_',str_run,';'])  
   
    %RegInfo.Stats2.Im_mat_std_USE_true = Im_mat_std_USE_true;
    %RegInfo.Stats2.Im_N_USE_true = Im_N_USE_true;

    %now extract fits
    for count_cell = 1:Ncells
        if Mask_cells_USE(count_cell) == 1
            if ~isempty(RegInfo.Stats2.Fit_struct(count_cell).True)                
                eval(['tmp = RegInfo.Stats2.Fit_struct(count_cell).True.s_',str_run,'.x;']);
    ...
    for count_shuf = 1:Nshuf
        for count_cell = 1:Ncells
            if Mask_cells_USE(count_cell) == 1
                if ~isempty(RegInfo.Stats2.Fit_struct(count_cell).Shuf)
                    eval(['tmp = RegInfo.Stats2.Fit_struct(count_cell).Shuf(count_shuf).s_',str_run,'.x;']);
    ...
    for count_cell = 1:Ncells
        if Mask_cells_USE(count_cell) == 1
            %take means from truedata fit:
            lbub_fits(count_cell,:,4) = fit_true_vec(count_cell,:);

            %fit params: SFband SFmax oritune oriangle dirtune dirangle
            for count2 = [1 2 3 5]; %1:Npars
            ...
            %now treat ori and direction angle separately
            %DO THIS: start with true mean, then add pi (or 2pi for dir
            %analysis) if shuf value is too more than pi/2 (pi) away
            %start with direction: 
              count2 = 6;
              true_angle = fit_true_vec(count_cell,count2);
            shuf_angle = squeeze(fit_shuf_vec(count_cell,count2,:));
            shuf_angle2 = shuf_angle; 
            for count3 = 1:length(shuf_angle)
                ang = shuf_angle(count3);
                if (true_angle - ang) < -180
                    shuf_angle2(count3) = shuf_angle2(count3) - 360;
                elseif (true_angle - ang) > 180
                    shuf_angle2(count3) = shuf_angle2(count3) + 360;
                end
            end
             %diffangle = true_angle - shuf_angle2;
            tmp = shuf_angle2; %diffangle; %squeeze(fit_shuf_vec(count_cell,count2,:));
            [i,j] = sort(tmp);
            ind_shuf_lb = ceil(Nshuf*alpha_bound);
            ind_shuf_ub = ceil(Nshuf*(1-alpha_bound));
            lbub_fits(count_cell,count2,1) = i(ind_shuf_lb);
            ...
            for count3 = 1:length(shuf_angle)
                ang = shuf_angle(count3);
                if (true_angle - ang) < -90
                    shuf_angle2(count3) = shuf_angle2(count3) - 180;
                elseif (true_angle - ang) > 90
                    shuf_angle2(count3) = shuf_angle2(count3) + 180;
                end
            end
    ...
    eval(['RegInfo.Stats2.lbub_fits_',str_run,' = lbub_fits;'])
    eval(['RegInfo.Stats2.alpha_bound = alpha_bound;']);
    ...
    eval(['RegInfo.Stats2.fit_shuf_vec_',str_run,' = fit_shuf_vec;'])
    eval(['RegInfo.Stats2.fit_true_vec_',str_run,' = fit_true_vec;'])
    

%    eval(['RegInfo.Stats2.Fit_struct_',str_run,' = Fit_struct;'])
%    eval(['RegInfo.Stats2.ORI_vec0_',str_run,' = ORI_vec0; '])
%    eval(['RegInfo.Stats2.SF_vec0_',str_run,' = SF_vec0;'])
%    eval(['RegInfo.Stats2.alpha2_',str_run,' = alpha2;'])
%    eval(['RegInfo.Stats2.tstat_',str_run,' = tstat;'])
    eval(['RegInfo.Stats2.p_ttest_',str_run,' = p_ttest;'])
    eval(['RegInfo.Stats2.H_ttest_',str_run,' = H_ttest;'])
    eval(['RegInfo.Stats2.Mask_cells_USE_',str_run,' = Mask_cells_USE;'])
...
RegInfo.Stats2.Fit_struct = Fit_struct;
RegInfo.Stats2.ORI_vec0 = ORI_vec0; 
RegInfo.Stats2.SF_vec0 = SF_vec0; 
RegInfo.Stats2.alpha2 = alpha2;
%RegInfo.Stats2.tstat = tstat; 
RegInfo.Stats2.p_ttest = p_ttest;
RegInfo.Stats2.H_ttest = H_ttest;
RegInfo.Stats2.H_ttest_OLDWAY = H_ttest_OLDWAY;
RegInfo.Stats2.Mask_cells_USE = Mask_cells_USE;


%RegInfo.Stats2.Im_mat_USE_true = Im_mat_USE_true;
%RegInfo.Stats2.Im_mat_std_USE_true = Im_mat_std_USE_true;
%RegInfo.Stats2.Im_N_USE_true = Im_N_USE_true;
...
%RegInfo.Stats2.Fit_struct(1).Shuf(100)
