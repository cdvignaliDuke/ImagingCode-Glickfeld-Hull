...
ttest_mask_interp = interp2(ttest_mask,2);
ttest_mask_long = reshape(ttest_mask_interp, [siz2(1)*siz2(2) 1]);
ind = find(ttest_mask_long>.1);
ttest_mask_long(ind) = 1;
ttest_mask_interp = reshape(ttest_mask_long, [siz2(1) siz2(2)]);

stack_max_ttest = ttest_mask.*stack_max;

figure; imagesq(max_interp);
...
overlay = local_max+(2*local_max_interp);
