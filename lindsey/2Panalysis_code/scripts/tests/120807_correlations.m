...
for iArea = 3
    ...
    for iexp = 1:nexp;
        ...
        if n2>1;
            ...
            r_shuf = zeros(n2);
            for ipix1 = 1:n2
                resp_dFoverF_shuf = zeros(1, size(resp_dFoverF,2));
                start = 1;
                for iCond = 1:nCond+1
                    nRep = stim_reps(:,iCond);
                    rand_seq = randperm(nRep);
                    resp_dFoverF_temp = resp_dFoverF(goodfit_ind(:,ipix1), start:start+ nRep-1);
                    resp_dFoverF_shuf(:,start:start+ nRep-1) = resp_dFoverF_temp(:,rand_seq);
                    start = start+nRep;
                end
                for ipix2 = 1:n2
                    r_shuf(ipix1,ipix2)= triu2vec(corrcoef(resp_dFoverF_shuf,resp_dFoverF(goodfit_ind(:,ipix2),:)));
                end
            end
            ...
            r_real_nan = r_real;
            for ipix = 1:n2;
                r_shuf_nan(ipix,ipix) = NaN;
                r_real_nan(ipix,ipix) = NaN;
            end

            r_real_avg = nanmean(r_real_nan,2);
            r_shuf_avg = nanmean(r_shuf_nan,2);

            r_sub = bsxfun(@minus, r_real_nan, r_shuf_nan);
            r_sub_avg = nanmean(r_sub,2);
        else
            r_real_avg = NaN;
            r_shuf_avg = NaN;
            r_sub_avg = NaN;
        ...
        save(fn_out, 'r_real_avg', 'r_shuf_avg', 'r_sub_avg');
    end
end
