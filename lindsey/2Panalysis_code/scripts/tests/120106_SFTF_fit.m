...
save(fn_out, 'lbub_fits', 'lbub_diff')
...
for iCell = 1:n_pix    
    if start>64
        fn_out = fullfile('\\Zmey\storlab\users\Lindsey\Projects\HVAs\2P Axon Imaging\Analysis\120109', [date '_' mouse '_run' num2str(userun) '_Data_vs_Fit_' num2str(fig) '.pdf']);
...
fn_out = fullfile('\\Zmey\storlab\users\Lindsey\Projects\HVAs\2P Axon Imaging\Analysis\120109', [date '_' mouse '_run' num2str(userun) '_Speed_vs_dF_errorbar.pdf']);
        print(gcf, '-dpdf', fn_out);
