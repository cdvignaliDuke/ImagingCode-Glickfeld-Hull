            ...
            if length(norun)>49
                if length(run_slow)>49
                    ...
                    for iCond = 1:25
                        ...
                        ind_runslow = find(Wheel_mat(start:start-1+nRep,:)>.1);
                        ...
                        for iCell = 1:n_pix;
                            ...
                            if max(a,[],2) > 0     
                                ...
                                if count_shuf == 0
                                    ...
                                    SAVEALLDATA = 1;
                                    Fit_2Dellipse_LG
                                    eval(['Fit_struct_runS(iCell).True_runslow.s_',' = s;']);
                                else
                                    if ~isnan(Fit_struct_runS(iCell).True_runslow.s_.x(1))
                                        SAVEALLDATA = 0;
                                        PLOTIT_FIT = 0;
                                        Fit_2Dellipse_LG
                                        eval(['Fit_struct_runS(iCell).Shuf_runslow(count_shuf).s_',' = s;']);
                                    else
                                        eval(['Fit_struct_runS(iCell).Shuf_runslow(count_shuf).s_.x',' = nan(1,6);']);
                                        eval(['Fit_struct_runS(iCell).Shuf_runslow(count_shuf).s_.SFhicut_50', '= nan;']);
                                        eval(['Fit_struct_runS(iCell).Shuf_runslow(count_shuf).s_.TFhicut_50', '= nan;']);
                                        eval(['Fit_struct_runS(iCell).Shuf_runslow(count_shuf).s_.SFhicut_10', '= nan;']);
                                        eval(['Fit_struct_runS(iCell).Shuf_runslow(count_shuf).s_.TFhicut_10', '= nan;']);
                                    end
                                end
                            else
                                if count_shuf == 0
                                    eval(['Fit_struct_runS(iCell).True_runslow.s_.x',' = nan(1,6);']);
                                    eval(['Fit_struct_runS(iCell).True_runslow.s_.SFhicut_50', '= nan(1,1);']);
                                    eval(['Fit_struct_runS(iCell).True_runslow.s_.TFhicut_50', '= nan;']);
                                    eval(['Fit_struct_runS(iCell).True_runslow.s_.SFhicut_10', '= nan;']);
                                    eval(['Fit_struct_runS(iCell).True_runslow.s_.TFhicut_10', '= nan;']);
                                elseif count_shuf> 0
                                    eval(['Fit_struct_runS(iCell).Shuf_runslow(count_shuf).s_.x',' = nan(1,6);']);
                                    eval(['Fit_struct_runS(iCell).Shuf_runslow(count_shuf).s_.SFhicut_50', '= nan;']);
                                    eval(['Fit_struct_runS(iCell).Shuf_runslow(count_shuf).s_.TFhicut_50', '= nan;']);
                    ...
                    if Nshuf>1;
                        ...
                        for iCell = 1:n_pix
                            if ~isempty(Fit_struct_runS(iCell).True_runslow)                
                                eval(['tmp = Fit_struct_runS(iCell).True_runslow.s_.x;']);
                                eval(['tmp = [tmp Fit_struct_runS(iCell).True_runslow.s_.SFhicut_50];']);
                                eval(['tmp = [tmp Fit_struct_runS(iCell).True_runslow.s_.TFhicut_50];']);
                                eval(['tmp = [tmp Fit_struct_runS(iCell).True_runslow.s_.SFhicut_10];']);
                                eval(['tmp = [tmp Fit_struct_runS(iCell).True_runslow.s_.TFhicut_10];']);
                        ...
                        for count_shuf = 1:Nshuf
                            for iCell = 1:n_pix
                                if length(Fit_struct_runS(iCell).Shuf_runslow)>= count_shuf
                                    if ~isempty(Fit_struct_runS(iCell).Shuf_runslow(count_shuf).s_)
                                        eval(['tmp = Fit_struct_runS(iCell).Shuf_runslow(count_shuf).s_.x;']);
                                        eval(['tmp = [tmp Fit_struct_runS(iCell).Shuf_runslow(count_shuf).s_.SFhicut_50];']);
                                        eval(['tmp = [tmp Fit_struct_runS(iCell).Shuf_runslow(count_shuf).s_.TFhicut_50];']);
                                        eval(['tmp = [tmp Fit_struct_runS(iCell).Shuf_runslow(count_shuf).s_.SFhicut_10];']);
                                        eval(['tmp = [tmp Fit_struct_runS(iCell).Shuf_runslow(count_shuf).s_.TFhicut_10];']);
                            ...
                            Npars = size(fit_true_vec_runslow,2);
                            ...
                            %now take means from truedata fit:
                                lbub_fits_new(iCell,:,4, 3) = fit_true_vec_runslow(iCell,:);
                            end
