...
for iArea = 1
    ...
    for iexp = 2:nexp
        ...
        ind_highP = find(ttest_long(ind_local_max,:)>=(alphaB));
        ...
        local_max_big = zeros(size(local_max));
        for ipix = 1:n_pix
            sub_y = [i(ipix)-2:i(ipix)+2]; 
            sub_x = [j(ipix)-2:j(ipix)+2];
        ...
        fn_out = fullfile(outDir, 'analysis', [date '_' mouse '_run' num2str(userun) '_resp_ttest_pt01_new.mat']);
        ...
        fn_out = fullfile(outDir, 'analysis', [date '_' mouse '_run' num2str(userun) '_Fit_struct_ttest_pt01_new.mat']);   
    ...
    fn_out = fullfile(outDir, 'analysis', [date '_' mouse '_run' num2str(userun) '_lbub_fits_ttest_pt01_new.mat']);   
