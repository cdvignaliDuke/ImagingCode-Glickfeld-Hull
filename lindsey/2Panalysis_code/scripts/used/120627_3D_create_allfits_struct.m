...
all_cells = [0 0];
    ...
    for iexp = 1:nexp;
        ...
        all_fits(iArea).expt(iexp).dirs = mouse;
        all_fits(iArea).expt(iexp).zoom = zoom;
        all_fits(iArea).expt(iexp).depth = depth;
        ...
        all_cells = all_cells+all_fits(iArea).expt(iexp).n;
...
%             all_fits(iArea).expt(iexp).bouton(iCell).pos = [i(iCell, :) j(iCell, :)];
    ...
    all_fits(iArea).n = all_cells;

    fn_out = fullfile(sum_base, [matrix '_' num2str(P) 'P_' inj '_soma_summary'], 'all_fits.mat');
