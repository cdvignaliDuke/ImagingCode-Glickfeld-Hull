function bwOut = imCellEditInteractive2(fovImg, bwCell, zoomToAxis, cellRadius)
...
%       
%       V2
%       addition by SY - using rigth click select cell countour points (3 or
%       more) then click "c" to crerate cell based on selected contour points. 
%       "u" - undo last contour point, "y" - delet all current contour points
%
...
% draw initial figure
figH = figure;
imH = subDrawShaded(fovImg, bwCell, zoomToAxis,contPoints);
axH = get(imH, 'Parent');
...
tStr = { [mfilename ': Click on a cell region to add.  Shift-click to del, RET to finish, Z to undo'], ...
         '       Rigth click to add contour point, C to fill contour, U to undo last point, Y to del all contour points' };     
...
while 1  % till broken out of
    ...
    if isnan(X)
        ...
        switch key
          ...
          case 'z' 
            ...
            subDrawShaded(fovImg, bwCurr, zoomToAxis,contPoints);
            fprintf(1, 'Undo!  %d cells total now\n', nTotal);
            case 'u'
                if numel(contPoints)>0
                    contPoints(end,:)=[];
                end
                fprintf(1, 'Del. last contour point. New number of countour points is: %d\n', size(contPoints,1));
                subDrawShaded(fovImg, bwCurr, zoomToAxis,contPoints);
            case 'y'
                    contPoints=[];
                fprintf(1, 'Contour points were deleted \n');
                subDrawShaded(fovImg, bwCurr, zoomToAxis,contPoints);
            case 'c'
                %% do add cell using contour points
                if numel(contPoints)>2
                    [bwNew bwCell] = subAddCell2(bwCurr, contPoints);
                    contPoints=[];
                    oldTotal = nTotal;
                    nTotal = nTotal+1;
                    ...
                    fprintf(1, 'At least 3 contour points are needed. You selected only  %d countour points \n', numel(contPoints));
                end
    ...
    switch lower(selectionType)
      ...
      case 'normal'    % left-click: add
        ...
        fprintf(1, 'Added object #%d: %d pix\n', ...
                nTotal, sum(bwCell(:)));
      case 'alt'    % rigth-click: create contour points for next cell
        % make sure not in a cell
        if bwCurr(Y,X) == 1
            fprintf(1, '** Trying to add in existing cell region, try again\n');
            continue;
        end
        contPoints(end+1,1:2)=[Y X];
        fprintf(1, 'Added contour points for next object : %d\n', size(contPoints,1));
        subDrawShaded(fovImg, bwCurr, zoomToAxis,contPoints);
        continue
      otherwise
...
function imH = subDrawShaded(fovImg, bw, zoomToAxis,contPoints)

if nargin<4 || isempty(contPoints)
    shadeimg = imShade(fovImg, bw);
else
    bw2=zeros(size(bw));
    contInd=sub2ind(size(bw),contPoints(:,1), contPoints(:,2));
    bw2(contInd)=1;
    re=round(min(size(bw))/100);
    se = strel('square',re);  
    bw2 = imdilate(bw2,se);
    shadeimg = imShade(fovImg, bw,bw2*2,bw2*2);    
end
cla;
...
function [bwNew bwCell] = subAddCell2(bwCurr, contPoints)

%%% Real cell-adding logic is here

[nRows nCols] = size(bwCurr);

% create mask of current cells to avoid
se = strel('square',3);  % for cells-to-avoid: all 9 pix in a square;
                       % avoid diagonally-connected cells.
dilateorg = imdilate(bwCurr,se);

% do not order points,assuming user put them in the correct oreder
% order points
% cPos=mean(contPoints);
% cPoss=repmat(cPos,[size(contPoints,1) 1]);
% dPos=contPoints-cPoss;
% 
% theta=atan2(dPos(:,1),dPos(:,2));
% [dummy thetaorder]=sort(theta);
% 
% contPoints=contPoints(thetaorder,:);
contPoints(end+1,:)=contPoints(1,:);
...
t = 1:size(contPoints,1);
ts = 1:1/1000:size(contPoints,1);
xs = round(spline(t,contPoints(:,2),ts));
ys = round(spline(t,contPoints(:,1),ts));
xs=max(1,xs);
xs=min(size(tempmask,2),xs);
ys=max(1,ys);
ys=min(size(tempmask,1),ys);
inds=sub2ind(size(tempmask),ys,xs);
tempmask(inds)=true;
tempmask=imfill(tempmask,'holes');
bwCell = tempmask & ~dilateorg;
