...
av = mean(zstack_sub_red(:,:,1:10),3);
[out,reg] = stackRegister(zstack_sub_red,av,1);
zstack2_red = zeros(240,256,nPlanes);
zstack2_red(:,:,iPlane) = mean(reg,3);
...
zstack2_green = stackShifts(double(zstack_sub_green), -out(:,4:-1:3));
zstack3_green = zeros(240,256,nPlanes);
zstack3_green(:,:,iPlane) = mean(zstack2_green,3);
...
for iPlane = 2:nPlanes
    av = mean(reg,3);
    zstack_sub_red = zstack_red(:,:,((iPlane-1)*(frames/down))+1:iPlane*(frames/down));
    [out,reg] = stackRegister(zstack_sub_red,av);
    zstack2_red(:,:,iPlane)= mean(reg,3);
    zstack_sub_green = zstack_green(:,:,((iPlane-1)*(frames/down))+1:iPlane*(frames/down));
    zstack2_green = stackshifts(double(zstack_sub_green), -out(:,4:-1:3));
    zstack3_green(:,:,iPlane) = mean(zstack2_green,3);
...
writetiff(zstack2_red, fn_out_red);