...
plotAvg = 0;
area_list = {'LS'};
share_out = '\\duhs-user-nc1.dhe.duke.edu\dusom_glickfeldlab\All_Staff\public\ClassicConditioningPaper\LS2019';
for id = [1:3 5]
...
%             print([share_out '\CC_summary\Day' num2str(id) '\' img_fn '_Cell#' num2str(IC_list(1)) '-' num2str(IC_list(end)) '_psth.pdf'],'-dpdf', '-fillpage')
    ...
    for i = find(sum(expt_areas,2)')
        ...
        if id ~= 3
            ...
            pie([length(ind_preresp_only), length(ind_postresp_only), length(ind_preandpost), length(ind_neither)])
            legend('Pre reward only','Post reward only', 'Pre and Post', 'Neither')
            title('Reward Pre vs Post')

            subplot(1,2,2)
            ind_preresp_only = setdiff(ind_preresp, ind_preomitresp);
            ind_preomit_only = setdiff(ind_preomitresp, ind_preresp);
            ind_rewandomit = intersect(ind_preomitresp, ind_preresp);
            ind_neither = setdiff(ind, [ind_preresp ind_preomitresp]);
            pie([length(ind_preresp_only), length(ind_preomit_only), length(ind_rewandomit), length(ind_neither)])
            legend('Pre reward only','Pre omit only', 'Pre Rew and Omit', 'Neither')
            title('Pre Omit vs Reward') 
        else
            subplot(1,2,1)
            ...
            pie([length(ind_preresp_only), length(ind_unexpresp_only), length(ind_preandunexp), length(ind_neither)])
            legend('Pre reward only','Post unexp reward only', 'Pre and Unexp reward', 'Neither')
            ...
            ind_neither = setdiff(ind, [ind_postresp ind_unexpresp]);
             pie([length(ind_preresp_only), length(ind_unexpresp_only), length(ind_preandunexp), length(ind_neither)])
            legend('Post reward only','Post unexp reward only', 'Post and Unexp reward', 'Neither')
    ...
    for i = find(sum(expt_areas,2)')
        ...
        title('Pre reward- Rewarded trials')
        ylim([-1 6])
        xlim([-500 2000])
        if id == 4
            vline([1100], 'c')
        ...
        xlabel('Time from cue')
        ylabel('Spike rate (Hz)')
        title('Post reward- Rewarded trials')
