function [best_dir_fit, null_dir_fit, R_best_dir_fit, R_null_dir_fit, DI_fit, OI_fit, DS,...
        tuning_width, A1, A2, k1, k2, phi2, A, resnorm,...
        residual, exitflag, output] = estimate_dir_tuningKO2 (ydata, DoInterp, method, printflag)

    
% Mod by AK to fit Niell & Stryker, 2008
%
...
options = optimset('LargeScale', 'off', 'LevenbergMarquardt', 'on','MaxFunEvals',1000,'TolFun',10^(-15),'TolX',10^(-15));
...
orth1_dir_fit = mod(best_dir_fit + 90, 360);
orth2_dir_fit = mod(null_dir_fit + 90, 360);
R_best_dir_fit = VonMises2(A, best_dir_fit)+ymin;
R_null_dir_fit = VonMises2(A, null_dir_fit)+ymin;
R_orth1_dir_fit = VonMises2(A, orth1_dir_fit)+ymin;
R_orth2_dir_fit = VonMises2(A, orth2_dir_fit)+ymin;

%DI_fit = 1-R_null_dir_fit/R_best_dir_fit;
DI_fit = (R_best_dir_fit-R_null_dir_fit)./(R_best_dir_fit+R_null_dir_fit);

R_null_ori_fit= (R_orth1_dir_fit+R_orth2_dir_fit);
R_best_ori_fit= (R_best_dir_fit+R_null_dir_fit);

OI_fit = (R_best_ori_fit-R_null_ori_fit)./(R_best_ori_fit+R_null_ori_fit);
