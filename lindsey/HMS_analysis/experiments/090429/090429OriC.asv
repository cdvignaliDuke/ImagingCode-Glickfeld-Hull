...
pn = 'G:\users\lindsey\analysisLG\090429\OriCRegistered.tif';
pn2 = 'G:\users\lindsey\dataLG\090429\OriC';
analpn = 'g:\users\lindsey\analysisLG\090429';
...
%% reload masks
load 'g:\users\lindsey\analysisLG\090417\OriBcellmasks.mat';
load 'g:\users\lindsey\analysisLG\090417\OriBastromasks.mat';
...
%% cell time courses
load 'g:\users\lindsey\analysisLG\090417\OriBneuronmasks.mat';
...
for iCell = 1:16;
    figure;
    for trial = 0:3
        plot(timeCourses_lowcut(cycle+(trial*epoch:trial*epoch),iCell), 'r-');
        hold on;
    end
    for trial = 4:6
        plot(timeCourses_lowcut(cycle+(trial*epoch:trial*epoch),iCell), 'k-');
        hold on;
    end
    for trial = 7:9
        plot(timeCourses_lowcut(cycle+(trial*epoch:trial*epoch),iCell), 'b-');
        hold on;
    end
end
...
figure;
tcOffsetPlot(tcRemoveDC(ratio(:,5:10)));
figure;
...
figure;
tcOffsetPlot(tcRemoveDC(ratio(:,101:105)));
figure;
tcOffsetPlot(tcRemoveDC(ratio(:,106:110)));
figure;
tcOffsetPlot(tcRemoveDC(ratio(:,111:115)));
plot(ratio(:, :));
xlabel('Time (s)');
ylabel('Cell #');
