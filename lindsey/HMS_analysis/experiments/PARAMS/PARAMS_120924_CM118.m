...
PWD = 'G:\users\lindsey\analysisLG\active mice\CM118\120924\';
...
file_mat = strvcat(...
    '120924_CM118_run1_dec_reg.tif', ...
    '120924_CM118_run2_dec_reg.tif', ...
    '120924_CM118_run3_dec_reg.tif', ...
    '120924_CM118_run4_dec_reg.tif', ...
    '120924_CM118_run5_dec_reg.tif');


file_ch2 = 'AVG_110210_LG25_run1.tif';
file_mask = 'AVG_110210_LG25_run1.tif';

DIR_rand0 = strvcat('\\zoloto\bigstorlab\Lindsey\vis_stim_protocols\120924\CM118_run1\TF2_SF2_ori16_Con1_Nfrpstim240_Ret1_GabSz13_24Sep2012\',...
    '\\zoloto\bigstorlab\Lindsey\vis_stim_protocols\120924\CM118_run1\TF2_SF2_ori16_Con1_Nfrpstim240_Ret1_GabSz13_24Sep2012\');
...
randvec = [1 1
           ...
           2 3
           2 4
           2 5]; %directory #, randnum (within that directory)
...
RandStim_info.prot(count_prot).TFvec = [2*ones(1,32) 8*ones(1,32) 0];
RandStim_info.prot(count_prot).SFvec = [repmat([.16*ones(1,16) .04*ones(1,16)],1,2) 0];
RandStim_info.prot(count_prot).pos = [ones(1,65)];
RandStim_info.prot(count_prot).StimXpos = [0*ones(1,64) 0]; %x pos 
RandStim_info.prot(count_prot).StimYpos = [-20*ones(1,64) 0]; %y pos 
RandStim_info.prot(count_prot).Nstimtypes0000 = 65; %2 or 6 or 8
RandStim_info.prot(count_prot).oris = [repmat([0   22.5000   45.0000   67.5000   90.0000  112.5000  135.0000 157.5000  180.0000  202.5000  225.0000  247.5000  270.0000  292.5000 315.0000  337.5000], 1, 4) 0]*pi/180;
RandStim_info.prot(count_prot).contrastvec = [.8*ones(1,64) 0];
...
if Nprotocols > 1
    count_prot = 2;
RandStim_info.prot(count_prot).userun0 = [3:5];
RandStim_info.prot(count_prot).userunDR = [1];
...
if Nprotocols > 2
    count_prot = 3;
RandStim_info.prot(count_prot).userun0 = [4:5];
RandStim_info.prot(count_prot).userunDR = [1];
...
RandStim_info.prot(count_prot).StimYpos = [-10*ones(1,25) 0]; %y pos 
