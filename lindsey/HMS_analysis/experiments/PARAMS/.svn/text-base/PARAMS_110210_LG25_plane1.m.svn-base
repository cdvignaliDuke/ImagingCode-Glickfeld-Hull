...
PWD = 'G:\users\lindsey\analysisLG\2010\LG25\110210\';
...
file_mat = strvcat(...
    '110210_LG25_run1_reg.tif', ...
    '110210_LG25_run2.tif');

%file_mat = ['100502_u12_run3023__ch2_fix_reg.tif'];
file_ch2 = 'AVG_110210_LG25_run1.tif';
file_mask = 'AVG_110210_LG25_run1.tif';

DIR_rand0 = strvcat('G:\users\lindsey\dataLG\vis_stim_protocols\TF1_SF8_ori4_Con1_Nfrpstim240_Ret1_GabSz13_10Feb2011\', ...
                    'G:\users\lindsey\dataLG\vis_stim_protocols\TF8_SF1_ori4_Con1_Nfrpstim240_Ret1_GabSz13_10Feb2011\');
...
RandStim_info.prot(count_prot).TFvec = [2*ones(1,32) 0];
RandStim_info.prot(count_prot).SFvec = [0*ones(1,4) .01*ones(1,4) .02*ones(1,4) .04*ones(1,4) .08*ones(1,4) .16*ones(1,4) .32*ones(1,4) .64*ones(1,4) 0];
RandStim_info.prot(count_prot).pos = [ones(1,33)];
RandStim_info.prot(count_prot).StimXpos = [-25*ones(1,32) 0]; %x pos 
RandStim_info.prot(count_prot).StimYpos = [0*ones(1,32) 0]; %y pos 
RandStim_info.prot(count_prot).Nstimtypes0000 = 33; %2 or 6 or 8
RandStim_info.prot(count_prot).oris = [repmat([0 45 90 135],1,8) 0]*pi/180;
...
if Nprotocols > 1
    ...
    RandStim_info.prot(count_prot).userunDR = [3:5];

    RandStim_info.prot(count_prot).TFvec = [2 2 2 2 2 2 2 2 15 15 15 15 15 15 15 15 0];
    RandStim_info.prot(count_prot).SFvec = [0.02 0.02 0.02 0.02 0.16 0.16 0.16 0.16 ...
        0.02 0.02 0.02 0.02 0.16 0.16 0.16 0.16 0];
    RandStim_info.prot(count_prot).pos = [ones(1,17)];
    RandStim_info.prot(count_prot).StimXpos = [ones(1,16)*0 0]; %x pos for positions 1-9
    RandStim_info.prot(count_prot).StimYpos = [ones(1,16)*-14 0]; %y pos for positions 1-9
    RandStim_info.prot(count_prot).Nstimtypes0000 = 9; %2 or 6 or 8
    RandStim_info.prot(count_prot).oris = [0 45 90 135 0 45 90 135 0 45 90 135 0 45 90 135 0]*pi/180;
    RandStim_info.prot(count_prot).contrastvec = [0.8*ones(1,16) 0];
    RandStim_info.prot(count_prot).Fs1 = 1; %acq rate of imaging Hz %downsampled if applicable
    RandStim_info.prot(count_prot).Fs1_orig = 16; %acq rate of imaging Hz %downsampled if applicable
    RandStim_info.prot(count_prot).Fs1 = 32; %acq rate of imaging Hz %downsampled if applicable
    RandStim_info.prot(count_prot).Fs1_orig = 32; %acq rate of imaging Hz; ignore unless you have eeg
    RandStim_info.prot(count_prot).Noff1 = 150;
    RandStim_info.prot(count_prot).Non1 = 150;
    RandStim_info.prot(count_prot).Toff1 = RandStim_info.prot(count_prot).Noff1/RandStim_info.prot(count_prot).Fs1; %s
    RandStim_info.prot(count_prot).Ton1 = RandStim_info.prot(count_prot).Non1/RandStim_info.prot(count_prot).Fs1; %s
    RandStim_info.prot(count_prot).Ntot1 = RandStim_info.prot(count_prot).Fs1 * ...
        (RandStim_info.prot(count_prot).Ton1 + RandStim_info.prot(count_prot).Toff1);
