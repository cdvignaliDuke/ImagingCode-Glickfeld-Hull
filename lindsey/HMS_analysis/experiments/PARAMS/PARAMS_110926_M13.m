...
PWD = 'G:\users\lindsey\analysisLG\active mice\M13\110926\';
...
file_mat = strvcat(...
    '110926_M13_run1_max_reg.tif', ...
    '110926_M13_run2_max_reg.tif', ...
    '110926_M13_run3_max_reg.tif', ...
    '110926_M13_run4_max_reg.tif');

%file_mat = ['100502_u3912_run3023__ch2_fix_reg.tif'];
file_ch2 = 'AVG_110210_LG25_run1.tif';
file_mask = 'AVG_110210_LG25_run1.tif';

DIR_rand0 = strvcat('G:\users\lindsey\dataLG\vis_stim_protocols\110926\M13_run1\TF5_SF5_ori2_Con1_Nfrpstim240_Ret1_GabSz20_26Sep2011\',...
    'G:\users\lindsey\dataLG\vis_stim_protocols\110926\M13_run1\TF2_SF3_ori8_Con1_Nfrpstim240_Ret1_GabSz20_26Sep2011\');
...
RandStim_info.prot(count_prot).TFvec = [repmat([2 8],1,24) 0];
RandStim_info.prot(count_prot).SFvec = [repmat([.32*ones(1,2) .08*ones(1,2) .02*ones(1,2)],1,8) 0];
RandStim_info.prot(count_prot).pos = [ones(1,49)];
RandStim_info.prot(count_prot).StimXpos = [-25*ones(1,48) 0]; %x pos 
RandStim_info.prot(count_prot).StimYpos = [-5*ones(1,48) 0]; %y pos 
RandStim_info.prot(count_prot).Nstimtypes0000 = 49; %2 or 6 or 8
RandStim_info.prot(count_prot).oris = [0*ones(1,6) 45*ones(1,6) 90*ones(1,6) 135*ones(1,6) 180*ones(1,6) 225*ones(1,6) 270*ones(1,6) 315*ones(1,6) 0]*pi/180;
