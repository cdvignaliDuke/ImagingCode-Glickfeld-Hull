...
RandStim_info.prot(count_prot).SFvec = [0.02*ones(1,8) 0];
    ...
    RandStim_info.prot(count_prot).StimXpos = [ones(1,8)*20 0]; %x pos for positions 1-9
    RandStim_info.prot(count_prot).StimYpos = [ones(1,8)*0 0]; %y pos for positions 1-9
...
if Nprotocols > 1
    ...
    RandStim_info.prot(count_prot).StimXpos = [ones(1,8)*-35 0]; %x pos for positions 1-9
    RandStim_info.prot(count_prot).StimYpos = [ones(1,8)*-15 0]; %y pos for positions 1-9
    RandStim_info.prot(count_prot).Nstimtypes0000 = 9; %2 or 6 or 8
    RandStim_info.prot(count_prot).oris = [0 45 90 135 0 45 90 135 0]*pi/180;
    RandStim_info.prot(count_prot).contrastvec = [0.8*ones(1,8) 0];
    RandStim_info.prot(count_prot).Fs1 = 3.2; %acq rate of imaging Hz %downsampled if applicable
    RandStim_info.prot(count_prot).Fs1_orig = 3.2; %acq rate of imaging Hz; ignore unless you have eeg
    RandStim_info.prot(count_prot).Noff1 = 15;
