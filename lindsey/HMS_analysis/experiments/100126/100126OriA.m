...
pn = 'G:\users\lindsey\analysisLG\100126\OriARegistered.tif';
pn2 = 'G:\users\lindsey\dataLG\2010\100112\OriA';
analpn = 'g:\users\lindsey\analysisLG\100126';
...
save(fullfile(analpn,'OriAGFPneuronmasks.mat'),'mask_GFPneuron');

%% cell time courses
load 'g:\users\lindsey\analysisLG\100126\OriAneuronmasks.mat';
...
%get dF/F for trial
baseline_all = mean(timeCourses_lowcut(off:(off+on):end,:));
...
for iCell = 81:91;
    ...
    hold on;
    for trial = 0:9;
        plot(ratio_all(cycle+(trial.*(epoch:epoch)), iCell), 'c');
        hold on;
    end
end
