...
image(mask_cell);
save(fullfile(expt.dirs.analrootpn,'LG27_runs5to7_masks.mat'),'mask_cell');

%% cell time courses
%reload
mask_cell = load(fullfile(expt.dirs.analrootpn,'LG27_runs5to7_masks.mat'));
...
for iCell = 1:allcells;
    ...
    subplot(2,2,1);
    title('freely rotating; 40 deg gabor');
    subplot(2,2,2);
    title('freely rotating; 25 deg gabor');
    subplot(2,2,3);
    title('motor at 10 rpm; 25 deg gabor'); 
end
...
for iCell = 1:allcells;
    subplot(ceil(sqrt(allcells)),ceil(sqrt(allcells)),iCell)
    errorbar([0 45 90 135], dFoverF_avg(:,:,1,iCell), dFoverF_sem(:,:,1,iCell),'or-','markerfacecolor','r');
    hold on;
    errorbar([0 45 90 135], dFoverF_avg(:,:,2,iCell), dFoverF_sem(:,:,2,iCell),'om-','markerfacecolor','m');
    hold on;
    errorbar([0 45 90 135], dFoverF_avg(:,:,3,iCell), dFoverF_sem(:,:,3,iCell),'ok-','markerfacecolor','k');
    hold on;
    xlabel('Orientation (Deg)');
    ylabel('dF/F');
    axis([-15 150 -5 75]);
...
figure;
subplot(2,1,1);
errorbar([1:nCond], dFoverF_norm_cond_avg, dFoverF_norm_cond_sem, 'ok-','markerfacecolor','k');
xlabel('Condition');
