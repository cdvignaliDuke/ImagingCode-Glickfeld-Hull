...
data_avg = mean(data_red(:,:,20:50),3);

[out, data_red_reg] = stackRegister(data_red,data_avg);
clear data_red
...
if isfield(ds, 'trialOutcomeCell')
    ...
    nOn = ds.nScansOn;
    nOff = ds.nScansOff;
    data_tr = reshape(data_green_reg,[sz(1), sz(2), nOn+nOff, ntrials]);
    ...
    if ds.doTFStim
        ...
        for iTF = 1:length(TFs)
            ind = find(TF == TFs(iTF));
            dfof_max(:,:,iTF) = mean(nanmean(data_dfof(:,:,nOff:nOff+nOn,ind),4),3);
        end
    end  
...
clear data_mat data_f data_df data_dfof
