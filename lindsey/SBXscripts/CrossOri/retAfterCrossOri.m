...
%%
    if ~isempty(expt(iexp).retFolder)
        ...
        load(fullfile(LG_base, 'Analysis\2P', [date '_' mouse], [date '_' mouse '_' co_run_str], [date '_' mouse '_' co_run_str '_respData.mat']),'resp_ind');
        ...
        data_dfof_stim = nan(nOn+nOff, nCells, nEl, nAz, nPha);
        h_stim = nan(nCells, nEl, nAz, nPha);
        p_stim = nan(nCells, nEl, nAz, nPha);
        trial_n = zeros(nEl, nAz, nPha,2);
        stim_resp_avg = nan(nCells, nEl, nAz, nPha,2);
        stim_resp_mat = [];
        stim_list = [];
        for iEl = 1:nEl
        ...
        for iCell = 1:nCells
            ...
            if start>25
                ...
                print(fullfile(LG_base, 'Analysis\2P', [date '_' mouse], [date '_' mouse '_' run_str], [date '_' mouse '_' run_str '_retCells' num2str(n) '.pdf']),'-dpdf','-fillpage')
                figure;
                movegui('center')
                n = n+1;
                start = 1;
            ...
            rf_all(:,:,:,iCell) = rgb_temp(:,:,[1 3]);
            start = start+1;
            if find(resp_ind == iCell)
                title(['Cell ' num2str(iCell) '- 1'])
            else
        ...
        nMod = length(mod_ind);
        rf_corr = zeros(nMod);
        rf_long = reshape(rf_all, [nEl.*nAz.*nPha nCells]);
        ind = find(~isnan(rf_long(:,1)));
        ...
        for i = 1:nMod
            ...
            for j = 1:nMod
                ...
                if j>i
                    ...
                    d_rf(i,j) = triu2vec(corrcoef(rf_long(ind,iC),rf_long(ind,jC)));
                end
            end
        end
        ...
        drf = reshape(d_rf,[nMod.*nMod 1]);
        drf(isnan(dphase_temp)) = [];
        ...
        [n edges bins] = histcounts(drf,5);
        phase_avg = zeros(2,length(n));
        rf_avg = zeros(2,length(n));
        for i = 1:length(n)
            ind = find(bins==i);
            phase_avg(1,i) = mean(dphase(ind,:),1);
            rf_avg(1,i) = mean(drf(ind,:),1);
            phase_avg(2,i) = std(dphase(ind,:),[],1)./sqrt(n(i));
            rf_avg(2,i) = std(drf(ind,:),[],1)./sqrt(n(i));
        end
        subplot(2,2,2)
        errorbar(rf_avg(1,:), phase_avg(1,:), phase_avg(2,:),phase_avg(2,:),rf_avg(2,:),rf_avg(2,:))
        xlabel('Spatial Correlation')
        ylabel('Phase distance (deg)')
        ylim([0 200])
        movegui('center')
        ...
        save(fullfile(LG_base, 'Analysis\2P', [date '_' mouse], [date '_' mouse '_' run_str], [date '_' mouse '_' run_str '_phaseDist.mat']),'dphase','drf')
    end
end