...
%% direction tuning
clear data_targ data_targ_dfof data_adapt data_adapt_dfof data_f
...
% 
% h_ori = nan(nOri, nCells);
% [h_all p_all] = ttest(mean(data_stim_dfof(base_win,:,ind_cond{1}),1),mean(data_stim_dfof(resp_win,:,ind_cond{1}),1),'tail','left','dim',3);
% [h_adapt, p_adapt] = ttest(mean(data_adapt_dfof(base_win,:,ind_aCon1),1),mean(data_adapt_dfof(resp_win,:,ind_aCon1),1),'tail','left','dim',3);
% for iori = 1:nOri
...
% end
% 
% good_ind = find(sum([h_all; h_ori; h_adapt],1));
