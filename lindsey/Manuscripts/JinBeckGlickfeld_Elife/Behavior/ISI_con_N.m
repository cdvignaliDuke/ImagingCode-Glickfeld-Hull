function [output] = ISI_con_N(Early,input,Off,tCycleNum,trialnum,Leverup,Leverdown,Success,tOrientation,Orien,RT_HM,StimOff,StimOff_pre,Miss)
    ...
    for  i_off = 1:length(Off) % off pre pre
        if cycle>=3 && cycle==length(trialoffs)  % only consider FA occur after cycle 2, because target never occur on cycle 1&2; through out early release for target
            ...
            for ii_off = 1:length(Off) % off before the last BS
                % if release in the time window-extended to the 250+100 ms
                ...
                if trialoffs(cycle-2)==Off(ii_off) && FA_RT<200 && trialoffs(cycle-1)==250
                    if cycle>3 && trialoffs(cycle-3)==Off(i_off) % note that we lose some cycle 3 trials
                        output.FA_Num(i_off,ii_off) = output.FA_Num(i_off,ii_off)+1;
        ...
        if Early(i)
            ...
            if FA_RT<200 && trialoffs(cycle-1)==250 
                ...
                for i_off = 1:length(Off) % pre pre off
                    ...
                    output.CR_Num(i_off,ii_off)= output.CR_Num(i_off,ii_off)+sum(CR_pres==Off(i_off)& CR_offs==Off(ii_off));
                    end
