Allmice = {'527','549','559','563','588','587'};
...
for i_mice = 1:num_subj
    ...
    % need to be reorganized for 45 deg base
    if i_mice >=5
    Thresh(i_mice,:) = circshift(out.Threshall,2);
    FA(i_mice,:) = circshift(out.RealFA_B,2);
    AbsThresh(i_mice,:) = circshift(out.AbsThreshall,2);   
...
%% plot threshold and FA in the same axis?
figure
subplot(1,2,1)
NormThresh = Thresh - repmat(Thresh(:,1),1,3);
Mean_norm = mean(NormThresh,1);
sem_norm = std(NormThresh,[],1)./sqrt(num_subj);
for i = 1:num_subj
    scatter([1 3],NormThresh(i,[2 3]),'MarkerEdgeColor',[0.5 0.5 0.5],'sizeData',10)
    hold on
end 
h=errorbar([2 1 3],Mean_norm,sem_norm,'k','marker','o');
h.LineStyle = 'none';
...
sem_norm = std(NormThresh,[],1)./sqrt(num_subj);
for i = 1:num_subj
    scatter([1 3],NormThresh(i,[2 3]),'MarkerEdgeColor',[0.5 0.5 0.5],'sizeData',15)
    hold on
end 
h=errorbar([2 1 3],Mean_norm,sem_norm,'k','marker','o');
h.LineStyle = 'none';
