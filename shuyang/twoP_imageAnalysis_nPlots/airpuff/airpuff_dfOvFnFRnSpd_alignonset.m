...
for ii = 1: length(sessions)
    ...
    for i = 1:length(airpuffon) 
        ...
        elseif sum(speed((airpuffon(i)-period):(airpuffon(i)-1))~= 0) >= period-4 && sum(speed(airpuffon(i):(airpuffon(i)+8))~= 0) >= 6
...
% averages and stes  
    ...
    for i = 1: size(spk_logic,2)                                                    %for each cell
        for j = 1:size(frms_airstim_run,2)                                    %for each running trial
            spk_airpuff_run_mat (:,j,i) = spk_logic(frms_airstim_run(:,j),i);%spk_airpuff_run_mat: trial*frame*cell
        end
    end
...
for ii = 1: length(sessions)
 ...
 % ------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    %average across cells:
    dfOvFbtm_airstimAve_run = mean(dfOvFbtm_airpuff_run_cells,2); %firing rate = spike probablity*imaging rate
    dfOvFbtm_airstimAveSte_run = std(dfOvFbtm_airpuff_run_cells,0,2)/sqrt(size(dfOvFbtm_airpuff_run_cells,2)); % cell by cell variation
...
% use fluorescence during stationary without airpuff
    ...
    for i = 1: size(dfOvF_stay,2)                                                    %for each cell
        for j = 1:size(frms_airstim_run,2)                                           %for each running trial
            dfOvFstay_airpuff_run_mat(:,j,i) = dfOvF_stay(frms_airstim_run(:,j),i);  %spk_airpuff_run_mat: trial*frame*cell
        end
    end
    % average across trials:
    ...
    save([image_analysis_dest '\' sessions{ii} '_dfOvF.mat' ],...
        'dfOvFstay_airpuff_run_mat','dfOvFstay_airpuff_stay_mat','dfOvFbtm_airpuff_run_mat','dfOvFbtm_airpuff_stay_mat',...
   ...
   %{
    ...
    shadedErrorBar(x,spd_airstimAve_run*2*3.1415926*7.5/128,spd_airstimAveSte_run*2*3.1415926*7.5/128);
    xlabel('time(s)'); ylabel('speed(cm/s)'); vline(period/30,'r');
    text(0.1,min(spd_airstimAve_run),['n = ' num2str(size(frms_airstim_run,1))]);
    ...
    saveas(fig, [image_analysis_dest '\' sessions{ii} '_airpuffTrigAve']);
