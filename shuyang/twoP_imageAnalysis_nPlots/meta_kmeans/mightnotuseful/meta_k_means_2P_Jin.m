...
function [allclusters, centroidcorr, dendmem, dunnsinitial]=meta_k_means_2P_Jin(eventsmat, distmetric);
...
%eventsmat should be a 2D matrix, cell*frame
...
dendevents=sum(eventsmat'); %sum across whole time for each cell, you get 1*nPCs
silentdends=find(dendevents<=1); % find the PCs that fluorescence is too low
silentdendcnt=zeros(numden,1);
silentdendcnt(silentdends)=1; % all dim dendrites equals to 1
...
if length(silentdends)>=numden-4 
    ...
    expr=zeros(numden-length(silentdends),numtimes);
    cnt=1;
    for i=1:numden
        
        if silentdendcnt(i)~=1
            expr(cnt,:)=eventsmat(i,:); %rewrite eventsmat, only include the ones that are not dim, this is all what this if loop is doing?
            cnt=cnt+1;
        end
    end
    numden = size(expr,1);
    ...
    % k clusters            % this is consistant with I Ozden & H.M. Lee et al. k=3 and run for 1000 times is reliable
    for k = 3
        ...
        for t = 1:1000
            ...
            try
                ...
                [IDX, C, SUMD, D] = kmeans(expr, k, 'Distance', distmetric); %IDX tells you which cluster that dendrite belongs to, so if your k=3, IDX should be a number between 1 and 3.
        ...
        if ~isempty(IDX)
            ...
            [newclusters, dunnsinitial, centroidcorr, dendmem]=ioclustervalidity2_Jin(allclusters, expr, combclusters); %calculate dunns and some correlations
            ...
            while endclustering==0
                ...
                for i=1:numclusters
                    for j=(i+1):numclusters
                        [newclusters, dunns, centroidcorr, dendmem]=ioclustervalidity2_Jin(allclusters, expr, [i j]);
