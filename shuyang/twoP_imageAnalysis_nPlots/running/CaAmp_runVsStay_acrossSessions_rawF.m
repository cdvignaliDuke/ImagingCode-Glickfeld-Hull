...
%% 
ave_rawF_btm_stay_iso_Allsessions = zeros(length(sessions),30);
ave_rawF_btm_run_iso_Allsessions = zeros(length(sessions),30);
...
rawF_run_isoevents_neurons_all = [];
rawF_stay_isoevents_neurons_all = [];
rawF_spk_run_begin_neurons_all = [];
rawF_spk_run_middle_neurons_all = [];
rawF_spk_run_late_neurons_all = [];
...
for ii = 1: length(sessions)
    ...
    ave_rawF_btm_stay_iso_Allsessions(ii,:) = isoCa_output.ave_rawF_stay_iso_session;
    ave_rawF_btm_run_iso_Allsessions(ii,:) = isoCa_output.ave_rawF_run_iso_session;
    rawF_run_isoevents_neurons_all = cat(1,rawF_run_isoevents_neurons_all,isoCa_output.rawF_run_isoevents_neurons);
    rawF_stay_isoevents_neurons_all = cat(1,rawF_stay_isoevents_neurons_all,isoCa_output.rawF_stay_isoevents_neurons);
    rawF_spk_run_begin_neurons_all = cat(1,rawF_spk_run_begin_neurons_all,isoCa_output.rawF_spk_run_begin_neurons);
    rawF_spk_run_middle_neurons_all = cat(1,rawF_spk_run_middle_neurons_all,isoCa_output.rawF_spk_run_middle_neurons);
    rawF_spk_run_late_neurons_all = cat(1,rawF_spk_run_late_neurons_all,isoCa_output.rawF_spk_run_late_neurons);
    nPCs_run (ii) = size(isoCa_output.rawF_spk_run_begin_neurons,1);
...
ave_rawF_stay_iso_across = mean(ave_rawF_btm_stay_iso_Allsessions);
ave_rawF_run_iso_across = mean(ave_rawF_btm_run_iso_Allsessions);

% 
ste_rawF_stay_iso_across = std(rawF_stay_isoevents_neurons_all)/sqrt(size(rawF_stay_isoevents_neurons_all,1));
ste_rawF_run_iso_across = std(rawF_run_isoevents_neurons_all)/sqrt(size(rawF_run_isoevents_neurons_all,1));

% t test
[h,p] = ttest(ave_rawF_stay_iso_across,ave_rawF_run_iso_across);
...
errorbar(x,ave_rawF_run_iso_across,ste_rawF_run_iso_across,'.','LineStyle','-','linewidth', 1.25,'MarkerSize',20); hold on;
...
savefig(['Z:\Analysis\2P_MovingDots_Analysis\across_sessions\' 'across_sessions_CaAmpRunvsStay_rawF.fig']);

%plot: make the y start at the same place
diff = ave_rawF_stay_iso_across(1)-ave_rawF_run_iso_across(1);
ave_rawF_stay_iso_across_plot = ave_rawF_stay_iso_across - diff;
x = (1:30)/30;
figure;
%haven't figured out how to use RGB colors in errorbar
errorbar(x,ave_rawF_stay_iso_across_plot,ste_rawF_stay_iso_across,'.','LineStyle','-','linewidth', 1.25,'MarkerSize',20); hold on;
errorbar(x,ave_rawF_run_iso_across,ste_rawF_run_iso_across,'.','LineStyle','-','linewidth', 1.25,'MarkerSize',20); hold on;
%legend(['stationary nevents=' num2str(ntotalevents_stay)],['running nevents=' num2str(ntotalevents_run)]);
legend('stataionry','running');
%text(0.75,0.45, ['n total PCs=' num2str(ntotalPCs)]);
%ylim([0 0.6]);
xlabel('time(s)'); ylabel('rawF');
savefig(['Z:\Analysis\2P_MovingDots_Analysis\across_sessions\' 'across_sessions_CaAmpRunvsStay_rawF_samey.fig']);
...
figure;
fast_errbar(x,rawF_spk_run_begin_neurons_all,1,'color',[0.1373 0.5451 0.2706]);hold on;
fast_errbar(x,rawF_spk_run_middle_neurons_all,1,'color',[0.2549 0.6706 0.3647]); hold on;
fast_errbar(x,rawF_spk_run_late_neurons_all,1,'color',[0.7294 0.8941 0.7020]); hold on;
