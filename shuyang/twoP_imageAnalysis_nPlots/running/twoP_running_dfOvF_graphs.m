...
[hAx,hline1,hline2] = plotyy(x(n1:n2),speedcm(n1:n2),x(n1:n2),avedfOvF_btm_cl(n1:n2));
...
totalT1 = 75; %2s # total T = of frames before running onset + # of frames following running onset/ # of frames before running offset + # of frames after running offset
aftRunStay = 90; % # of frames that speed = 0 after running
aftRun = 105;%# of frames to include after running ends
totalT2 = 135; 
[~,~,frms_runTrig_mat,frms_runoff_mat,frames_runoff_include,~]= findFrames_runWindows_2P(speed,...
    frames_run_cell,period,befoRunStay,totalT1,totalT2,aftRunStay,befoRun,aftRun);
save([behav_dest days '_behavAnalysis.mat'],'frames_runoff_include','-append');
...
vline((totalT2-aftRun)/30,'r'); ylabel('df/f'); %xlabel('frame');
%ylim([0 0.05]);
...
vline((totalT2-aftRun)/30,'r');ylabel('speed(cm/s)'); xlabel('time(s)');
%ylim([-5 20]);
