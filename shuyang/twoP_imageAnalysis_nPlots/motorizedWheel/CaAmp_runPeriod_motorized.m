...
%% calcium amplitude during different runing period
 ...
 for ii = 1: length(sessions)
     ...
     dfOvF_btm_runfast_isoevents = Caoutput.dfOvF_btm_runfast_isoevents;
     dfOvF_btm_runslow_isoevents = Caoutput.dfOvF_btm_runslow_isoevents;
     
     % put slow trials and fast trials together
     spk_iso_run_neurons = cell(size(spk_iso_runslow_neurons));
     dfOvF_btm_run_isoevents = cell(size(spk_iso_runslow_neurons));
     for c = 1:size(spk_iso_runslow_neurons,2)
         spk_iso_run_neurons{c} = cat(1,spk_iso_runslow_neurons{c}, spk_iso_runfast_neurons{c}); 
         dfOvF_btm_run_isoevents{c} = cat(1,dfOvF_btm_runslow_isoevents{c},dfOvF_btm_runfast_isoevents{c});
     end
     ...
     dfOvF_spk_run_begin_neurons = zeros(size(spk_iso_run_neurons,2),30);
     dfOvF_spk_run_middle_neurons = zeros(size(spk_iso_run_neurons,2),30);
     dfOvF_spk_run_late_neurons = zeros(size(spk_iso_run_neurons,2),30);
     isospk_run_begin = 0;
     isospk_run_middle = 0;
     isospk_run_late = 0;
     dfOvF_spk_run_begin = cell(1,size(spk_iso_run_neurons,2));
     dfOvF_spk_run_middle = cell(1,size(spk_iso_run_neurons,2));
     dfOvF_spk_run_late = cell(1,size(spk_iso_run_neurons,2));
     for c = 1: size(spk_iso_runslow_neurons,2)
     ...
     saveas(Caevent_run,[image_analysis_dest '\' days{ii} '_CaAmp_runPeriod']);
     
     Carun=figure;
     bar([isospk_run_begin,isospk_run_middle,isospk_run_late]);
     set(gca,'XTickLabel',{'0-0.5s','0.5-1.5s','later than 1.5s'});
     ylabel('number of events');
     saveas(Carun,[image_analysis_dest '\' days{ii} '_CaAmp_runPeriod']);
     ...
     save([image_analysis_dest sessions{ii} '_isoCaEvent.mat'],...
         ...
         'isospk_run_begin','isospk_run_middle','isospk_run_late','-append');
