...
for c = 1:size(dfOvF,2) %for each cell
    ...
    for t = 1:size(stationary_mat,1)
        dfOvF_stationary(t,:,c) = dfOvF(stationary_mat(t,:),c);
    end
end
...
dfOvF_fast_cells = mean(squeeze(mean(dfOvF_fast,1))); % average across trials
dfOvF_fast_all = mean(dfOvF_fast_cells);
dfOvF_fast_ste = std(dfOvF_fast_cells,0,2)/sqrt(size(dfOvF,2));

dfOvF_slow_cells = mean(squeeze(mean(dfOvF_slow,1)));
dfOvF_slow_all = mean(dfOvF_slow_cells);
dfOvF_slow_ste = std(dfOvF_slow_cells,0,2)/sqrt(size(dfOvF,2));

dfOvF_stay_cells = mean(squeeze(mean(dfOvF_stationary,1)));
dfOvF_stay_all = mean(dfOvF_stay_cells);
dfOvF_stay_ste = std(dfOvF_stay_cells,0,2)/sqrt(size(dfOvF,2));

save([image_analysis_dest sessions '_dfOvF.mat'],'dfOvF_fast_cells','dfOvF_slow_cells','-append');
