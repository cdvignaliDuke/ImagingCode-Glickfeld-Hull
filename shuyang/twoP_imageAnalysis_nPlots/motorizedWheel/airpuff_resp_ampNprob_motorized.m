...
for ii = 1:length(sessions)
    ...
    dfOvFbtm_airpuff_stay_cells = dfOvF_strct.dfOvF_airpuff_stay_cells;
    dfOvFbtm_airpuff_fastrun_cells = dfOvF_strct.dfOvF_airpuff_fast_cells;
    dfOvFbtm_airpuff_slowrun_cells = dfOvF_strct.dfOvF_airpuff_slow_cells;
    dfOvFbtm_airpuff_run_cells = (dfOvFbtm_airpuff_fastrun_cells+dfOvFbtm_airpuff_slowrun_cells)/2; %combine fast running trials and slow running trials, this is the same thing as combine the trials first and then average across trials
    ...
    for c = 1: size(dfOvFbtm_airpuff_stay_cells,2)
        ...
        baseline_cell_stay(c) = mean(temp(1:29));
        resp_cell_stay(c) = max(temp(31:36)) - baseline_cell_stay(c);
    end
    ...
    for c = 1: size(dfOvFbtm_airpuff_run_cells,2)
        ...
        baseline_cell_run(c) = mean(temp(1:29));
        resp_cell_run(c) = max(temp(31:36)) - baseline_cell_run(c);
    end
    ...
    savefig([image_analysis_dest 'resp\' sessions{ii} '_airpuff_resp_amp_scatter']);
...
for ii = 1:3
    image_analysis_dest = [image_analysis_base, sessions{ii}, '\'];
    dfOvF_strct = load([image_analysis_dest sessions{ii} '_dfOvF.mat']);
    dfOvFbtm_airpuff_stay_mat = dfOvF_strct.dfOvF_airpuff_stay;      % trial*frame*cell
    dfOvFbtm_airpuff_allrun_mat = dfOvF_strct.dfOvF_airpuff_run;
    ...
    for c = 1: size(dfOvFbtm_airpuff_stay_mat,3)
        temp1 = dfOvFbtm_airpuff_stay_mat(:,1:29,c);
        base_trials_stay(:,c) = mean(temp1,2);
        stdbase_trials_stay(:,c) = std(temp1,0,2);
        temp2 = dfOvFbtm_airpuff_stay_mat(:,31:36,c);
        ...
        temp3 = dfOvFbtm_airpuff_allrun_mat(:,1:29,c);
        base_trials_run(:,c) = mean(temp3,2);
        stdbase_trials_run(:,c) = std(temp3,0,2);
        temp4 = dfOvFbtm_airpuff_allrun_mat(:,31:36,c);
...
%     resp_hist = figure;
%     subplot(1,2,1);histogram(resp_trials_run_vec,'BinWidth',0.05);title('resp amp run');
%     subplot(1,2,2);histogram(resp_trials_stay_vec,'BinWidth',0.05);title('resp amp stay');
%     supertitle(sessions{ii});
%     saveas(resp_hist,[image_analysis_dest '\' sessions{ii} '_resp_hist']);
...
%     resp_prob_stay = zeros(1,size(dfOvFbtm_airpuff_stay_mat,3));
    resp_prob_run_std = zeros(1,size(dfOvFbtm_airpuff_stay_mat,3));
    resp_prob_stay_std = zeros(1,size(dfOvFbtm_airpuff_stay_mat,3));
    for c = 1: size(dfOvFbtm_airpuff_stay_mat,3)
        % use ave baseline+2std
        resp_prob_run_std(c) = sum(max_trials_run(:,c)> base_trials_run(:,c)+3*stdbase_trials_run(:,c))/ntrials_run; %when you get very few numbers of running trials, the response probability during running of a bunch of cells will be the same
        resp_prob_stay_std(c) = sum(max_trials_stay(:,c)>base_trials_stay(:,c)+3*stdbase_trials_stay(:,c))/ntrials_stay;
...
%         resp_prob_stay(c) = sum(resp_trials_stay(:,c)>resp_thres)/ntrials_stay;
    ...
    mean_resp_prob_run = mean(resp_prob_run_std);
    mean_resp_prob_stay = mean(resp_prob_stay_std);
%     scatter_prob = figure;
%     scatter(resp_prob_stay_std,resp_prob_run_std,8,'filled','MarkerEdgeColor',...
%         [0.45 0.45 0.45],'MarkerFaceColor',[0.45 0.45 0.45]); hold on;
%     scatter(mean_resp_prob_stay,mean_resp_prob_run,8,'filled','MarkerEdgeColor',...
...
%     title(['airpuff response probability ' sessions{ii}]);
%     savefig([image_analysis_dest 'resp\' fig_name]);
%     
...
%     savefig([image_analysis_dest 'resp\' sessions{ii} '_resp_prob_std_mask']);
    ...
    save([image_analysis_dest '\' sessions{ii} '_dfOvF.mat' ],...
        ...
        'resp_prob_run_std','resp_prob_stay_std',...
