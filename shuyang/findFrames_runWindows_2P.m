...
for m = 1: size(frames_run_cell,2)
        ...
        elseif (frames_run_cell{m}(1)-befoRunStay <1) || (frames_run_cell{end}(end)+ aftRunOff > frames(end))
            ...
            if length(frames_runTrigger_temp) >= runTriggerDura && sum(speed(frames_runTrigger_temp(1:15)) == 0) == 15 
               frames_runTrigger_mat = cat(2, frames_runTrigger_mat, frames_runTrigger_temp(1:45));
            end
            frms_runoff_temp = frames_run_cell{m}(1):frames_run_cell{m}(end)+aftRunOff;
            if length(frms_runoff_temp) >= runTriggerDura && sum(speed(frms_runoff_temp(end-14:end))==0)==15
                frms_runoff_mat = cat(2,frms_runoff_mat, frms_runoff_temp(end-44:end));
            end
