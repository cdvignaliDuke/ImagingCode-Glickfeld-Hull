...
stay_woTone = behav_output.stay_woTone;
...
for c = 1: size(TCave,2)
    ...
    num_spks_cell(c) = sum(spk_logic(stay_woTone,c)==1);
    FRstay_cell(c)= num_spks_cell(c)/length(stay_woTone)*30; % firing rate = # of spikes/duration(s)
end
...
% load mask3D final
filename2 = dir([image_analysis_dest 'getTC\' '*' 'thresh97_coor0.8_mask3D_final.mat']);
...
figure; imshow([image_analysis_dest 'getTC\' 'AVG_' sessions '_000_rgstr_tiff_1_29999_50_ref36_' 'jpeg.jpg']); hold on;
