...
for i = 1: size(filename,1)
    behav_dest{i} = ['Z:\Analysis\motorizedWheel_Analysis\running\behavioral_analysis\' sessionID '_' num2str(i)];
...
run_slow_mat = zeros(nslowTrials,lenframe_include_run); % each row is a trial, if changed trial length in the middle of the experiment, the for loop won't work
run_fast_mat = zeros(nfastTrials,lenframe_include_run);
staytionary_mat = zeros(nstayTrials,lenframe_include_stay); 
run_slow_vec = [];
run_fast_vec = [];
stay_vec = [];
runslow_start = zeros(1,nslowTrials);
runfast_start = zeros(1,nfastTrials);
...
for t = 1:ntrials
    ...
    elseif commandVolt(t) == 3
        ...
        runfast_start(t2) = countValue{t}(1);
...
run_start = [runslow_start,runfast_start];

% save matrices
save([behav_dest{i} '\' sessionID '_' num2str(i) '_behavAnalysis.mat' ],...
   'staytionary_mat','run_slow_mat','run_fast_mat','run_slow_vec',...
   'runslow_start','runfast_start','run_fast_vec','stay_vec','run_start','-append');
end
