...
x = pix_fgta2(xLeft2:(xLeft2+width),yBottom2:(height+yBottom2));
    ...
    imRegisTForm = imregtform(double(x), double(a), 'similarity', optimizer, metric);
    x = imwarp(double(x),imRegisTForm, 'OutputView', imref2d(size(a)));
    imagesc(x)
    pos = get(gca, 'Position');
    pos(1) = 0.05;
    pos(3) = 0.02;
...
%     if xLeft2 > 12 && xLeft3 < 488 && yBottom3 > 12 && yBottom3 < 772
...
% print(fullfile(fnout, [ref_date '_' mouse], [ref_date '_' mouse '_' ref_str], ['AcrossAllDays'], ['CellMaps'], [ref_date '_' mouse '_' ref_str '_map_xcorr_pix.pdf']),'-dpdf', '-bestfit')
