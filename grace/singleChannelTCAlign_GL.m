...
data_avg = mean(data(:,:,16001:16500),3);
...
for iStim = 1:size(data_dfof_avg,3)
...
figure; imagesc(mask_cell)
    ...
    load(fullfile(fnout, [date '_' mouse], [date '_' mouse '_' ref_str], [date '_' mouse '_' ref_str '_stimActFOV.mat']))
    load(fullfile(fnout, [date '_' mouse], [date '_' mouse '_' ref_str], [date '_' mouse '_' ref_str '_reg_shifts.mat']))
...
%     print(fullfile(fnout, [date '_' mouse], [date '_' mouse '_' run_str], ['RGB Overlays'], [date '_' mouse '_' run_str '_data_avg_FOVs.pdf']), '-dpdf','-bestfit')
    ...
    r2rFGTA = imwarp(double(reg),fgta3, 'OutputView', imref2d(size(reg)));
    r2rFGTA_dfof = imwarp(double(data_dfof_max),fgta3, 'OutputView', imref2d(size(data_dfof_max)));
...
%     figure; subplot(2,1,1);imagesc(rgb_reg2ref); title(['day 2 on day 1, data reg rgb overlay'])
...
%     figure; subplot(2,1,2);imagesc(rgb_reg2ref_dfof); title(['day 2 on day 1, dfof rgb overlay'])
