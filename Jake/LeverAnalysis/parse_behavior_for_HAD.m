function [lever, frame, trial_outcome] = parse_behavior_for_HAD(input, first_frame, last_frame, frame_times)
...
trial_start =  double(cell2mat(input.tThisTrialStartTimeMs));
...
for i=1:num_trials-1
    ...
    c_count = double(input.counterValues{i});
    c_time =  double(input.counterTimesUs{i})/1000;
    
    frame.times(end+1:end+length(c_time)) = c_time;
    frame.counter_by_time(end+1:end+length(c_count)) = c_count;
    c_time = round(c_time);
    if(length(c_count) < 2) % if less than 2 camera is off
        pre_frame_count = NaN;
    end
    for j=1:length(c_count)
        d_frame = c_count(j) - pre_frame_count;
        d_time = c_time(j) - pre_frame_time;
        update_inx = (pre_frame_time:c_time(j)) -  START_EXP_TIME+1;
...
%This code looks at counter.frames while it is still in free floating
...
%frame.counter has been cut to align with the camera start.....OR NOT
first_time_baseline=  find(frame.counter>= first_frame, 1, 'first');
last_time_baseline=  find(frame.counter<= last_frame, 1, 'last');
        ...
        for i = 1:length(ind_press)      
            if ind_press(i) == 1                   %if this is the first lever event of the trial...   (also it must be a press)
                if leverTimes(ind_press(i))- (cell2mat(input.tThisTrialStartTimeMs(iT))*1000) > holdT_min + 100000   %and if this lever press occurs a certain amount of time after trial start...
                    baseline_times(1,iT) = leverTimes(ind_press(i)) - holdT_min - 100000; 
...
%     if baseline_timesMs(1,t)>=first_time_baseline & baseline_timesMs(2,t)<=last_time_baseline;
%         baseline_timesMs_cut = [baseline_timesMs_cut, baseline_timesMs(:,t)];
%     elseif isnan(baseline_timesMs(1,t))==1
%         baseline_timesMs_cut = [baseline_timesMs_cut, baseline_timesMs(:,t)];
%     end
% end
% baseline_timesMs = baseline_timesMs_cut;
