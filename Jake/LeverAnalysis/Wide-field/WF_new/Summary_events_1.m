...
a = [fliplr(nanmean(a,2)')'; nanmean(a,2)];
a(size(a,1)/2) = [];
...
b1 = b(:,11); b2 = b(:,10); b3 = b(:,15); b4 = b(:,5);
...
xlabel('Lag (s)')
ylabel('Auto correlation');
supertitle('example auto correlation for cells from img 38');
saveas(fig, [out_base 'img38_autocorrelation.fig']);
print([out_base 'imd38_autocorrelation.eps'], '-depsc');
print([out_base 'img38_autocorrelation.pdf'], '-dpdf');
...
% supertitle(['PSTH- responsive cells- Black: success; Red: Failure'])
% saveas(fig, [out_base 'Summary_PSTH_avgexpt_10Hz.fig']);
% print([out_base 'Summary_PSTH_avgexpt_10Hz.eps'], '-depsc');
% print([out_base 'Summary_PSTH_avgexpt_10Hz.pdf'], '-dpdf');
% 
...
tth = [1-(size(all_spont_renorm,2)/2):(size(all_spont_renorm,2)/2)].*double(min((TC_ifi)));
subplot(1,2,1)
errorbar(tth, nanmean(all_spont_renorm(cell_use,:),1), std(all_spont_renorm(cell_use,:),[],1)./sqrt(length(cell_use)), '-m')
hold on;
errorbar(tth, nanmean(all_success_renorm(cell_use,:),1), std(all_success_renorm(cell_use,:),[],1)./sqrt(length(cell_use)), '-k')
hold on;
errorbar(tth, nanmean(all_press_renorm(cell_use,:),1), std(all_press_renorm(cell_use,:),[],1)./sqrt(length(cell_use)), '-c')
hold on;
errorbar(tth, nanmean(all_fail_renorm(cell_use,:),1), std(all_fail_renorm(cell_use,:),[],1)./sqrt(length(cell_use)), '-r')
xlim([ -600 600]);
xlabel('Time (ms)')
ylabel('dF/F')
title(['Average event- all cells- ' num2str(total_cells) ' cells'])
...
errorbar(tth, nanmean(RS_spont_renorm(cell_use,:),1), std(RS_spont_renorm(cell_use,:),[],1)./sqrt(length(cell_use)), '-m')
hold on;
errorbar(tth, nanmean(RS_success_renorm(cell_use,:),1), std(RS_success_renorm(cell_use,:),[],1)./sqrt(length(cell_use)), '-k')
hold on;
errorbar(tth, nanmean(RS_press_renorm(cell_use,:),1), std(RS_press_renorm(cell_use,:),[],1)./sqrt(length(cell_use)), '-c')
hold on;
errorbar(tth, nanmean(RS_fail_renorm(cell_use,:),1), std(RS_fail_renorm(cell_use,:),[],1)./sqrt(length(cell_use)), '-r')
xlim([ -600 600]);
xlabel('Time (ms)')
ylabel('dF/F')
title(['Average event- responsive cells- ' num2str(total_resp) ' cells'])
