...
for session_num = [[1,2], [5:length(days)]]
    ...
    %get the diff of peak df/f for correct - early 
    ...
    if exist([early_TC_dir, days{session_num}, '.mat']) ~= 0
        ...
        if size(lick_TCs,1) > 3 & size(no_lick_TCs,1) > 3
            ...
            peak_lick = max(lick_TCs(:,:,peak_window),[],3);
            peak_no_lick = max(no_lick_TCs(:,:,peak_window),[],3);
            peak_lick_mean = mean(peak_lick);
...
percent_early_with_lick_mat = [];
figure; diag_line = [-1:10];
for session_num = find(~cellfun(@isempty, corr_early_diff));
    plot(corr_early_diff{session_num}, percent_early_with_lick{session_num}*100, strcat('o', colors{session_num}) ); hold on;
    peak_diff_mat = [peak_diff_mat, corr_early_diff{session_num}];
    percent_early_with_lick_mat = [percent_early_with_lick_mat, percent_early_with_lick{session_num}];
end
...
xlim([-0.02 0.2]);
ylim([0 100]);
savefig([output_dir, 'percent_earlies_with_lick_vs_diff_in_peak_f_corrects_minus_early']);
