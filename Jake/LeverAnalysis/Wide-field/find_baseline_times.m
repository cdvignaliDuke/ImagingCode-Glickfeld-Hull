...
if isempty(cell2mat(b_data.leverTimesUs(40:50)));  %if no lever event occurs durign that trial   i.e. REWARD ONLY CONDITION
    for iT = 1:nTrials-1;
        baseline_times(1,iT) = round(b_data.tStartTrialWaitForPressTimeMs{iT}-1200);  %in reward only conditions the window for F is the last 700ms of the iti
        baseline_times(2,iT) = round(b_data.tStartTrialWaitForPressTimeMs{iT}-500);
    end
else
