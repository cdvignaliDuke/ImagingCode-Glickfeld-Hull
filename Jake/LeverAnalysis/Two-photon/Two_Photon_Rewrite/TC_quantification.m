...
save([dest_sub '_cell_TCs.mat'], 'avg_release', 'avg_press','avg_success', 'avg_tooFast', 'avg_fail', 'sem_release', 'sem_press','sem_success', 'sem_tooFast', 'sem_fail')
...
poss_rel_win_vals = []; 
for i = pre_release_frames+1:pre_release_frames+1+round(100./double(ifi))
    poss_rel_win_vals = [poss_rel_win_vals; mean(mean(mean(release_movie(:,:,i-1:i+1)),3))];
    poss_rel_wins = [poss_rel_wins, i];
end
resp_release_window = poss_fail_wins(find(poss_rel_win_vals == max(poss_rel_win_vals))); %selects window with the peak response 
...
save([dest_sub '_cell_resp.mat'], 'press_base', 'press_resp', 'success_base', 'success_resp', 'tooFast_resp', 'tooFast_base', 'fail_base', 'fail_resp', 'release_resp', 'release_base');
...
save([dest_sub '_cell_categories.mat'], 'release_resp_cells', 'success_resp_cells', 'tooFast_resp_cells', 'fail_resp_cells', 'press_resp_cells','noresponse_cells', 'fail_only_cells', 'success_only_cells', 'fail_and_success_cells', 'press_and_success_cells', 'press_notsuccess_cells', 'success_notpress_cells','trans_cell_ind', 'sustain_cell_ind');
...
ylabel('dF/F')
xlabel('Time (ms)')
supertitle([date ' ' mouse ' Average release: Success- black; Failure- red'])
