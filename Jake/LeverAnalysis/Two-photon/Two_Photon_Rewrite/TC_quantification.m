...
save([dest_sub '_cell_TCs.mat'], 'avg_release', 'avg_press','avg_success', 'avg_tooFast', 'avg_OR', ...
...
% end
if ~isempty(lick_data)
    base_lick_window = 1:pre_release_frames-floor(300/double(ifi));
    poss_licks_wins = [];  %using a sliding window to determine peak response to lever release
    poss_licks_win_vals = [];
    for i = pre_release_frames-round(300./double(ifi)):pre_release_frames+1+round(300./double(ifi))
    ...
    resp_licks_window = [resp_licks_window-1, resp_licks_window, resp_licks_window+1];
    ...
    resp_lickb_window = [resp_lickb_window-1, resp_lickb_window, resp_lickb_window+1];
...
%     if ~isempty(omitReward_movie) || ~isempty(itiReward_movie)
        nlickF = 3;
        tlick = ((-pre_release_frames:nlickF:post_release_frames + 1.5).*double(ifi));
        [success_lick_freq, sem_success_lick] = binLicks(success_lick, ifi);
        
        tlick2 = ((-pre_release_frames2:nlickF:post_release_frames2 + 1.5).*double(ifi));
...
save([dest_sub '_cell_resp.mat'], 'press_base', 'press_resp', 'success_base', 'success_resp', 'tooFast_resp', 'tooFast_base', 'fail_base', ...
    'fail_resp', 'release_resp', 'release_base', 'lickb_resp', 'lickb_base', 'licks_resp', 'licks_base', 'OR_base', 'OR_resp','IR_base', 'IR_resp',...
...
save([dest_sub '_dff_lick.mat'], 'success_lick_corr', 'fail_lick_corr', 'itiR_lick_corr', 'omitR_lick_corr');
...
%     lick_cells = unique([lickb_resp_cells licks_resp_cells]);
%     notlick_cells = find(~or(lickb_h, licks_h));
...
% 
% save([dest_sub '_cell_categories.mat'], 'release_resp_cells', 'success_resp_cells', 'tooFast_resp_cells', 'fail_resp_cells', 'press_resp_cells',...
...
%     'success_notpress_cells','lickb_resp_cells', 'licks_resp_cells', 'trans_cell_ind', 'sustain_cell_ind', 'lick_cells', 'success_notlick_cells', ...
%     'lick_notsuccess_cells', 'lick_success_cells', 'licks_success_cells', 'lickb_success_cells', 'OR_resp_cells', 'IR_resp_cells', ...
%     'notlick_cells', 'notlick_rel_cells');
% 
