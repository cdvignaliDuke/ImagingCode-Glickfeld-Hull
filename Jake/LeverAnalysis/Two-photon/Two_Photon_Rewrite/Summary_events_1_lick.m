...
normalR_nevents = 0; spont_nevents = 0;
...
%             
            normalR_hist{id} = all_success_hist.*(1000/double(ifi));
            omitR_hist{id} = all_omitR_hist.*(1000/double(ifi));
...
for id = [12 14 16]
    ...
    for rID  = 1:2
        ...
        if exist(dest_sub)
            ...
            normalR_hist_P1{id} = all_success_hist.*(1000/double(ifi));
            itiR_hist_P1{id} = all_itiR_hist.*(1000/double(ifi));
            normalR_hist_RS_P1{id} = all_success_hist(:,RS_cells_P1{id}).*(1000/double(ifi));
...
fig=figure;
subplot(2,2,1)
shadedErrorBar(tth, nanmean(NR_hist_D1,1), nanstd(NR_hist_D1,[],1)./sqrt(size(NR_hist_D1,1)), 'k');
hold on;
...
subplot(2,2,3)
shadedErrorBar(tth, nanmean(NR_hist_PN1,1), nanstd(NR_hist_PN1,[],1)./sqrt(size(NR_hist_PN1,1)), 'k');
hold on;
...
xlabel('Time (ms)')
ylabel('Firing rate (Hz)')
title('Day N with Unexpected Reward');
...
xlabel('Time (ms)')
ylabel('Firing rate (Hz)')
title('Day N with 1000ms delay');

supertitle(['PSTH- All cells- Black: Reward; Red: Omission; Green: Unexpected'])
...
fig=figure;
subplot(2,2,1)
shadedErrorBar(tth, nanmean(NR_hist_NL_D1,1), nanstd(NR_hist_NL_D1,[],1)./sqrt(size(NR_hist_NL_D1,1)), 'k');
hold on;
...
subplot(2,2,2)
shadedErrorBar(tth, nanmean(NR_hist_NL_P1,1), nanstd(NR_hist_NL_P1,[],1)./sqrt(size(NR_hist_NL_P1,1)), 'k');
hold on;
...
subplot(2,2,3)
shadedErrorBar(tth, nanmean(NR_hist_NL_PN1,1), nanstd(NR_hist_NL_PN1,[],1)./sqrt(size(NR_hist_NL_PN1,1)), 'k');
hold on;
...
subplot(2,2,4)
shadedErrorBar(tth, nanmean(NR_hist_NL_PN2,1), nanstd(NR_hist_NL_PN2,[],1)./sqrt(size(NR_hist_NL_PN2,1)), 'k');
hold on;
...
supertitle(['PSTH- All cells without lickbout- Black: Reward; Red: Omission; Green: Unexpected'])
...
supertitle(['PSTH- responsive cells without lickbout- Black: Reward; Red: Omission; Green: Unexpected'])
