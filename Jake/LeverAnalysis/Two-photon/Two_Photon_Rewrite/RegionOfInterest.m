classdef (CaseInsensitiveProperties = true) RegionOfInterest < hgsetget
	...
	properties
		...
		MinSufficientOverlap = .0001				% Use in sufficientlySimilar() and reduceSuperRegion() methods as a threshold to designate ROIs to merge
		...
		GroupingSimilarityMin = .65				%  Used in findGroups() via reduceRegions() method as an initial threshold for clustering
        ...
        MaxCentroidSeparation = 20				% Increase to cluster super-regions by centroid proximity more leniently
	end
				...
				case 3 % RIGHT-BUTTON: PLOT MULTIPLE TRACES
					for k = 1:numel(obj)
						if any(bsxfun(@eq,obj(k).PixelSubScripts, clickPoint))
							...
							hLine(numel(hLine)+1) = handle(line((1:numel(obj(k).Trace))./fps, obj(k).Trace,...
								'Color',obj(k).Color, 'Parent',hAx,'LineWidth',1));
							% 				  plot((1:numel(obj(k).Trace))./fps, obj(k).Trace,...
							% 					 'Color',obj(k).Color, 'Parent',hAx);
							% Add Text
