function CellsortICAplot_2P(mode, ica_filters, ica_sig, f0, tlims, dt, ratebin, plottype, ICuse, spt, spc)
...
%                     axis image tight off
    ...
    case {'contour'}
        ...
        for j=1:length(ICuse)
            ...
            text(xpeak,ypeak,num2str(ICuse(j)), 'horizontalalignment','c','verticalalignment','m','color','y')
        end
        hold off
        caxis(cax)
        formataxes
        ...
        set(gcf,'Color','w','PaperPositionMode','auto')
        set(gca,'yticklabel',num2str(fliplr(ICuse)'))
