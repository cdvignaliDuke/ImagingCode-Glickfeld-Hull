function [ica_segments, segmentlabel, segcentroid] = CellsortSegmentation_2P(ica_filters, smwidth, thresh, arealims, plotting, weight)
...
for j = 1:nic
    ...
    ica_filtersbw(:,:,j) = imclearborder(ica_filtersbw(:,:,j));
    L = bwlabel(ica_filtersbw(:,:,j), 8);
    Lu = 1:max(L(:));
    ...
    for jj = 1:length(Lbig)
        if weight ~= 0
            ...
            ica_segments(jj+k,:,:) = ( 0*(L==0) + (L==Lbig(jj)) );  % Exclude background
        end
    end
