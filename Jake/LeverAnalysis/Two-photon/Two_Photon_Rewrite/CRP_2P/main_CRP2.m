...
for session_subset = 1:4
    ...
    for sub = [1:15]%[1:15] %  %[2, 4, 5, 8, 9, 10, 12, 15]     %[1:2, 4:(length(stable_int)-2)]; %[2,5, 9, 22, 23, 31, 3, 6, 10, 25, 27, 32]
        if sub > length(days_subset) | isempty(days_subset{sub})
            continue
        end
        %collect session/mouse information   %set pathnames
        [out_dir, ~, ~, data_dir, session, session_date, mouse_num, mouse_ID, rID] ...
            = get_sess_info(days_subset{sub}, runID, crp_dir, []);
%         if ~exist(out_dir)
%             mkdir(out_dir);
%         end
...
%         end
        ...
        save([out_dir, 'img_reg.mat'], 'img_reg', 'reg_out', 'img_ref', 'laser_on_ind_conserv', 'img_nframes', '-v7.3'); 
        %end
        % %            continue
%         
            if doRerun == 0 
...
%                    continue
                   ...
                   %Dtermines which PCs to use from the data manually entered into file_info_CRP_all
%                    if this_nPCA == 1
%                        PCuse = all_PCuse{sub};
%                    elseif this_nPCA ==2
...
%              
            ...
            dest = out_dir;
             ...
             getTC_events_CRP;
              resp_cell_criteria_CRP; % extracts
              plot_session_TCs;
%          
...
disp('number of exluded cells over number of total cells per session')
ex_cell_all
