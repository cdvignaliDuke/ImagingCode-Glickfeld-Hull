...
for ie = 1:size(all_success_syn,3)
    spike_per_event = all_success_syn(1+sz1_half-round(100/ifi):1+sz1_half+round(100/ifi),:,ie);
    success_syn_all(ie) = calcEventStd_LG(spike_per_event)*ifi;
end

RS_success_syn = all_success_syn(:,RS_cells,:);
success_syn_RS = zeros(1,sz_s);
for ie = 1:size(all_success_syn,3)
    spike_per_event_f = RS_success_syn(sz1_half-round(100/ifi):sz1_half,:,ie);
    spike_per_event_l = RS_success_syn(sz1_half+2:sz1_half+2+round(100/ifi),:,ie);
...
for ic = 1:size(all_success_syn,2)
    spike_per_cell_f = squeeze(all_success_syn(sz1_half-round(100/ifi):sz1_half,ic,:));
    spike_per_cell_l = squeeze(all_success_syn(sz1_half+2:sz1_half+2+round(100/ifi),ic,:));
...
for ie = 1:size(all_fail_syn,3)
    spike_per_event_f = all_fail_syn(sz1_half-round(100/ifi):sz1_half,:,ie);
    spike_per_event_l = all_fail_syn(sz1_half+2:sz1_half+2+round(100/ifi),:,ie);
...
for ie = 1:size(all_fail_syn,3)
    spike_per_event_f = RS_fail_syn(sz1_half-round(100/ifi):sz1_half,:,ie);
    spike_per_event_l = RS_fail_syn(sz1_half+2:sz1_half+2+round(100/ifi),:,ie);
...
for ic = 1:size(all_fail_syn,2)
    spike_per_cell_f = squeeze(all_fail_syn(sz1_half-round(100/ifi):sz1_half,ic,:));
    spike_per_cell_l = squeeze(all_fail_syn(sz1_half+2:sz1_half+2+round(100/ifi),ic,:));
...
% %
save([dest_temp '_event_hist.mat'], 'success_ind_RS', 'success_rate_RS', 'fail_ind_RS', 'fail_rate_RS', 'success_ind_all', ...
...
save([dest_temp '_event_summary.mat'], 'data_start', 'data_end', 'press', 'release', 'success', 'fail', 'events', 'omitR', 'itiR');
...
% saveas(fig, [dest_sub '_avg_releaseVspontVpressVlick.fig']);
% print([dest_sub '_avg_releaseVspontVpressVlick.eps'], '-depsc');
% print([dest_sub '_avg_releaseVspontVpressVlick.pdf'], '-dpdf');
% 
% save([dest_sub '_norm2spont.mat'], 'release_norm_avg', 'spont_norm_avg', 'press_norm_avg', 'success_norm_avg', 'fail_norm_avg', 'lickb_norm_avg', 'licks_norm_avg', ...
