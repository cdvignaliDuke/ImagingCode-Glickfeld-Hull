...
%     lickStart = min(find((lickTimes(1) - frameTimes<0))) - 1;
    ...
    buffer_lick = round(500/double(ifi)); %changed from 500 ms (180528)
    buffer_press = round(500/double(ifi));
    buffer_fail = round(1000/double(ifi));
    buffer_correct = round(2500/double(ifi));
    licking_data.buffer_lick = buffer_lick;
    for i = 2:length(b_data.cTrialStart)-1
        % added below to account for trials with unexpected reward in ITI
        if ~isfield(b_data, 'tItiUnexpectedRewardTrial') || (isempty(b_data.tItiUnexpectedRewardTrial{i}) || ~b_data.tItiUnexpectedRewardTrial{i})
            %added below for additonal buffer after reward
%             if strcmp(b_data.trialOutcomeCell{i-1},'success')
%                 lick_itiTrial = licksByFrame(b_data.cTrialStart{i} + buffer_correct : b_data.cLeverDown{i} - buffer_press);
%             else
%                 lick_itiTrial = licksByFrame(b_data.cTrialStart{i} + buffer_fail : b_data.cLeverDown{i} - buffer_press);
%             end
            lick_itiTrial = licksByFrame(b_data.cTrialStart{i} + buffer_fail: b_data.cLeverDown{i} - buffer_press);
            %lick_itiTrial2 = licksByFrame(b_data.cTrialStart{i} : b_data.cTrialStart{i} + b_data.tItiWaitFrames{i});
            lick_idx = find(lick_itiTrial);
            %lick_idx2 = find(lick_itiTrial2); %removed 180528- using lick_idx

            for f = 1:length(lick_idx)
                single_idx = (lick_idx(f) + b_data.cTrialStart{i}-1+buffer_fail);
            ...
            if length(lick_idx) >= 3
                ...
                for li = 1:length(inter_lick)
                    ...
                    if length(inter_lick(li:end)) >= 3 && mean(inter_lick(li: li+2 )) <= round(300/double(ifi)) % each lick is less than 300ms apart
                        ...
                        onset_idx = lick_idx(li) + b_data.cTrialStart{i} -1 + buffer_fail;
    ...
    %requires minimum number of events to count for each experiment
    ntrial_cutoff = 3;
    if size(licking_data.bout_onsetTime,2)<ntrial_cutoff
        licking_data.bout_onsetTime = [];
    end
    if size(licking_data.bout_onsetTime,2)<ntrial_cutoff
        licking_data.single_lickTime = [];
    end
