...
%entire function editted by JH 3/18/16 to make flexible for old and new 2P
%lever data sets. 
 nframes = info.config.frames;
    ...
    %have an associated missed counter value. So I can insert counterValues and
    ...
    %associated long or short diff(counterTimesMs) value. I can use the
    %diff(counterValues) outliers to ID the diff(counterTimesMs) which I can
    %fix.
    ...
    %its typical value and I suspect these events could be fixed as well.
    insertionShift=0;
    ...
    % Time. I can use the location of these repeated counter values to remove
    ...
    %be and the following value is 0.5 times what it should be. I can easily
    %fix this so that the first value is the standard 1.5*ifi error and the
    %second value is equal to the ifi. This will more accurately track the real
    %frame times.
    ...
    %ifi pattern. I can fix these by reasssigning the firts 1.5*ifi value to a
