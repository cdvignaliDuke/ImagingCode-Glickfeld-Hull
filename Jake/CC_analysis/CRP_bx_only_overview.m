...
time_after_ms = 3000;
CRP_training_days_lists;  %contains information for allocating memory to variables and for which days will be analyzed
...
for ii = 1:length(days)
    ...
    if exist([session_fig_dir, days{ii}, '_rew_lick_raster.fig'], 'file')==0;
        ...
        title(['Rewarded Trials: lick time raster ', this_mouse, ' ' this_date ' n=' num2str(size(licks_by_trial_rewarded,1))]);
    ...
    if exist([session_fig_dir, days{ii}, '_block2_lick_raster.fig'], 'file')==0;
        ...
        title(['CS- Trials: lick time raster ', days{ii}(end-4:end), ' ' days{ii}(1:6) ' n=' num2str(size(licks_by_trial_block2,1))]);
    ...
    elseif b_data.RewardDelayDurationMs ==450 & b_data.rewardDelayPercent ==100;
        ...
        RT_this_session = RT_this_session(find(RT_this_session>200 & RT_this_session<1000));
    elseif TFT_ms > 0 & b_data.RewardDelayDurationMs == 0
        miss_rate_this_session = length(find(RT_this_session>1000))/(num_trials-1);
        RT_this_session = RT_this_session(find(RT_this_session>200 & RT_this_session<1000));
        if b_data.doBlock2 ==1
            miss_rate_this_session_b2 = (length(find(RT_this_sesssion_block2>1000))+no_lick_trials_b2) / num_trials_b2;
    ...
    if b_data.doBlock2 ==1
        ...
        RT_across_days_sem_b2 = [RT_across_days_sem_b2, std(RT_this_sesssion_block2)/sqrt(size(licks_by_trial_block2,1))];
    ...
    if b_data.doBlock2 ==1
        if exist([session_fig_dir, days{ii}, '_RT_plot_block2.fig'], 'file')==0;
            ...
            title(['RT for CS- trials within for day ' this_date, this_mouse]);
            xlabel('trial #');
            ylabel('RT (ms)');
...
save(['Y:\home\jake\Analysis\Cue_reward_pairing_analysis\CRPFigureFolder\across_animals_post_cue_lick\', days{ii}(end-4:end), 'rew_om_post_cue_lick'], ...
...
save(['Y:\home\jake\Analysis\Cue_reward_pairing_analysis\CRPFigureFolder\across_animals_RT_stats\', days{ii}(end-4:end), 'RTstd_FA_misses'], ...
...
if b_data.doBlock2 ==1 
    suptitle(['img', this_mouse, ': blue=CS+, black=CS-']);
else
    suptitle(['img', this_mouse]);
end
