...
PSPDur = input.postSoundPauseDurationMs;
scanOff = input.nScansOff;
scanOn = input.nScansOn;
RunData = NaN(1,[ntrial*(scanOn)]); %creates empty vector for data. Final destination of running wheel data
IntData = NaN(ntrial ,scanOn); %creates empty matrix to hold wheel pulse data

aa=1;
for b = 1:2:size(IntData,2);     %for loop to load wheel data into intdata from spCounters WARNING: starting valuee of b is hard coded for 9/9 data. rewrite. 
...
IntData = IntData(1:50,:);
ntrial = input.stopAfterNTrials;
...
%% Section2 cut away frames from nScansOff sections in imaging data
imdata = readtiff(pname);
imdata2 = NaN(size(imdata, 1), size(imdata, 2), [ntrial*(scanOn)]);
for a = 1:ntrial;
    imdata2(:,:,[1+(a-1)*(scanOn):(a*(scanOn))])=imdata(:,:,[1+(a*(scanOn+scanOff+PSPDur/500))-scanOn:(a*(scanOn+scanOff+PSPDur/500))]);    
end
size(imdata2)
...
nonRunIms = mean(nonRunIms(:,:,1:454),3);
