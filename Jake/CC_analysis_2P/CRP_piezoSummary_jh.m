clear all
close all
CRP_OT_expt_list_LS_jh  %toggle between Crus vs LS
...
for id = [4]
    ...
    %% extract and calculate relavant info from each session in this expt type
    for iexp = 1:nexp
        ...
        %load data
        load(fullfile(jh_out,img_fn, [img_fn '_targetAlign.mat']))
        if ~exist([jh_out,'\', img_fn, '\', [img_fn '_cueAlignPiezo.mat']]) 
            expt_areas(:,iexp) = NaN;
            fprintf([date ' ' mouse ' skipped due to no piezo data' '\n'])
            continue
        else 
            load(fullfile(jh_out,img_fn, [img_fn '_cueAlignPiezo.mat']))
            fprintf([date ' ' mouse '\n'])
        end 
        if exist(fullfile(jh_out,img_fn, [img_fn '_splitImage.mat']))
            load(fullfile(jh_out,img_fn, [img_fn '_splitImage.mat']))
            area_temp = zeros(1,size(targetAligndFoverF,2));
            for i = 1:2
                x = find(strcmp(area_list, expt(id).areas{iexp}(i))); %x is the numerical ID for the currect lobule i.e. 1:3 for LS, C1, C2
                ind = find(maskCat==i);
                area_temp(1,ind) = x;
                expt_areas(x,iexp) = 1;
            end
        ...
        %area id for each neuron
        all_area_id = [all_area_id area_temp];
        
        %calculate some variables for plotting and indeces
        frameRateHz = 30;
        rewdelay_frames = round(0.6.*frameRateHz);   %HARDCODED
        ...
        %store data for across days analyis. 
    ...
    expt_areas(:,[find(isnan(expt_areas(1,:)))]) = []; %get rid of expts with no piezo data
    totIC = size(all_area_id,2);
    totExp = sum(expt_areas,2);
    all_HL_piezo = concatenateStructuresLG(all_HL_piezo); %will remove expts with no piezo data from the structure
    ...
    for i = find(sum(expt_areas,2)')
        ...
        figure;
        shadedErrorBar(tt, nanmean(all_rew_piezo(:,find(expt_areas(i,:))),2), (nanstd(all_rew_piezo(:,find(expt_areas(i,:))),[],2)./sqrt(sum(expt_areas(i,:),2))),   'k');
        hold on
        shadedErrorBar(tt, nanmean(all_omit_piezo(:,find(expt_areas(i,:))),2), (nanstd(all_omit_piezo(:,find(expt_areas(i,:))),[],2)./sqrt(sum(expt_areas(i,:),2))),   'r');
