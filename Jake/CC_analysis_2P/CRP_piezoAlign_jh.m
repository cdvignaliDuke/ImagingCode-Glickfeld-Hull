...
for id = [1:5]; 
    ...
    %threshold in Hz for the highpass filter applied to the piezo data
    hpass_thresh = 1; 
    ...
    for iexp = 1:nexp
        ...
        %format piezo data so it is useable
        ...
        if exist(fullfile(lg_out,img_fn, [img_fn '_piezo_frames.mat']))
            load(fullfile(lg_out,img_fn, [img_fn '_piezo_frames.mat']))
        else
            ...
            save(fullfile(lg_out,img_fn, [img_fn '_piezo_frames.mat']), 'piezo_frames');
        end
        %apply a highpass filter to the piezo data - this removes drifting baselines and many outlier events
        piezo_frames = highpass(piezo_frames, hpass_thresh, frameRateHz);
        
        %define some useful variables for plotting
        rewDelay_frames =  round(0.6.*frameRateHz); %HARDCODED REWARD DELAY DURATION
