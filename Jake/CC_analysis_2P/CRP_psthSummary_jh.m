% huge script for plotting summary psths 
%need to run CRP_spikeAlign_jh and CRP_lickAlign_jh first? maybe others too
...
for id =[3]  %experiment type
    ...
    for iexp = 1:nexp  
        ...
        %load data and divide neurons by lobule
        load(fullfile(jh_out,img_fn, [img_fn '_targetAlign.mat']))
        load(fullfile(jh_out,img_fn, [img_fn '_cueAlignLick.mat']))
        load(fullfile(jh_out,img_fn, [img_fn '_lickResp_preCue.mat']))
        ...
        %optional - plot traces for 1st 50 neurons from each lobule plus
        %their mean. 
%         figure;
...
%         savefig([share_out '\CC_examples\Day' num2str(id) '_' mouse '_' date '_allCells.fig']) 
        ...
        %save the experiment ID of the expts used in this analysis
        save(fullfile(share_out, 'CC_summary', 'expNums.mat'),'expNums');
        
        %calculate more averages
        ...
        %bin trials according to time of first lick post reward
        ...
        %for each lobule, plot some averages for each animal
...
%     end
%     save(fullfile(jh_out,'CC_Summary',['Day' num2str(id)],['Day' num2str(id) '_PeakSpikingResp_Windows.mat']),'preresp_rew_range','postresp_rew_range');
%     
%     thresh = 4;
%     for i = find(sum(expt_areas,2)')
%         ind = find(all_area_id==i);
...
%     end
% 
%     save([share_out, '\Jake\CC_summary\Day' num2str(id) '\Day' num2str(id) '_allArea_Data.mat'], 'all_rew', 'all_omit', 'all_unexp', 'expt_areas', 'all_area_id', 'preresp_rew_range', 'postresp_rew_range', 'base_rew_range', 'prewin_frames', 'rewdelay_frames')
% 
%        
%         
