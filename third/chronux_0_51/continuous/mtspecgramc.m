function [S,t,f,Serr,phase]=mtspecgramc(data,movingwin,params)
...
for n=1:nw;
   ...
   datawin=data(indx,:);
   if nargout>=5
     [s,f,phaseC{n},serr]=mtspectrumc(datawin,params);
     Serr(1,n,:,:)=squeeze(serr(1,:,:));
     Serr(2,n,:,:)=squeeze(serr(2,:,:));
   else
     [s,f,phaseC{n}]=mtspectrumc(datawin,params);
   end
   S{n}=s;
end;
S=permute(cat(3,S{:}), [3 1 2]); 
phase=permute(cat(3,phaseC{:}), [3 1 2]);
