...
for k=1:length(ReallyPossibleSpecies),
    indexspecies(end+1) = strmatch(ReallyPossibleSpecies{k},{SBMLmodel.species.id},'exact');
end
for k=1:length(ReallyPossibleParameters),
    indexparameters(end+1) = strmatch(ReallyPossibleParameters{k},{SBMLmodel.parameter.id},'exact');
end
for k=1:length(ReallyPossibleCompartments),
    indexcompartments(end+1) = strmatch(ReallyPossibleCompartments{k},{SBMLmodel.compartment.id},'exact');
end
