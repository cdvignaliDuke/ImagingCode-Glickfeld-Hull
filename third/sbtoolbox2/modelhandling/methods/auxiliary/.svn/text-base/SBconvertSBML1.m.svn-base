...
% HANDLE TIME VARYING COMPARTMENT SIZES
    ...
    if changingFlag,
        ...
        % (if not done already)
        if isempty(strmatch(compartmentName,compartmentddtdonelist,'exact')),
...
ARindices = strmatch('SBML_ALGEBRAIC_RULE',{SBMLmodel.rule.typecode},'exact');
...
for k=1:length(ReallyPossibleSpecies),
    indexspecies(end+1) = strmatch(ReallyPossibleSpecies{k},{SBMLmodel.species.name},'exact');
end
for k=1:length(ReallyPossibleParameters),
    indexparameters(end+1) = strmatch(ReallyPossibleParameters{k},{SBMLmodel.parameter.name},'exact');
end
for k=1:length(ReallyPossibleCompartments),
    indexcompartments(end+1) = strmatch(ReallyPossibleCompartments{k},{SBMLmodel.compartment.name},'exact');
end
...
for k=1:length(ARindices),
    ...
    indexstate = strmatch(name,allStates,'exact');
    indexparam = strmatch(name,allParams,'exact');
    indexvar = strmatch(name,allVars,'exact');
...
indexState = strmatch(compartmentName,{SBstructure.states.name},'exact');
indexVariable = strmatch(compartmentName,{SBstructure.variables.name},'exact');
indexAlgebraic = strmatch(compartmentName,{SBstructure.algebraic.name},'exact');
if ~isempty(indexState),
    changingFlag = 1; 
