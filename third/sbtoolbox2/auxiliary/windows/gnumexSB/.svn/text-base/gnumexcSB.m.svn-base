function varargout = gnumexcSB(directories)
% gnumexcSB: an adapted version of the gnumex package
% (https://sourceforge.net/projects/gnumex). It allows everyone to build
% C MEX files under Windows using the freely available MinGW 
% compiler (http://www.mingw.org/). 
%
% gnumexcSB constructs a C/C++ MEX options file for windows that is used by the
% MEX function to determine the compiler, linker, options, etc.
% 
% This file does not need to be run by the user. Instead have a
% look at the mexcSB function included in this folder.
...
optfile = 'gccopt.bat'; % create options file in current folder
gnumexpath = shortpath(getPath('gnumexcSB.m','gnumexSB'));
matlabpath = shortpath(matlabroot);

% write out the options file
fid = fopen(optfile,'w');
fprintf(fid,'@echo off\n');
fprintf(fid,'rem gccopt.bat\n');
fprintf(fid,'rem\n');
fprintf(fid,'rem    Compile and link options used for building C MEX-files\n');
...
fprintf(fid,'set GM_MEXLANG=c\n');
fprintf(fid,'set COMPILER=gcc\n');
fprintf(fid,'set COMPFLAGS=-c -DMATLAB_MEX_FILE -mrtd -fleading-underscore -w \n'); % disable warnings
...
fprintf(fid,'set CPPCOMPFLAGS=%%COMPFLAGS%% -x c++ \n');
fprintf(fid,'set CPPOPTIMFLAGS=%%OPTIMFLAGS%%\n');
fprintf(fid,'set CPPDEBUGFLAGS=%%DEBUGFLAGS%%\n');
fprintf(fid,'\n');
fprintf(fid,'rem ********************************************************************\n');
...
fprintf(fid,'set CPPLINKFLAGS=GM_ISCPP\n');
