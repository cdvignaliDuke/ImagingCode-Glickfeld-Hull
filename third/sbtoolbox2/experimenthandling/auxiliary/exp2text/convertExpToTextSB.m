...
for k = 1:length(SBstructure.paramicsettings),
    ...
    if SBstructure.paramicsettings(k).icflag == 1,
...
for k = 1:length(listofstatesic),
    expTextStructure.paramicsettings = regexprep(expTextStructure.paramicsettings,strcat('\<',listofstatesic{k},'\>'),strcat(listofstatesic{k},'(0)'));
end
