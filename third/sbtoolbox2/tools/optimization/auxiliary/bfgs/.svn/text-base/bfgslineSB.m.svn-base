...
% apprGradient: mode in which the program operates, =1 if GRAD=@apprGradSB
...
% Information:
% ============
% Author: Mats Jirstrand, matsj@fcc.chalmers.se
...
while(1)
    ...
    if phiVal_new > phi0 + c1*a_new*phiGrad0'*p || (i>1 && phiVal_new >= phiVal_old)
        stepLength = zoom(a_old,phiVal_old,a_new,FUN,GRAD,X,p,TOL,apprGradient,phi0,phiGrad0,c1,c2);
        if stepLength == inf
            break;
        end        
    ...
    if phiGrad_new'*p >= 0
        stepLength = zoom(a_new,phiVal_new,a_old,FUN,GRAD,X,p,TOL,apprGradient,phi0,phiGrad0,c1,c2);
        if stepLength == inf
            break;
        end
        % displaying performance
...
% ZOOM FUNCTION
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [alpha] = zoom(a_lo,VALa_lo,a_hi,FUN,GRAD,X,p,TOL,apprGradient,phi0,phiGrad0,c1,c2)
