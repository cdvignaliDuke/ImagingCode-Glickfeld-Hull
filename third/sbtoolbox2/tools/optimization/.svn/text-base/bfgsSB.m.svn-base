function [X,FVAL] = bfgsSB(varargin)
...
%          OPTIONS.xtol: Convergence tolerance
...
%          OPTIONS.maxfunevals: Maximum number of function and gradient
%               evaluations
%          OPTIONS.silent: Displaying information: 0 = on, 1 = off
% 
% DEFAULT VALUES:
% ===============
...
% OPTIONS.const = 1.5
% OPTIONS.aMax  = 500
% OPTIONS.maxfunevals = 500*numberVariables
% OPTIONS.silent = 1
%
% Output Arguments:
% =================
...
% GLOBAL VARIABLES
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
global NoLineS NoZoom FunEval GradEval subopt display stopOptimization
...
if nargin == 0,
    ...
    X.name = 'bfgsSB';
    X.constrained = 0;
    X.description = 'Quasi-Newton algorithm BFGS method (local)';                      
    X.defaultOptions.names = {'maxfunevals', 'tolx'};
    X.defaultOptions.values = {'10000','1e-10'};
    X.defaultOptions.description = {'Maximum number of function evaluations', 'Termination tolerance on X'};  
    FVAL = [];
    EXITFLAG = [];
    return
elseif nargin == 2,
...
if ~isempty(OPTIONS)
    ...
    if isfield(OPTIONS,'xtol'),
        if ~isempty(OPTIONS.xtol), xtol = OPTIONS.xtol; end
    end
    ...
    if isfield(OPTIONS,'maxfunevals'),
        if ~isempty(OPTIONS.maxfunevals), maxfunevals = OPTIONS.maxfunevals; end
    end
    if isfield(OPTIONS,'silent'),
        if ~isempty(OPTIONS.silent), silent = OPTIONS.silent; end
    end
end
display = ~silent;
...
FVAL = feval(FUN,X);
if apprGradSBient
    grad = feval(GRAD,x_old,FUN,xtol);
else
...
while norm(grad) > xtol
    % search direction
    p = -H*grad;
    % compute step length in lineSearch
    alpha = bfgslineSB(FUN,GRAD,x_old,FVAL,p,xtol,apprGradSBient,c1,c2,a1,const,aMax);
    if alpha == inf
        break;
    end
    ...
    x_new = x_old + alpha*p;
    if apprGradSBient
        grad_new = feval(GRAD,x_new,FUN,xtol);
    else
    ...
    if apprGradSBient
        if FunEval + 2*ndim*GradEval > maxfunevals
            disp('Exceeded maximum number of function evaluations.');
            break;
        end
    else
        if FunEval + GradEval > maxfunevals
