function [X,FVAL] = brentSB(varargin)
...
% ======
% [info] = brentSB()
% [X,FVAL] = brentSB(FUN,X)
% [X,FVAL] = brentSB(FUN,X,OPTIONS)
%
% FUN: Function to optimize
% X: Starting guess
% OPTIONS: Structure containing options for the algorithm.
%          OPTIONS.H: Estimate of the distance to the minimum of FUN
%          OPTIONS.tolx: Positive absolute tolerance
...
% OPTIONS.pos  = 0.001;
% OPTIONS.stop = 2
% OPTIONS.maxfunevals = 1000*numberVariables
...
% GLOBAL VARIABLES
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
global FunEval stopOptimization
...
if nargin == 0,
    ...
    X.name = 'brentSB';
    X.constrained = 0;
    X.description = 'Principal Axis Method for Minimization without Derivatives (local)';                  
...
if ~isempty(OPTIONS)
    if isfield(OPTIONS,'H'),
      if ~isempty(OPTIONS.H), H = OPTIONS.H; end
    end
    if isfield(OPTIONS,'tolx'),
      if ~isempty(OPTIONS.tolx), tolx = OPTIONS.tolx; end
    end
...
while(1)
    ...
    % approximation x_new
    if 2*norm(x0 - x_new) <= tolx    % 2*norm(x0 - x_new) <= eps^(0.5)*norm(x_new) + tolx
        STOP = STOP+1;
        if STOP >= stop
            X = x_new;
            if display
