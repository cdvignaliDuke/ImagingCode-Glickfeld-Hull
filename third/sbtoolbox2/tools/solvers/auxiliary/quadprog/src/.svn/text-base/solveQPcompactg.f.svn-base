...
C MEX INTERFACE FOR THE solveQPcompact FUNCTION
C CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C
C Calling from MATLAB:
C [sol,soluc,crval,iact,nact,iter,ierr] = solveQPcompact(Dmat,dvec,n,n,Amat,Aind,bvec,anrow,q,meq,factorized)
C
...
C CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      ...
      integer Dmat_pr, dvec_pr, n1_pr, n2_pr, Amat_pr, Aind_pr
      integer bvec_pr, anrow_pr, q_pr, meq_pr, factorized_pr

      integer n1, n2, anrow, q, meq, factorized
      real*8 n1r, n2r, anrowr, qr, meqr, factorizedr
...
C     Get pointers
      ...
      Aind_pr = mxGetPr(prhs(6))
      bvec_pr = mxGetPr(prhs(7))
      anrow_pr = mxGetPr(prhs(8))
      q_pr = mxGetPr(prhs(9))
      meq_pr = mxGetPr(prhs(10))
      factorized_pr = mxGetPr(prhs(11))
C     Get integers
      call mxCopyPtrToReal8(n1_pr, n1r, 1)
      n1 = INT(n1r)
      call mxCopyPtrToReal8(n2_pr, n2r, 1)
      n2 = INT(n2r)
      call mxCopyPtrToReal8(anrow_pr, anrowr, 1)
      anrow = INT(anrowr)
...
C CCCCCCCCCCCCCCCCCCC
      call qpgen1(%VAL(Dmat_pr), %VAL(dvec_pr), n1, n2, %VAL(sol_pr),
     *     crvalr, %VAL(Amat_pr), %VAL(Aind_pr),
     *     %VAL(bvec_pr), anrow, q, meq, %VAL(iact_pr), nact, 
