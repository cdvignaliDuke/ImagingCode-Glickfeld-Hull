...
c
     ...
     *     r, iwnbv, meq, fddmat
      REAL*8 dmat(fddmat,*), dvec(*),sol(*), bvec(*),
     *     work(*), temp, sum, t1, tt, gc, gs, crval,
     *     nu, amat(fdamat,*), iter(*), iact(*), iamat(fdamat+1,*)
      logical t1inf, t2min
      r = min(n,q)
      l = 2*n + (r*(r+5))/2 + 2*q + 1
c 
...
c
      ...
      do 60 i=1,q
         l = l+1
         sum = -bvec(i)
         do 61 j = 1,INT(iamat(1,i))
            sum = sum + amat(j,i)*sol(INT(iamat(j+1,i)))
 61      continue
         if (i .GT. meq) then
            work(l) = sum
         else
...
c
 55   continue
      do 80 i=1,n
         sum = 0.d0
         do 81 j=1,INT(iamat(1,nvl))
            sum = sum + dmat(INT(iamat(j+1,nvl)),i)*amat(j,nvl)
 81      continue
         work(i) = sum
 80   continue
c
...
c
         sum = 0.d0
         do 120 i = 1,INT(iamat(1,nvl))
            sum = sum + work(iwzv+INT(iamat(i+1,nvl)))*amat(i,nvl)
 120     continue
...
c
            sum = -bvec(nvl)
            do 190 j = 1,INT(iamat(1,nvl))
               sum = sum + sol(INT(iamat(j+1,nvl)))*amat(j,nvl)
 190        continue
            if( nvl .GT. meq ) then
               work(iwsv+nvl) = sum
            else
