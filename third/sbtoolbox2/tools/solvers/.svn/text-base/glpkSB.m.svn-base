function [xopt,fmin,status,extra] = glpkSB (c,a,b,lb,ub,ctype,vartype,sense,param)
% Matlab MEX interface for the GLPK library
%
% [xopt, fmin, status, extra] = glpkSB (c, a, b, lb, ub, ctype, vartype, sense, param)
%
...
%
%           mpsinfo (LPX_K_MPSINFO, default: 1) If this is set, 
...
%
%           mpsobj (LPX_K_MPSOBJ, default: 2) This parameter tells the 
...
%                        no free rows 
%           mpsorig (LPX_K_MPSORIG, default: 0) If this is set, the 
...
%
%           mpswide (LPX_K_MPSWIDE, default: 1) If this is set, the 
%                   routine uses all data fields. Otherwise the routine 
%                   keeps fields 5 and 6 empty. 
%
%           mpsfree (LPX_K_MPSFREE, default: 0) If this is set, the routine 
...
if (nargin < 3 || nargin > 9)
    disp('GLPK Matlab interface. Version: 2.0');
