...
nameStart = regexp(contents,'\[Name\]');
notesStart = regexp(contents,'\[Notes\]');
componentsStart = regexp(contents,'\[Components\]');
componentnotesStart = regexp(contents,'\[Componentnotes\]');
valuesStart = regexp(contents,'\[Values\]');
...
nameraw = strtrim(contents(nameStart+length('[Name]'):notesStart-1));
notesraw = strtrim(contents(notesStart+length('[Notes]'):componentsStart-1));
componentsraw = strtrim(contents(componentsStart+length('[Components]'):componentnotesStart-1));
componentnotesraw = strtrim(contents(componentnotesStart+length('[Componentnotes]'):valuesStart-1));
valuesraw = strtrim(contents(valuesStart+length('[Values]'):end));
...
% find 'time' component
timeindex = strmatch('time',components,'exact');
...
for k = 1:length(componentnotesall),
    ...
    index = strmatch(componentname,{componentdata.name},'exact');
    if isempty(index),
...
for k=1:length(errorbounddata),
    indexcomponent = strmatch(errorbounddata(k).name,{componentdata.name},'exact');
