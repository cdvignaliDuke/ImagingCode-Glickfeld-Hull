...
if loadFlag
    ...
    fprintf(['2P imaging analysis - by KM, Glickfeld Lab\nSelected data:\nMouse: ' mouse '\nDate: ' date '\nExperiments:\n'])
    for irun=1:nrun
...
% this fits size tuning curves to log transformed size data
figure(2);clf;
for i = 1:nCutCells
    iCell = goodCutCells(i);
    fprintf(['\nCell# ' num2str(iCell)])
    ...
    for iCon = 1:nCon
        ...
        lb = [0 0];
        ub = [inf 10]; %[inf inf]; % here limit steepness to 1 (above this is way too steep)
        [fitx,resnorm1] = lsqcurvefit(logfit1,guess,log(szs0),dumdum,lb,ub,opts);
        ...
        fitout1 = logfit1(fitx,log(szRng));
        ...
        lb = [0 0 0 0];
        ub = [inf 10 inf 10]; %[inf inf inf inf]; % here limit steepness to 1 (above this is way too steep)
        [fitx,resnorm2] = lsqcurvefit(logfit2,guess,log(szs0),dumdum,lb,ub,opts);
        ...
        fitout2 = logfit2(fitx,log(szRng));
        ...
        if 1%iCell == chosen
            subplot(2,nCon,iCon)
            %errorbar([0 log(szs)],[0 sizeMean(:,iCon,i)'],[0 sizeSEM(:,iCon,i)'])
            errorbar([0 szs],[0 sizeMean(:,iCon,i)'],[0 sizeSEM(:,iCon,i)'])
            hold on
            %plot(log(szs0),dumdum,'.b')
            %plot(log(szRng),fitout1,'-')
            plot(szs0,dumdum,'.b')
            plot(szRng,fitout1,'-')
            hold off
            ylim([min([-0.5*maxResp1 min(sizeMean(:,iCon,i))]) 1.4*max([maxResp2 max(sizeMean(:,iCon,i))])])
            title(['Cell #' num2str(iCell) ', Con ' num2str(cons(iCon)) ', R^2=' num2str(allRsq1(i,iCon))]) % ', SI=' num2str(suppInd1)
            %xlabel('log(Stimulus Size)')
            xlabel('Stimulus Size')
            ylabel('dF/F')
            legend('data','fit')
            
            subplot(2,nCon,iCon+3)
            %errorbar([0 log(szs)],[0 sizeMean(:,iCon,i)'],[0 sizeSEM(:,iCon,i)'])
            errorbar([0 szs],[0 sizeMean(:,iCon,i)'],[0 sizeSEM(:,iCon,i)'])
            hold on
            %plot(log(szs0),dumdum,'.b')
            %plot(log(szRng),fitout2,'-')
            plot(szs0,dumdum,'.b')
            plot(szRng,fitout2,'-')
            hold off
            ylim([min([-0.5*maxResp1 min(sizeMean(:,iCon,i))]) 1.4*max([maxResp2 max(sizeMean(:,iCon,i))])])
            title(['Cell #' num2str(iCell) ', Con ' num2str(cons(iCon)) ', R^2=' num2str(allRsq2(i,iCon))]) % ', SI=' num2str(suppInd2)
            %xlabel('log(Stimulus Size)')
            xlabel('Stimulus Size')
            ylabel('dF/F')
            legend('data','fit')
