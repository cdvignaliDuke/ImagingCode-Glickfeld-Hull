...
%
%  bwOut = imCellEditInteractive(fovImg, bwCell, zoomToAxis, cellRadius, zstack)
%
...
%==========================================================================
%
% <FILLSTACK> - A function to fill a stack with images
%
%    [ImStack] = fillstack (path, filename, format, images, dim, <colorDepth>)
%    ImStack   : 3D array containing the images
%    path      : String containing the path
%    filename  : String containing the filename without the numbers
%    format    : 'img'  - tvx .img format
...
%==========================================================================
%
[nRows nCols z] = size(fovImg);

if isempty(bwCell), bwCell = false([nRows,nCols z]); end
labCell = bwlabeln(bwCell,8); %CHANGE MA
...
%%Zstack Processing
%sliceImage = imread('fovimg');
sliceImage = squeeze(fovImg(:,:,1));
...
valueToLookFor = 88;
[nRows nCols] = find(sliceImage == valueToLookFor);
...
binarizedImage = (fovImg == valueToLookFor);
subplot(1,2,2);
...
if nargin < 3 || isempty(zoomToAxis), 
    ...
    zoomToAxis = [1 nCols 1 nRows 1 z] + 0.5+[-1 1 -1 1 -1 1];  
end
...
% draw initial figure
figH = figure;
imH = subDrawShaded(sliceImage, bwCell, zoomToAxis);
axH = get(imH, 'Parent');
set(axH, 'Position', [0.06 0.06 0.88 0.88 0.88 0.88])
...
tStr = { [mfilename ': Click on a cell region to add'], ...
         '       Shift-click to del, RET to finish, d to undo' };
...
while 1  % till broken out of
    ...
    [X Y Z selectionType] = getAPoint(gca);
    ...
    if isnan(Z)
        ...
        switch key
            ...
            case 'd'
                if nActions == 0
                    fprintf(1, '**Cannot undo: no Z cells added yet\n');
                    continue;
                end
        end
    end
    ...
    Y = round(Y);
    Z = round(Z);
    if X <= 0 || X >= nCols || Y <= 0 || Y >= nRows || Z <=0 || Z>=z   
    ...
    switch lower(selectionType)
      case 'extend'    % shift-left: delete
        ...
        %% do delete
        bwNew = subDeleteCell(bwCurr, X, Y, Z);
        nTotal = nTotal-1;
      ...
      case 'normal'    % left-click: add
        ...
        %% do add
        [bwNew bwCell] = subAddCell(bwCurr, fovImg, X, Y, Z, cellRadius, cThresh, diskR);
...
function bwNew = subDeleteCell(bwCurr, X, Y, Z)

tBwCell = bwselect(bwCurr, round(X), round(Y), round (Z));  % only 4-connected
                                                    % objs
bwNew = bwCurr & ~tBwCell;

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function [bwNew bwCell] = subAddCell(bwCurr, fovImg, X, Y, Z, cellRadius, cThresh, diskR);

%%% Real cell-adding logic is here

[nRows nCols z] = size(bwCurr);
...
seZ = strel('column',round(cellRadius), 4); %Z column projection...
...
connMeanBw = bwselect(allMeanBw &~dilateorg, X, Y, Z, 4);
