function [transform_coeff inverse_transform_coeff list_pairs_out dist_list]=D3align_L_2(cells1, cells2, coordinate_pairs,list_pairs_in,fig_flag,tsh_dist,num_of_manual_pairs,analysis_flags)
...
if nargin<8 || isempty(analysis_flags)
...
if nargin<3 % no initial pairs
    ...
    if length(answer)>1
        ...
        close(h);
        list_pairs
        [transform_coeff inverse_transform_coeff list_pairs_out dist_list]=D3align_L_2(p_1, p_2, [],list_pairs,fig_flag,tsh_dist,num_of_manual_pairs,analysis_flags);
    end
    return
end
...
if ~isempty(coordinate_pairs) %coordiante pairs available
    ...
    if size(p_p_1,1)<num_of_manual_pairs
        display(['Requires initial pair list with more than ' num2str(num_of_manual_pairs) ' pairs']);
        ...
        [transform_coeff inverse_transform_coeff list_pairs_out dist_list]=D3align_L_2(p_1, p_2, new_coordinate_pairs,fig_flag,tsh_dist,num_of_manual_pairs,analysis_flags);
...
dtr=sqrt((a0-a1).^2 +(b0-b1).^2+(c0-c1).^2);
if dtr==0
    return
end
...
if size(list_pairs_out,1)>size(p_p_1,1)
    [transform_coeff inverse_transform_coeff list_pairs_out dist_list]=D3align_L_2(p_1, p_2, [],list_pairs_out,fig_flag,tsh_dist,num_of_manual_pairs,analysis_flags);
else
