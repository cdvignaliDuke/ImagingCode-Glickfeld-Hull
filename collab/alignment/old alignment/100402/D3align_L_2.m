function [transform_coeff inverse_transform_coeff list_pairs_out dist_list]=D3align_L_2(cells1, cells2, coordinate_pairs,list_pairs_in,fig_flag)
...
tsh_dist=6; %max distance between paired cells (in pixels)
...
%drawCells(p_1,p_2,transform_coeff);
    ...
    if length(answer)>1
        ...
        close(h);
        list_pairs
        [transform_coeff inverse_transform_coeff list_pairs_out dist_list]=D3align_L_2(p_1, p_2, [],list_pairs,fig_flag);
    end
    return
end
...
if ~isempty(coordinate_pairs) %coordiante pairs available
    ...
    if size(p_p_1,1)<num_of_manual_pairs
        ...
        [transform_coeff inverse_transform_coeff list_pairs_out dist_list]=D3align_L_2(p_1, p_2, new_coordinate_pairs,fig_flag);
...
if size(list_pairs_out,1)>size(p_p_1,1)
    [transform_coeff inverse_transform_coeff list_pairs_out dist_list]=D3align_L_2(p_1, p_2, [],list_pairs_out,fig_flag);
else
