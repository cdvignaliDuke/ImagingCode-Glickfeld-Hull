...
%tsh=0.8;
max_v=round(120/(resolution_in(1)*resolution_in(2)*resolution_in(3))); %empirical max bead volume
...
%figure;
m1=max(Red_im,[],1);
m2=max(m1,[],2); %max intensity for each frame
...
mmm=max(mm,[],2); %max for 5 frames
ma=max(Red_im(:)); %global max
mmm=max(mmm,ma*0.3); % normalazation max as lacal max, but not smaller then 0.3 of global max
ma3 = shiftdim(mmm,-2);
...
BW_BEST=Red_im.*0;
for tsh1=0.2:0.05:1
    for tsh2=tsh1:0.05:1
    im_bw=Red_im>tsh1*ma4;
    BW_L = bwareaopen(im_bw,8,6);
    BW_L_2 = bwareaopen(BW_L,max_v,6);
    BW_S= BW_L & ~BW_L_2;
    Red_im_2=Red_im.*BW_L_2;
    
    im_bw_2=Red_im_2>tsh2*ma4;
    BW_L_3 = bwareaopen(im_bw_2,8,6);
