...
Sft =zeros(5,100);
j=0;
for i=[1:size(tformed,2)];
    if abs(tformed(1,i)-A(1))<r && abs(tformed(2,i)-A(2))<r && abs(tformed(3,i)-A(3))<r
        j=j+1,  
        Sft(:,j)=cat(1,tformed(1:3,i),1,i); end;
end;

% Overlape the result on fixedtformed.fig.
hold on;
scatter3(Sft(1,1:j),Sft(2,1:j),Sft(3,1:j),50,'m','filled');
