...
% function SAVEPPT2(save_file,<additional parameters>) saves the current Matlab figure
...
%
% 'device' - ['meta','bitmap'] default is meta
%
% Examples: (Identical calling methods are listed below commends in
% comments.)
...
% plot(rand(1,100),rand(1,100),'*');
% saveppt2('test.ppt','notes','Hello again world');
%
% % scale the plot to fill the page, maintaining aspect ratio
% plot(rand(1,100),rand(1,100),'*');
% saveppt2('test.ppt','scale');
% % saveppt2('test.ppt','scale',true);
%
% % scale the plot to fill the page, ignoring aspect ratio
% plot(rand(1,100),rand(1,100),'*');
% saveppt2('test.ppt','scale','stretch');
% saveppt2('test.ppt','scale',true,'stretch',true);
% saveppt2 test.ppt -scale -stretch
%
...
%Ver 3.1, Vincent bonin
%  Added bitmap printer driver

function saveppt2(filespec,varargin)
...
%% Set up defaults
if checkParm (addlParms,'device');
    addlParms.device='meta';
end
...
if isfield(addlParms,'figure')
    ...
    if checkParm(addlParms,'figure')
        ...
        if length(addlParms.figure)>4
            warning('saveppt:TooManyFigures','>4 figures is not reccomended')
        end
    end
...
for i=1:fig.count
    ...
    if strcmp(addlParms.device,'meta')
        print('-dmeta',['-f' num2str(addlParms.figure(i))],rendOpt,resOpt);
    elseif strcmp(addlParms.device,'bitmap')
        print('-dbitmap',['-f' num2str(addlParms.figure(i))],rendOpt,resOpt);
    else
