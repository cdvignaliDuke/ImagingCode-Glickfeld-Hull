function positive_cells=statsFindPositiveResp (stats, alpha)

% find positively responsive cells with a statistical significance 
...
for i=1:Ncells
    ...
    if p_value_resp >=alpha | isempty(sig_epochs_resp)
        ...
        if ~isempty(find(sig_epochs_resp(:,2)==baseline))
            positive_cells=[positive_cells,i];
        end
    end
end