...
%
%$Id: imFindCells.m 267 2008-06-20 19:16:59Z histed $

%%%%%%% default values %%%%%%%%%%

defs = { 'do_hist_eq', true, ...
         'java_hist_eq', true, ...
         'no_cell_mask', [], ...
         'pre_filter_pix', [], ...
         'min_area', 80, ...
...
params = stropt2struct(stropt_defaults(defs, struct2stropt(params)));
...
%% do filter if requested
if ~isempty(params.pre_filter_pix) && params.pre_filter_pix > 0
    in = medfilt2(in, params.pre_filter_pix*[1 1]);
    if params.show_flag > 0
        figH = figure; 
        imshow(in);
        title(sprintf('Median filtered with %d pix neighborhood', ...
                      params.pre_filter_pix));
    end
end
...
%% mask with rejection mask
if ~isempty(params.no_cell_mask)
    regionmask = regionmask & ~params.no_cell_mask;
end
...
bw3 = imBreakCells(regionmask, shrinkmask, params.min_area, ...
                   params.fast_breakup, params.con_ratio, params.show_flag);
...
%% this crashes imageJ for me - histed
...
if params.do_manual == 1
    bw3 = imCellEliminate(saveadapt, bw3);
end

%select positions to add cells
if params.do_manual == 1
    bw3 = imCellAdd(saveadapt, bw3);
end
