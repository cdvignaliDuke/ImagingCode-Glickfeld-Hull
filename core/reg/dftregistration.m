...
%     rgzero = sum(abs(buf2ft(:)).^2); 
    ...
    rfzero = sum(buf1ft(:).*conj(buf1ft(:)));
    rgzero = sum(buf2ft(:).*conj(buf2ft(:))); 
...
%    rgzero = sum(abs(buf2ft(:)).^2)/(m*n); 
    ...
    rfzero = sum(buf1ft(:).*conj(buf1ft(:)))/(m*n);
    rgzero = sum(buf2ft(:).*conj(buf2ft(:)))/(m*n); 
...
%}
%SY mod
if nargin<6 || isempty(coff), coff=0; end
if nargin<5 || isempty(roff), roff=0;end
if nargin<4 ||isempty(usfac), usfac=1; end
if nargin<3 ||isempty(noc), noc=nc; end
if nargin<2 ||isempty(nor), nor=nr; end

kernc2=(ifftshift([0:nc-1]).' - floor(nc/2))*([0:noc-1] - coff)*2*pi/(nc*usfac);
kernr2=([0:nor-1].' - roff)*(ifftshift([0:nr-1]) - floor(nr/2))*2*pi/(nr*usfac);
out=exp(kernr2*(-i))*in*exp(kernc2*(-i));
