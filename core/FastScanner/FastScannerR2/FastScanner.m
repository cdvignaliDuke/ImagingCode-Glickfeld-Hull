...
% Last Modified by GUIDE v2.5 05-Nov-2007 10:48:33
...
%        get(hObject,'Min') and get(hObject,'Max') to determine range of slider
global fs;
fs.position.z=get(hObject,'Value');
...
%    putsample(fs.DAQ.aoXYMirrors,[fs.position.scale/fs.position.zoom fs.position.scale*fs.position.XtoYscale/fs.position.zoom]); %here we assume that max voltage is 5V
    putsample(fs.DAQ.aoXYMirrors,[fs.position.zoom fs.position.XtoYscale*fs.position.zoom]); %here we assume that max voltage is 5V
end
...
global fs;
if ~isempty(str2num(get(hObject,'String')))
    fs.timing.FastMirrorTriggerDelay=str2num(get(hObject,'String'));
end
set(hObject,'String',fs.timing.FastMirrorTriggerDelay);
fs.DAQ.firstdone=0;
...
function txtFastMirrorPeriod_Callback(hObject, eventdata, handles)
% Hints: get(hObject,'String') returns contents of txtFastMirrorPeriod as text
%        str2double(get(hObject,'String')) returns contents of txtFastMirrorPeriod as a double
global fs;
if ~isempty(str2num(get(hObject,'String')))
    fs.timing.FastMirrorPeriod=str2num(get(hObject,'String'));
end
set(hObject,'String',fs.timing.FastMirrorPeriod);

% --- Executes during object creation, after setting all properties.
function txtFastMirrorPeriod_CreateFcn(hObject, eventdata, handles)
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
end
...
% --- Executes on key press over btnIniStage with no controls selected.
function btnIniStage_KeyPressFcn(hObject, eventdata, handles)
% hObject    handle to btnIniStage (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
eventdata
...
% --- Executes on button press in chkUseCh1.
function chkUseCh1_Callback(hObject, eventdata, handles)
% hObject    handle to chkUseCh1 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of chkUseCh1
global fs;
fs.img.Ch(1).use=get(hObject,'Value');


% --- Executes on button press in chkUseCh2.
function chkUseCh2_Callback(hObject, eventdata, handles)
% hObject    handle to chkUseCh2 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of chkUseCh2
global fs;
fs.img.Ch(2).use=get(hObject,'Value');
