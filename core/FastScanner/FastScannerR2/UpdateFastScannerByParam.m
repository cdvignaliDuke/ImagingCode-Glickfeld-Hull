...
% fs.timing.FastMirrorPeriod
set(fs.handles.txtFastMirrorPeriod,'String',fs.timing.FastMirrorPeriod);
...
if isempty(fs.pCell.LUT)
    ...
    set(fs.handles.sliderPCellMax,'Value',cmin);
    set(fs.handles.txtPCellMax,'String',num2str(cmin));
    fs.pCell.min=cmin;

    set(fs.handles.lblpCellUnits2,'String','mW');
    set(fs.handles.sliderPCellMin,'Min',cmin);
    set(fs.handles.sliderPCellMin,'Max',cmax);
    set(fs.handles.sliderPCellMin,'Value',cmin);
    set(fs.handles.txtPCellMin,'String',num2str(cmin));
    fs.pCell.max=cmin;
end
