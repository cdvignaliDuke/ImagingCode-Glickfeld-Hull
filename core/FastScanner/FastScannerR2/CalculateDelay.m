...
%    currentImgData=currentImgData+fd.img.Ch(1).CData;
    currentImgData=currentImgData+reshape(fs.DAQ.ch(1).CData1D,256,256);
end
if fs.img.Ch(2).use
%    currentImgData=currentImgData+fd.img.Ch(2).CData;
    currentImgData=currentImgData+reshape(fs.DAQ.ch(2).CData1D,256,256);
end
...
currentDelay=fs.timing.FastMirrorTriggerDelay;
...
fs.timing.FastMirrorTriggerDelay=out;

set(fs.handles.txtFastMirrorTriggerDelay,'String',fs.timing.FastMirrorTriggerDelay);