...
set(fs.DAQ.ai, 'TriggerCondition', fs.DAQ.aiTriggerPolarity )
...
set(fs.handles.lstRanges,'Value',3); % set default input range [-1,1]
...
fs.DAQ.InPointsToCollect=round((256)*(fs.DAQ.realinputRate*fs.timing.FastMirrorPeriod/2000));
...
% receiver triggers on negative edge, set trigger channel to 0V
putsample(fs.DAQ.aoPockels,[0 fs.DAQ.aoCurrentTriggerValue]); 
...
fs.DAQ.shutterLine = addline(fs.DAQ.dio, fs.DAQ.shutterLineIndex, 'out'); % acq shutter
closeShutter;
fs.DAQ.stopLine = addline(fs.DAQ.dio, fs.DAQ.stopLineIndex,'in');
fs.DAQ.MainshutterLine = addline(fs.DAQ.dio, fs.DAQ.MainshutterLineIndex, 'out'); % manual shutter
closeMainShutter;
