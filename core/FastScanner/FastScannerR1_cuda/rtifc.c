 ...
 * $Revision: 1.1.6.12 $  $Date: 2006/10/14 12:24:28 $
 */

static char rcsid[] = "$Id: rtifc.c,v 1.1.6.12 2006/10/14 12:24:28 batserve Exp $";
...
#include <string.h>
#include "tiffio.h"
#include "tiff_utils.h"  /* For Sol64 defines. */
...
typedef struct {
    uint32_T width;
    uint32_T height;
    uint32_T depth;  /* Z-dimension, NOT samples per pixel. */
...
static void readTile(TIFF *tif, TILE *tileDetails, uint32_T x, uint32_T y, 
                     uint32_T z, void * tileBuffer);
static void changeMajority8(uint8_T *post_buffer, uint8_T *pre_buffer, 
                           TILE *tileDetails);
static void changeMajority32(uint32_T *post_buffer, uint32_T *pre_buffer, 
                           TILE *tileDetails);
...
static void getRegionDetails(REGION *regionDetails,
                             ...
                             IMAGE *imageDetails, 
                             TILE *tileDetails);
static void storeTile8(IMAGE *imageDetails, TILE *tileDetails, 
                       REGION *regionDetails,
                       uint32_T x, uint32_T y, uint32_T sample,
                       uint8_T *imageArray, uint8_T *tileBuffer);
static void storeTile32(IMAGE *imageDetails, TILE *tileDetails, 
                        REGION *regionDetails,
                        uint32_T x, uint32_T y, uint32_T sample,
                        uint32_T *imageArray, uint32_T *tileBuffer);
static void cleanup(TIFF *tif, IMAGE *imageDetails, TILE *tileDetails,
                    REGION *regionDetails);
static void getSampleLayerFromTile8(uint8_T *sampleLayer, 
                                    uint8_T *tileBuffer, 
                                    tsample_t sample,
                                    IMAGE * imageDetails, 
                                    TILE * tileDetails);
static void getSampleLayerFromTile32(uint32_T *sampleLayer, 
                                     uint32_T *tileBuffer, 
                                     tsample_t sample,
                                     IMAGE * imageDetails, 
                                     TILE * tileDetails);
static bool isSupportedTileDepth(IMAGE *imageDetails);
...
void mexFunction(int nlhs, mxArray *plhs[],
    ...
    if ((imageDetails->compressionType == COMPRESSION_JPEG) ||
    ...
    if (imageDetails->isTiled)
    {
        ...
        if (!isSupportedTileDepth(imageDetails))
        {
            cleanup(tif, imageDetails, tileDetails, regionDetails);
            mexErrMsgIdAndTxt("MATLAB:rtifc:unsupportedTiledImageDepth",
                              "Only the following tiled image types are supported:\n- 8-bit integer samples\n- 32-bit IEEE floating point samples");
        }
...
ReadIndexedGrayOrBinaryImage(TIFF *tif)             /* Grayscale, Binary, or Indexed Image */
{
    ...
    uint8_T  *ptrUint8_T, out8;
    uint16_T *ptrUint16, out16;
    uint32_T *ptrUint32_T, out32;
    real32_T *ptrFloat32;
    mxArray *outArray;
    ...
    uint32_T imageWidth,imageHeight;         /* image width, height */
   ...
   * routines. */
    ...
    /* 
     *  Comment out the following four lines to enable reading of
     *  N-bit UINT packed images (16<N<32) and 32 bit integer images
     */


    if ((bps > 16) && (sampleFormat != SAMPLEFORMAT_IEEEFP)) {
        TIFFClose(tif);
	...
	if(bps == 1) {
	    ...
	    ptrLogical = mxGetLogicals(outArray);
            ...
            outArray = mxCreateNumericArray(2, dims, mxINT8_CLASS, mxREAL); 
            ptrUint8_T = (uint8_T *) mxGetData(outArray);
        } 
        else if (bps>8 && bps<=16) {
            outArray = mxCreateNumericArray(2, dims, mxINT16_CLASS, mxREAL); 
            ptrUint16 = (uint16_T *) mxGetData(outArray);
        }
        else if (bps>16 && bps<=32) {
            outArray = mxCreateNumericArray(2, dims, mxINT32_CLASS, mxREAL); 
            ptrUint32_T = (uint32_T *) mxGetData(outArray);
        }
        else {
            TIFFClose(tif);
        ...
        /* It must be SAMPLEFORMAT_UINT or SAMPLEFORMAT_UNDEFINED.  TIFF 6.0 Spec 
         * suggests on page 80 that readers typically treat UNDEFINED as UINT.
         */
	if(bps == 1) {
	    outArray = mxCreateLogicalArray(2, dims);
	    ptrLogical = mxGetLogicals(outArray);
        ...
        for (row = 0; row < imageHeight; row++)
        {
            ...
            for (col = 0; col < imageWidth; col++)
            {
                if (photo==0) /* White is Zero - complement */
                    ptrUint8_T[row+(col*imageHeight)] = (pixmax)-scanline8[col];
                else          /* Black is zero */
                    ptrUint8_T[row+(col*imageHeight)] = scanline8[col];
            }
        }        
        mxFree(scanline8);
    }
    ...
    else if ((bps == 32) && (sampleFormat!=SAMPLEFORMAT_IEEEFP))
    {
        ...
        for (row = 0; row < imageHeight; row++)
        {
            ...
            {
                if (photo==0) /* White is Zero - complement */
                    ptrUint32_T[row+(col*imageHeight)] = (pixmax)-buf_int_32[col];
                else          /* Black is zero */
                    ptrUint32_T[row+(col*imageHeight)] = buf_int_32[col];
            }
        }        
        mxFree(buf_int_32);
    }
    ...
    else if (bps == 4)
    {
        ...
        for (row = 0; row < imageHeight; row++)
        {
            ...
            for (col = 0, imcol = 0; col < scanlineSize; col++)
            {
                if (imcol < imageWidth)
                {
                    ptrUint8_T[row + imcol*imageHeight] = 
    ...
    else if(bps<32)   
    {
        ...
        if (bps>1 && bps<8)   /* We're using uint8_T's */
        ...
        else if(bps>16 && bps<32) /* We're using uint32_T's */
        ...
        for (row = 0; row < imageHeight; row++)
        {
            ...
            if (bps>1 && bps<8) {  /* We're using uint8_T's */
                ...
                for(col=0; col < imageWidth; col++) 
                {
                    ...
                    if (photo==0) /* White is Zero - complement */
                        ptrUint8_T[row+(col*imageHeight)] = (pixmax)-(out8); 
                    else          /* Black is zero */
                        ptrUint8_T[row+(col*imageHeight)] = out8; 
                }
            }
                ...
                for(col=0; col < imageWidth; col++)  
                {
                    ...
                    if (photo==0) /* White is Zero - complement */
                        ptrUint32_T[row+(col*imageHeight)] = (pixmax)-(out32); 
                    else          /* Black is zero */
                        ptrUint32_T[row+(col*imageHeight)] = out32; 
                }
            }
        }
...
getNthKbitNumberFromScanline(void *out, int n, int bps, uint8_T *scanline)
{
    ...
    /* Case 1: The output is stored in uint8_T's */
    if(bps>=1 && bps<=8) {
    ...
    /* Case 3: The output is stored in uint32_T's */
    else if(bps>16 && bps<=32) {
   ...
   * routines. */
    ...
    if (bps==8) {
        if (sampleFormat == SAMPLEFORMAT_UINT) {
            ...
            outArray = mxCreateNumericArray(3, dims, mxINT8_CLASS, mxREAL); 
        }            
    ...
    if (bps==8) {
        ...
        *ptrSamples8 = (uint8_T *) mxGetData(outArray);
        ...
        if (sampleFormat == SAMPLEFORMAT_UINT) {
            ...
            outArray = mxCreateNumericArray(3, dims, mxINT16_CLASS, mxREAL); 
        }
...
void getImageDetails(TIFF *tif, IMAGE *imageDetails)
{
    ...
    TIFFGetFieldDefaulted(tif, TIFFTAG_BITSPERSAMPLE,
                          &(imageDetails->bps));
    TIFFGetFieldDefaulted(tif, TIFFTAG_SAMPLEFORMAT,
                          &(imageDetails->sampleFormat));
    TIFFGetFieldDefaulted(tif, TIFFTAG_PLANARCONFIG,
                          &(imageDetails->planarConfiguration));
    TIFFGetFieldDefaulted(tif, TIFFTAG_IMAGEDEPTH,
                          &(imageDetails->depth));
...
void getTileDetails(TIFF *tif, TILE *tileDetails)
{
    ...
    tileDetails->tileBytes = TIFFTileSize(tif);
...
void createOutputArray(mxArray **outArray, IMAGE *imageDetails)
{
    ...
    {
        if (imageDetails->sampleFormat == SAMPLEFORMAT_INT)
        {
            *outArray = mxCreateNumericArray(3, dims, mxINT8_CLASS, mxREAL);
        }
        else
    ...
    {
        if (imageDetails->sampleFormat == SAMPLEFORMAT_IEEEFP)
        {
            *outArray = mxCreateNumericArray(3, dims, mxSINGLE_CLASS, mxREAL);
        }
        else if (imageDetails->sampleFormat == SAMPLEFORMAT_INT)
        {
            *outArray = mxCreateNumericArray(3, dims, mxINT32_CLASS, mxREAL);
        }
        else
        {
                    ...
                    mxArray *outArray, REGION *region)
{
    ...
    tileBuffer = (void *) mxMalloc(tileDetails->tileBytes);

    /* Get the samples for the pixels within each of the required tiles. */
    for (y = region->minY; y < region->maxY; y += (tileDetails->height))
    {
        for (x = region->minX; x < region->maxX; x += (tileDetails->width))
        {

            /* Read the sample(s) from each tile and store in the buffer. */
            if (imageDetails->planarConfiguration == PLANARCONFIG_SEPARATE)
            {

                /* If the planar configuration is "separate", samples
                 * are accessed by different calls to TIFFReadTile(). */

                uint8_T * permutedLayer;

                permutedLayer  = (uint8_T *) mxMalloc(tileDetails->tileBytes);

                for (sample = 0; 
                     sample < imageDetails->samplesPerPixel; 
                     sample++)
                {
                    
                    /* Read the tile, permute, and store. */
                    TIFFReadTile(tif, tileBuffer, x, y, 0, sample);

                    if (imageDetails->bps == 8)
                    {
                        changeMajority8(permutedLayer, 
                                        tileBuffer, 
                                        tileDetails);

                        storeTile8(imageDetails, tileDetails, region,
                                   x, y, sample, 
                    ...
                    {
                        changeMajority32((uint32_T *) permutedLayer,
                                         (uint32_T *) tileBuffer, 
                                         tileDetails);

                        storeTile32(imageDetails, tileDetails, region,
                                    x, y, sample, 
                                    (uint32_T *) mxGetData(outArray), 
                                    (uint32_T *) permutedLayer);
                    }

                }
                
                mxFree(permutedLayer);
 
            }
            else  /* PLANARCONFIG_CONTIG */
            {
                
                /* If the planar configuration is "contiguous",
                 * samples are interleaved by pixel.  All samples are
                 * returned in one call to TIFFReadTile(). */
                ...
                layerBytes = mxGetElementSize(outArray) * tileDetails->height *
                    tileDetails->width;

                sampleLayer = (uint8_T *) mxMalloc(layerBytes);
                permutedLayer = (uint8_T *) mxMalloc(layerBytes);
                     ...
                     sample++)
                {
                    ...
                    if (imageDetails->bps == 8)
                    {
                        /* Tease apart samples from tile. */
                        getSampleLayerFromTile8(sampleLayer, tileBuffer, 
                                                sample, imageDetails, tileDetails);

                        /* Permute each sample-tile. */
                        changeMajority8(permutedLayer, sampleLayer, tileDetails);
                        
                        /* Store each sample-tile. */
                        storeTile8(imageDetails, tileDetails, region,
                                   x, y, sample, 
                                   (uint8_T *) mxGetData(outArray), 
                                   (uint8_T *) permutedLayer);
                    ...
                    {
                        /* Tease apart samples from tile. */
                        getSampleLayerFromTile32((uint32_T *) sampleLayer, 
                                                 (uint32_T *) tileBuffer, 
                                                 sample, 
                                                 imageDetails, 
                                                 tileDetails);
...
void changeMajority8(uint8_T *post_buffer, uint8_T *pre_buffer, 
                    TILE *tileDetails)
{

    uint32_T pre_width = tileDetails->width;
    uint32_T pre_height = tileDetails->height;
    uint32_T x, y;
    uint32_T idx;
...
void changeMajority32(uint32_T *post_buffer, uint32_T *pre_buffer, 
                    TILE *tileDetails)
{
                       ...
                       uint8_T *imageArray, uint8_T *tileBuffer)
{
    ...
    imagePos = startX * imageDetails->extentRows + startY + 
        sample * imageDetails->extentRows * imageDetails->extentCols;
    tilePos = 0;
                                    ...
                                    TILE * tileDetails)
{
    ...
    /* If the samples are already separated, return the same data. */
    if (imageDetails->planarConfiguration == PLANARCONFIG_SEPARATE)
    {
        
        memcpy(sampleLayer, tileBuffer, tileDetails->tileBytes);
        return;

    }

    /* Traverse the tile, getting all of one sample.
     * Assumes Z depth is 1. */
    for (layerIndex = 0;
         layerIndex < (tileDetails->height *
                       tileDetails->width);
         layerIndex++)
    {
        
        sampleLayer[layerIndex] = tileBuffer[(layerIndex
                                              * imageDetails->samplesPerPixel)
                                             + sample];
...
static bool isSupportedTileDepth(IMAGE *imageDetails)
{
  ...
  if (imageDetails->bps == 8)
  {
      return true;
           ...
           (imageDetails->sampleFormat == SAMPLEFORMAT_IEEEFP))
  {
      return true;
  }
  else
