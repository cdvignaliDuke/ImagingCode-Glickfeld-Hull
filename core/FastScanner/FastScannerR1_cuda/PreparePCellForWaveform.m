...
% OUTDATED generate a pulse instead (for the ced box) VB 12/13/07
% ced box could not be triggered from zquad. now using vis stimulation to 
% trigger ced box

% generate low  signal upon acquisition. gate for pulse counter estim
if floor(oneFrameLe/2)==oneFrameLe/2
%    fs.pCell.TTLout=7*[ones(floor(oneFrameLe/2),1);zeros(floor(oneFrameLe/2)+1,1)]; 
    fs.pCell.TTLout=7*[ones(floor(oneFrameLe/2),1);ones(floor(oneFrameLe/2)+1,1)]; 
else
%    fs.pCell.TTLout=7*[ones(floor(oneFrameLe/2),1);zeros(floor(oneFrameLe/2)+2,1)]; 
    fs.pCell.TTLout=7*[ones(floor(oneFrameLe/2),1);ones(floor(oneFrameLe/2)+2,1)]; 
end
