...
if isempty(options.TargetFileNs)
    ...
    fileNs = options.TargetFileNs;
end

green = readtiff(expt.dirs.rawgreenpn,fileNs);
...
for iP = 1:nP
    targets(:,:,iP) =mean(green(:,:,1+((iP-1)*nF):((iP-1)*nF)+nF),3);
end;
...
for iFile =1:nFiles            
    ...
    for iP = 1:nP % loop over starting indices
        %iP = mod(nn + iS - 1,nP) + 1;
...
%         drawnow;
        ...
        if strcmp(options.Engine,'subpixel')
            [outputs{iFile},stack_reg] = stackRegister(stack(:,:,1+((iP-1)*nF):((iP-1)*nF)+nF),targets(:,:,iP), os);
            
        elseif strcmp(options.Engine,'turboreg')
            stack_reg = RegMulticore(stack(:,:,1+((iP-1)*nF):((iP-1)*nF)+nF),targets(:,:,iP), [], dirs.multicore, 3);
            outputs{iFile}=[];
        end;
